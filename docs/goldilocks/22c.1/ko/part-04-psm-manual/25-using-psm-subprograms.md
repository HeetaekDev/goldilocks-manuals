...
자세한 내용은 [Block (BEGIN .. END)](28-psm-language-element-references.md#715afecb623f3a43)을 참조한다.
...
자세한 내용은 [Procedure Declaration and Definition](28-psm-language-element-references.md#bbf3372ce8f689c4)을 참조한다.
...
<a id="82b0350081c10e78"></a>
## Nested Function

Nested function은 nested procedure와 동일하지만 함수 형태를 가지는 subprogram이다.  
자세한 내용은 [Function Declaration and Definition](28-psm-language-element-references.md#5c723265ff63c02b)을 참조한다.
...
자세한 내용은 [ROUTINES](../part-02-administration-manual/9-database-information.md#dc58b1fde9df59d6)를 참조한다.
...
자세한 내용은 [PARAMETERS](../part-02-administration-manual/9-database-information.md#7e5aab65e02e5cf0)를 참조한다.
...
자세한 내용은 [CALL Statement](29-psm-sql-references.md#4d37fd7827c46acf)를 참조한다.
...
<a id="d0955332ce6ef44e"></a>
### 제거

Schema-level procedure는 다음 DROP PROCEDURE 구문으로 제거한다.  
자세한 내용은 [DROP PROCEDURE](29-psm-sql-references.md#d4f590ab9ed9207b)를 참조한다.
...
<a id="9b1e3b7322381d84"></a>
### 제거

Schema-level function은 다음 DROP FUNCTION 구문으로 제거한다.  
자세한 내용은 [DROP FUNCTION](29-psm-sql-references.md#84b30fac2e4fd826)을 참조한다.
...
| DBMS_OUTPUT.SET_LOG( file_path IN VARCHAR(4000) ) | 메시지를 로깅할 때 주어진 경로에 있는 파일에도 동시에 출력한다. 만일 상대 경로 (첫 글자가 /가 아닌 경우)로 주어지면 &lt;SYSTEM_LOGGER_DIR&gt; 프로퍼티에 해당하는 디렉토리 아래에서 대상 파일을 찾는다.  다음과 같이 permission을 지정할 수 있다. * CALL DBMS_OUTPUT.SET_LOG( 'a.txt' , 640 ) |
| DBMS_OUTPUT.PUT_LINE( item IN VARCHAR(4000) ) | 주어진 expression으로 만들어진 메시지를 개행문자를 포함하여 버퍼에 저장한다. |
| DBMS_OUTPUT.PUT( item IN VARCHAR(4000) ) | 주어진 expression으로 만들어진 메시지를 버퍼에 저장한다. |
| DBMS_OUTPUT.NEW_LINE | 개행문자를 버퍼에 저장한다. |
| DBMS_OUTPUT.GET_LINE( line OUT VARCHAR(4000), status OUT NATIVE_INTEGER ) | 버퍼에 저장된 메시지들 중에 아직 읽지 않은 가장 오래된 메시지 라인을 한 줄 반환한다. 메시지가 존재하면 status는 0을 반환하고 없으면 1을 반환한다. |
| DBMS_STANDARD.RAISE_APPLICATION_ERROR( error_code IN NATIVE_INTEGER, error_message IN VARCHAR(4000), stack_flag IN BOOLEAN := FALSE ) | 임의의 사용자 exception을 발생시킨다. error_code는 -20000 ~ -20999 사이의 값이어야 하며, TRUE일 경우에는 마지막 stack_flag 인자가 기존 error들 위에 주어진 에러를 쌓고 FALSE이면 해당 에러가 모든 에러를 대체 (replace) 한다. (생략할 수 있고 이 경우 default는 FALSE이다.) |
| DBMS_LOCK.SLEEP( seconds IN NUMBER ) | 지정된 시간 동안 세션을 일시 중단한다. |
| DBMS_SQL.RETURN_RESULT( rc IN SYS_REFCURSOR ) | ref cursor를 통해 실행된 질의 결과를 클라이언트 어플리케이션에 반환한다. |

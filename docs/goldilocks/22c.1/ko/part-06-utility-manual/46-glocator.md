...
<a id="3e5a57b195c603dd"></a>
## glocator 소개

<a id="05cb5959d1ed8b26"></a>
### 정의

glocator는 GOLDILOCKS cluster system에서 client에게 location을 제공하고 관리하는 유틸리티이다.  
glocator 프로그램은 [gloctl](48-gloctl.md#93f88cc11d2100c5)를 통해 cluster member 노드들의 location 정보를 제공받는다.  
...
<a id="5492d13987989106"></a>
### Option

<a id="fb8492851388bb48"></a>
#### help

<a id="0a630ff11cd3fec3"></a>
##### 설명

Help 메시지를 출력한다.

<a id="bea1a16a64d14b17"></a>
##### 사용 예
...
<a id="18441acb08132389"></a>
#### create

<a id="ec02401e25988f7b"></a>
##### 설명

glocator의 데이터 파일을 생성한다.

<a id="8b6714ecab5874e7"></a>
##### 사용 예
...
<a id="acbe359dd6eda48c"></a>
#### start

<a id="2a7501a447052d91"></a>
##### 설명

glocator를 구동한다. 같은 port를 갖는 glocator가 이미 구동 중이라면 에러가 발생한다.

<a id="fa2fae99f899609c"></a>
##### 사용 예
...
<a id="a4abd8a05f2b6c04"></a>
#### stop

<a id="d80ddfe1f94ee857"></a>
##### 설명

구동 중인 glocator를 정지시킨다. 구동 중인 glocator를 멈추려면 같은 port가 설정되어야 한다.

<a id="8c8807325e1893e7"></a>
##### 사용 예
...
<a id="9e82d6ae23e19f3a"></a>
#### conf

<a id="33085570ffa951fe"></a>
##### 설명

glocator를 구동할 때 configure file을 설정한다.

<a id="c314e6e5002d6370"></a>
##### 사용 예
...
<a id="6c40dc66cbac2bf8"></a>
#### sync

<a id="fcb8c4f7b60f2fee"></a>
##### 설명
...
<a id="7432b96d246b49c7"></a>
#### silent

<a id="9d3d0be3d6c27719"></a>
##### 설명

실행에 대한 glocator의 메시지를 출력하지 않는다.

<a id="11f3294ac50c0a77"></a>
##### 사용 예
...
<a id="93b578cc0f2205f6"></a>
#### no-copyright

<a id="3426c980351aa00d"></a>
##### 설명

실행에 대한 glocator의 copy right와 버전 메시지를 출력하지 않는다.

<a id="8b5aabca31a8a863"></a>
##### 사용 예
...
<a id="7d4fac2a1ebd49c1"></a>
## glocator 사용

<a id="07f8b20dd139f72c"></a>
### 데이터 파일
...
glocator 데이터 파일이 저장되는 디렉토리는 configuration [LOCATION_FILE_DIR](#3687c49bcd23c9d2)을 수정하여 변경할 수 있다. 기본값은 &lt;GOLDILOCKS_DATA&gt;/db에 생성된다. 데이터 파일은 [LOCATION_FILE_NAME](#ca5fbd3e08c18a92) 을 수정하여 변경할 수 있다. 기본값은 glocator.dat이다.

glocator 데이터 파일의 최대 크기와 초기 크기는 configuration [LOCATION_FILE_MAX_SIZE](#48b6c955e5121bbf)와 [LOCATION_FILE_SIZE](#f29abff36f28fb32)를 수정하여 변경할 수 있다.

<a id="18f049677dd2e83f"></a>
### CSTARTUP과 CSHUTDOWN
...
자세한 내용은 [GOLDILOCKS UNIX ODBC driver 라이브러리](../part-05-developer-manual/31-odbc.md#2335fc2f2a01bf3c)를 참조한다.
...
> - glocator와 관련된 ODBC와 gagent 설정에 대한 자세한 내용은 [odbc.ini 파일](../part-05-developer-manual/31-odbc.md#6eb05a63d0c64f05), [ALTERNATE_LOCATORS](47-gagent.md#def3656ca1d2c38d)을 참조한다.
> 

<a id="1e9cd268fe1e6c15"></a>
#### 데이터 동기화

다중화된 glocator를 서비스할 때 데이터는 일관되게 유지되어야 한다. 기존에 작동 중인 glocator가 없을 경우, 모든 glocator는 일반 시작을 하면 된다. 기존에 동작 중인 glocator가 있을 경우, [sync](#6c40dc66cbac2bf8) 옵션을 사용하여 데이터를 동기화하는 방법으로 alternate glocator를 시작할 수 있다.

[sync](#6c40dc66cbac2bf8) 옵션에는 glocator 자신의 데이터와 alternate glocator의 데이터를 병합하는 BOTH와 상대 glocator로부터 데이터를 가지고 오는 SOURCE가 있다. glocator의 sync 작업은 glocator와 alternate glocator 사이의 1 : 1 대응 작업이다. 따라서 A, B, C 세 개의 glocator를 다중화하려고 할 때 A와 B가 이미 구동 중이고 C를 BOTH 방식으로 sync하여 구동하면 A 또는 B glocator의 데이터가 서로 다를 수 있다.

서비스 중인 glocator는 변경된 내용만 동기화한다. 패킷 손실 등의 이유로 데이터 동기화에 실패하는 경우 데이터가 서로 다를 수 있다. 이 경우, [gloctl](48-gloctl.md#93f88cc11d2100c5) 프로그램을 사용하여 해당 glocator에서 직접 데이터를 수정하거나 glocator를 sync 옵션으로 다시 시작할 수 있다.

<a id="6ec3886911bde397"></a>
## 기능

<a id="04d10f2d8e562928"></a>
### Connection Service
...
응용 프로그램은 ODBC 드라이버를 사용하여 접속해야 하며 [odbc.ini 파일](../part-05-developer-manual/31-odbc.md#6eb05a63d0c64f05)에 LOCATOR_DSN과 LOCATOR_SERVICE 속성을 지정해야 한다.

<a id="20091058b644be9a"></a>
![locator_service](../assets/images/0a62a8954e5984cf.png)
...
<a id="e3cefd63f5e7f99c"></a>
### Cluster Failover

glocator는 server가 failover를 진행할 때 도움이 된다.

Server 프로퍼티 [CLUSTER_SPLIT_BRAIN_RESOLUTION_POLICY](../part-02-administration-manual/10-server-property.md#5952b20d977ac530) 값이 1 또는 2로 설정되고, cluster 환경에서 노드 간에 연결이 끊어져 failover가 발생하면 각 노드는 cluster failover를 진행하여 gagent를 통해 glocator에게 자신의 viability를 질의한다.
...
<a id="6c459ff54427b219"></a>
### Configuration Property

<a id="6c8cc99ec1a81977"></a>
#### HOST

<a id="f0ca3cf343abc126"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | HOST |
...
<a id="c215dfae62c18e35"></a>
#### PORT

<a id="8ae10454a04d3439"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | PORT |
...
<a id="b53c412a7be41ab5"></a>
#### WORKER_COUNT

<a id="c27dda4759121936"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | WORKER_COUNT |
...
<a id="fc1a6352d7be1b50"></a>
#### MAX_NODE_COUNT

<a id="96253c8ba754b88a"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | MAX_NODE_COUNT |
...
<a id="05a4646b2ca7a79c"></a>
#### MESSAGE_QUEUE_SIZE

<a id="0ff4c5a74180a0ad"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="1dec8446aab415c1"></a>
#### MESSAGE_ALLOCATOR_SIZE

<a id="81672dc54b4a4421"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="bf8fba8c57b26af4"></a>
#### PACKET_ALLOCATOR_SIZE

<a id="d9a938b41a9de727"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="fe3c8b351192f1cb"></a>
#### SYSTEM_LOGGER_DIR

<a id="99a25320e3c4bd10"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="7e3498566c3ce6ce"></a>
#### SYSTEM_UDS_DIR

<a id="3d0b43f466feb491"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | SYSTEM_UDS_DIR |
...
<a id="3687c49bcd23c9d2"></a>
#### LOCATION_FILE_DIR

<a id="01fcad9e5cbea828"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="ca5fbd3e08c18a92"></a>
#### LOCATION_FILE_NAME

<a id="52eab2a5ba72f914"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="f29abff36f28fb32"></a>
#### LOCATION_FILE_SIZE

<a id="7859f4262ae89108"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="48b6c955e5121bbf"></a>
#### LOCATION_FILE_MAX_SIZE

<a id="11e84728e1043f9f"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="47404db619320479"></a>
#### MESSAGE_TIMEOUT

<a id="854519708178c8b5"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | MESSAGE_TIMEOUT |
...
<a id="abff2dea5fab3d7e"></a>
#### ALTERNATE_LOCATORS

<a id="dd99b0d6ffb35413"></a>
| 항목 | 설명 |
| --- | --- |
...
glocator의 [다중화](#a25ec77e782b3400)를 설정한다.

<a id="b1d6197c32652679"></a>
#### SYNC_RETRY_COUNT

<a id="b3aadace8ba4c5f5"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | SYNC_RETRY_COUNT |
...
<a id="e5e30cdd480cf966"></a>
#### SYNC_RESPONSE_TIMEOUT

<a id="291850a2f815c095"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="37359d9abad94d1b"></a>
#### KEEPALIVE_IDLE_TIME

<a id="2b3f0629fc3ba39f"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="e36192676c223046"></a>
#### KEEPALIVE_COUNT

<a id="336908040a00378b"></a>
| 항목 | 설명 |
| --- | --- |
| 이름 | KEEPALIVE_COUNT |
...
<a id="439deb01c2b722ce"></a>
#### KEEPALIVE_INTERVAL

<a id="028144ddef3cce54"></a>
| 항목 | 설명 |
| --- | --- |

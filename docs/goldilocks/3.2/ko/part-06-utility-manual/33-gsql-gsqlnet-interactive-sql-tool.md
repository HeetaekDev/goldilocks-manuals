<a id="c5ffba923045082c"></a>

# 33. gsql/gsqlnet (Interactive SQL Tool)

> 원본: [GOLDILOCKS 3.2 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/3_2_x/manual/ko/c5ffba923045082c)  
> 태그: `GOLDILOCKS_Venus_3_2_14_retagged`

[← 32. glsnr](32-glsnr.md) · [전체 목차](../README.md) · [34. gloader/gloadernet (Upload/download Tool) →](34-gloader-gloadernet-upload-download-tool.md)

<a id="a2ebcc6a3c1f35b8"></a>
## gsql 개요
...
gsql 프로그램은 SQL 구문을 수행할 뿐만 아니라 `\` 로 시작하는 gsql 고유의 명령어를 통해 테이블과 인덱스 등의 간략한 객체 정보를 조회할 수 있으며 서버 구동과 종료, 출력 결과의 제어, SQL 구문의 실행 계획 정보 조회 등의 다양한 기능들을 제공한다.

<a id="7b59c3be12bed4bd"></a>
### 사용 예
...
다음과 같이 [CREATE TABLE](../part-03-sql-manual/16-sql-references.md#1586c5952309fa38) 구문을 수행하여 테이블을 생성하고 트랜잭션을 COMMIT 한다.
...
생성한 테이블에 [INSERT INTO](../part-03-sql-manual/16-sql-references.md#3d30a4728f25da9f) 구문을 수행하여 데이터를 추가하고 트랜잭션을 COMMIT 한다. 두 번째 INSERT 구문은 두 개의 row를 추가하는 예이다.
...
다음은 테이블에 addr column을 추가하고 [UPDATE](../part-03-sql-manual/16-sql-references.md#545d9c2796a3df91) 구문을 수행하여 데이터를 갱신하는 예이다.
...
다음과 같이 입력된 데이터를 [SELECT](../part-03-sql-manual/16-sql-references.md#c9d76bf073f60db2) 질의를 이용하여 조회한다. 질의 결과는 column 이름을 표현하는 header와 각 row를 하나의 line에 출력한 정보로 구성되어 있다. NULL 값은 소문자 null로 표현된다.
...
[`\prepare sql`](#4da67701b4fdc1fc) 명령을 이용해 SQL 구문을 준비하고 호스트 변수의 값을 바꿔가며 [`\exec`](#7fbfc7289468d89b) 명령을 이용해 위의 SELECT 구문을 반복 수행한다. 다음과 같이 gsql 대화형 모드에서 ODBC와 JDBC의 prepare/ execute 동작을 simulation 할 수 있다.
...
다음은 embedded SQL, ODBC, JDBC에서 scrollable 커서를 선언하고 gsql을 통해 커서를 이용한 데이터 검색을 simulation 하는 예이다. [DECLARE cursor_name](../part-03-sql-manual/16-sql-references.md#c0f5909b51d661a3) 구문을 이용해 선언한 커서 cur1은 스크롤이 가능한 KEYSET 커서로써 [FETCH cursor_name](../part-03-sql-manual/16-sql-references.md#f8b0914239210b68) 구문을 이용해 다양한 fetch orientation을 사용할 수 있다.
...
<a id="c26fe53a4c14cb39"></a>
## gsql 실행

<a id="791790962d1f2b34"></a>
### gsql 정보
...
<a id="07c23cfc58d20b32"></a>
| 옵션 | 설명 |
| --- | --- |
| [--help](#d696467b0e8fc963) | 옵션 목록 |
| [--version](#6d387efaf97c8d97) | gsql 버전 정보 |

Shell 프롬프트 상에서 수행할 수 있는 gsql의 command option은 다음과 같이 [--help](#d696467b0e8fc963) 옵션을 통해 확인할 수 있다.
...
gsql 프로그램은 GOLDILOCKS 서버와 달리 별도의 version을 가지는데 다음과 같이 [--version](#6d387efaf97c8d97) 옵션을 사용하여 확인할 수 있다. GOLDILOCKS 서버의 version과 동일한 gsql 프로그램을 사용할 것을 권장한다.
...
참고로 GOLDILOCKS 서버의 version은 다음과 같이 [VERSION](../part-03-sql-manual/11-sql-elements.md#de2cfe412d1f521d) 함수를 통해 확인할 수 있다.
...
<a id="2419c03231257fc6"></a>
| 옵션 | 설명 |
| --- | --- |
| [Username Password](#bbbd98003191d7b4) | 접속 사용자와 패스워드이다. |
| [--as {SYSDBA\|ADMIN}](#43473a7ea7a63469) | 접속 role을 지정한다. |
| [--conn-string](#f5a726055c723b39) | Connection string을 지정한다. |
| [--dsn](#a42abf1325863d59) | Data Source Name (DSN)을 지정한다. |

<a id="38aa1bed609055ab"></a>
#### 사용자 접속
...
- [--conn-string](#f5a726055c723b39)
- [--dsn](#a42abf1325863d59)

<a id="35d5b3d4a24f8aef"></a>
#### SYSDBA 접속

서버를 구동하거나 database에 대한 모든 권한을 가진 SYSDBA role 등에 접속하려면 [--as {SYSDBA|ADMIN}](#43473a7ea7a63469) 옵션을 사용해야 한다.
...
서버 구동 및 종료에 대한 자세한 내용은 [서버 구동 및 종료](#27035efe8472921f)를 참조한다.

<a id="b35769e3502b63e0"></a>
#### gsql 종료

gsql은 다음과 같이 `\quit` 또는` \q` 명령을 이용하여 종료한다.   
자세한 내용은 [`\quit`](#a3ad7dc41be165ea)을 참조한다.
...
<a id="14075409454f5f49"></a>
| 옵션 | 설명 |
| --- | --- |
| [--prompt](#fbc8ad011840c281) | 프롬프트를 지정한다. |
| [--no-prompt](#2cd178091ca43fde) | 프롬프트를 제거한다. |
| [--import](#2cbe6ffb8b3af2ea) | 입력으로 SQL 파일을 실행한다. |
| [--silent](#b1ae2abadd9555d8) | SQL 파일을 실행할 때 명령어와 실행 결과를 출력하지 않는다. |
| [--enable-color](#68dca1e8faaea67a) | 질의 결과를 row 별로 구분하여 출력한다. |

<a id="3acac03f25fb6b4d"></a>
#### 프롬프트 제어
...
- [--prompt](#fbc8ad011840c281)
- [--no-prompt](#2cd178091ca43fde)

<a id="a33a01bc3e3fcc29"></a>
#### 파일로부터 수행
...
- [--import](#2cbe6ffb8b3af2ea)
- [--silent](#b1ae2abadd9555d8)
- [--enable-color](#68dca1e8faaea67a)

<a id="c9fd6d3ee2b970fb"></a>
### gsql 설정 저장

<a id="45456e36f4539f21"></a>
#### gsql 환경 파일 (gsql.ini)
...
<a id="8488576a1d2c0289"></a>
| 옵션 | 최소값 | 최대값 | 기본값 |
| --- | --- | --- | --- |
| [AUTOCOMMIT](#bbcc1db8d869ee1c) | OFF | ON | OFF |
| [AUTOTRACE](#2372df662eef28fe) | OFF | ON/TRACEONLY | OFF |
| [LINESIZE](#8ad8e5ae9481dd53) | 1 | 10000 | 80 |
| [PAGESIZE](#f3d95aa99a1f9d27) | 1 | 10000 | 20 |
| [VERTICAL](#962c28f61f26211b) | OFF | ON | OFF |
| [TIME](#1e2ed68ed1d593fc) | OFF | ON | OFF |
| [TIMING](#1fc85fd41709471a) | OFF | ON | OFF |
| [ERROR](#0d801fe08ac45f8e) | ON | OFF | ON |
| [COLSIZE](#7fe81892940edbed) | 1 | 10485760 | 8192 |
| [NUMSIZE](#2030abff10987b43) | 1 | 50 | 20 |
| [DDLSIZE](#2b6c88dca7920a01) | 1 | 10485760 | 10000 |
| [HISTORY](#e80c3f97a399cc1c) | 0 | 100000 | 128 |

> .gsql.ini가 없거나 옵션값이 유효하지 않은 경우에는 이를 무시한다.

<a id="654a0a47df880078"></a>
#### glogin.sql
...
<a id="17b462820cca4761"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\help`](#1d4da2ee1a10931a) | gsql 명령어 목록 |

gsql 프로그램은 [서버 접속](#7134eeaff4a1a5d1)을 통해 다음과 같은 대화형 모드로 동작한다.
...
사용자는 프롬프트 상에서 SQL 구문을 입력하여 수행하거나 gsql 대화형 모드에서 수행할 수 있는 고유 명령어를 입력하여 수행할 수 있다. gsql 고유 명령어는 SQL 구문과 구별하기 위해 `back-slash(\)`로 시작한다. 즉, SQL 구문은 응용 프로그램에서 직접 사용할 수 있는 반면에 `back-slash(\)`로 시작하는 gsql 명령어는 응용 프로그램에서 사용할 수 없다.
...
다음은 gsql 명령어 [`\exec sql`](#35765f534b203586)을 사용하여 동일한 SELECT 구문을 사용하는 예이다. `\exec sql` 명령은 [Embedded SQL](../part-05-developer-manual/27-embedded-sql.md#36b9dbffe1044ae4)의 SQL 구문 시작을 의미하는 EXEC SQL과 구문이 동일할 뿐 gsql 고유의 명령어이다.
...
[`\set time`](#1e2ed68ed1d593fc)을 통해 현재 시간을 프롬프트에 출력하도록 설정할 수 있다.
...
gsql 대화형 모드의 고유 명령어들은 다음과 같이 [`\help`](#1d4da2ee1a10931a) 명령을 이용해 조회할 수 있다.
...
<a id="d15003f50d802070"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\startup`](#f8f8557c1ad2c1a6) | 서버를 구동한다. |
| [`\shutdown`](#b18e96eee57f8243) | 서버를 종료한다. |
| [`\cstartup`](#a26626f2caa96c6d) | Cluster 환경을 위한 서버를 구동한다. |
| [`\cshutdown`](#104a75b925ad3bbe) | Cluster 환경을 위한 서버를 종료한다. |

<a id="6970b9bc3f903ce7"></a>
#### Standalone
...
- ADMIN role
    ...
    - 유효 session이 없거나 비정상적인 상황에서의 비상 접속을 위한 role이다.
...
gsql은 GOLDILOCKS 서버가 없는 상태에서 idle instance에 접속하고 [`\startup`](#f8f8557c1ad2c1a6)명령을 통해 GOLDILOCKS 서버를 특정 단계로 구동하여 서버에 접속한다. 다음은 NOMOUNT 단계로 GOLDILOCKS 서버를 구동하는 예이다.
...
GOLDILOCKS를 최초로 구동할 때만 [`\startup`](#f8f8557c1ad2c1a6)명령을 사용하며 GOLDILOCKS 서버를 구동하고 서버에 접속한 후에는 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8) 구문을 이용해 GOLDILOCKS 서버의 각 단계로 전환한다.
...
GOLDILOCKS 서버를 별도의 단계 전환 없이 서비스 가능한 OPEN 단계로 구동할 경우, 다음과 같이 별도의 옵션없이 `\startup` 명령을 수행한다.
...
- 서버 구동 단계: [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#d48808b833e799ba)
- 최초 서버 구동 명령: [`\startup`](#f8f8557c1ad2c1a6)
- 서버 단계 변경: [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8)

서버를 종료하려면 다음과 같이 `\shutdown` 명령을 사용한다. 서버 종료에 대한 자세한 내용은 [`\shutdown`](#b18e96eee57f8243) 명령을 참조한다.
...
다음은 [odbc.ini 파일](../part-05-developer-manual/25-odbc.md#78b0fc955ee2c6e5)에 LOCATOR_DSN 속성과 LOCATOR DSN이 정의된 예이다.
...
`\CSTARTUP {LOCAL | GLOBAL} OPEN` 이외의 명령어는 해당 GOLDILOCKS 서버에만 적용된다. 따라서 `\CSTARTUP NOMOUNT, MOUNT` 명령어 이후 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8) 을 이용하여 단계별로 구동하기 위해서는 다른 서버들도 LOCAL OPEN 상태까지 직접 구동해야 한다.
...
- 서버 구동 단계: [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#d48808b833e799ba)
- 최초 서버 구동 명령: [`\cstartup`](#a26626f2caa96c6d)
- 서버 단계 변경: [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8)

전체 서버를 종료하려면 다음 예와 같이 `\cshutdown` 명령을 사용한다. 전체 서버 종료에 대한 자세한 내용은 [`\cshutdown`](#104a75b925ad3bbe) 명령을 참조한다.
...
<a id="71c71771e972a489"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\import`](#53c60644a1c9756d) | SQL 파일을 실행한다. |
| [`\set autocommit`](#bbcc1db8d869ee1c) | SQL 구문을 실행할 때마다 자동으로 COMMIT 한다. |
| [SQL References](../part-03-sql-manual/16-sql-references.md#e2e6df00f51b29a6) | SQL 구문의 종류이다. |

<a id="61358e703e09edd4"></a>
#### SQL 구문 입력
...
<a id="c9caef83d0fef5cf"></a>
#### 파일로부터 SQL 구문 입력

대화형 모드에서 SQL 구문을 직접 입력하지 않고 파일로부터 수행할 경우, `\import` 명령을 사용한다. 다음은 sample.sql 파일을 읽어 다수의 SQL 구문들을 수행하는 예이다. 자세한 내용은 [`\import`](#53c60644a1c9756d) 명령을 참조한다.
...
<a id="6f0cd9ace541629d"></a>
#### gsql 주석

대화형 모드에서 사용되는 주석은 SQL 구문의 주석과 동일하다. [--import](#2cbe6ffb8b3af2ea) 옵션을 통해 파일로부터 입력되는 주석도 SQL 구문의 주석과 동일하다. SQL 구문의 주석에 대한 자세한 내용은 [Comments](../part-03-sql-manual/11-sql-elements.md#ce1ba3f06cc825a0)를 참조한다.
...
gsql 대화형 모드에서 사용할 수 있는 주석은 SQL 구문의 주석과 동일하며 주석에 대한 자세한 내용은 SQL 구문의 [Comments](../part-03-sql-manual/11-sql-elements.md#ce1ba3f06cc825a0)를 참조한다.

<a id="ea6ac3bc756a29ac"></a>
#### SQL 수행 결과
...
SQL 질의 이외의 [Data Definition Language](../part-03-sql-manual/12-sql-languages.md#f4a4a3c909b3a7fb), [Data Manipulation Language](../part-03-sql-manual/12-sql-languages.md#6d7636b0b6c273ce), [Control Language](../part-03-sql-manual/12-sql-languages.md#c46b8bb2f516cbcc) 등의 구문은 각 SQL 구문의 특성에 맞는 수행 결과를 요약한 정보를 출력한다.
...
다음은 SELECT 구문을 수행할 때 에러가 발생하는 예이다. ERR-42000 값이 SQLSTATE에 해당하며, (16040) 값이 GOLDILOCKS 고유의 error code이고 두 번째 라인에서 invalid_table에 해당하는 테이블이나 view가 존재하지 않는다는 에러 메시지로 구성되어 있다.
...
<a id="00b427fc8ab091da"></a>
#### 대화형 명령어의 수행 결과

SQL 구문과 함께 사용하는 대화형 명령어들은 SQL 구문을 수행한 것과 동일한 결과와 에러를 출력한다.

다음은 [`\prepare sql`](#4da67701b4fdc1fc) 명령을 이용해 SQL 구문을 준비하고 [`\exec`](#7fbfc7289468d89b) 명령을 이용해 준비된 SQL 구문을 수행하는 예이다.
...
다음은 [`\prepare sql`](#4da67701b4fdc1fc) 명령을 사용할 때 SQL 구문에 에러가 존재하는 경우로써 SQL 구문을 수행한 것과 동일한 에러를 출력한다.
...
대화형 명령어의 수행 결과는 [Interactive Command References](#f127dbb1989b80d3)의 각 명령어 설명과 사용 예를 참조한다.

<a id="b4105d9103abf05e"></a>
#### SQL 구문의 자동 COMMIT

gsql의 대화형 모드에서 수행하는 SQL 구문은 [COMMIT](../part-03-sql-manual/16-sql-references.md#75b82fec67e8ca7c) 또는 [ROLLBACK](../part-03-sql-manual/16-sql-references.md#1476005da98d89d8) 구문을 통해 트랜잭션을 완료하거나 철회할 수 있다.
...
SQL 구문을 수행할 때마다 자동으로 트랜잭션을 COMMIT 하려면 다음과 같이 [`\set autocommit`](#bbcc1db8d869ee1c) 명령을 이용하여 제어할 수 있다. 위의 예와 동일한 SQL 구문을 수행한 경우, 각 SQL 구문이 자동으로 COMMIT 되어 두 번째, 세 번째 INSERT 구문의 트랜잭션도 ROLLBACK과 관계없이 완료되었음을 알 수 있다.
...
자동 COMMIT 제어에 대한 자세한 내용은 [`\set autocommit`](#bbcc1db8d869ee1c) 명령어를 참조한다.

<a id="99f26d703050ee77"></a>
#### 수행 중인 SQL 구문의 강제종료
...
<a id="6d823efdedc22008"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\set color`](#0be106bdb0518b8e) | 각 row를 다른 색깔로 구별한다. |
| [`\set colsize`](#7fe81892940edbed) | Column 결과의 최대 크기를 제어한다. |
| [`\set error`](#0d801fe08ac45f8e) | 에러 메시지 출력 여부를 제어한다. |
| [`\set linesize`](#8ad8e5ae9481dd53) | Row의 최대 출력 길이를 제어한다. |
| [`\set numsize`](#2030abff10987b43) | 숫자값의 최대 digit 개수를 제어한다. |
| [`\set pagesize`](#f3d95aa99a1f9d27) | Page에 포함할 row의 개수를 제어한다. |
| [`\set timing`](#1fc85fd41709471a) | SQL 구문 수행 시간을 출력한다. |
| [`\set vertical`](#962c28f61f26211b) | 각 column을 한 line에 출력한다. |

<a id="75be1578de5c1409"></a>
#### Page 구성 제어

앞서 [SQL 수행 결과](#ea6ac3bc756a29ac)에 기술한대로 질의 결과의 상단부에는 column 이름을 출력하고 데이터를 출력한 후, 하단부에 요약 결과를 출력한다.

질의 결과가 많을 경우 pagesize (기본값 20) 단위로 나누어 각 page마다 column 이름과 데이터를 출력하고 모든 page의 출력이 완료된 후 하단부에 요약 결과를 출력한다.

다음은 pagesize를 5로 변경하여 질의 결과를 출력한 예이다. Pagesize 제어에 대한 자세한 내용은 [`\set pagesize`](#f3d95aa99a1f9d27)명령을 참조한다.
...
다음은 linesize를 200으로 조정하기 전과 후의 질의 결과이다. Linesize 제어에 대한 자세한 내용은 [`\set linesize`](#8ad8e5ae9481dd53) 명령을 참조한다.
...
터미널 상에서 각 row를 구별하여 가독성을 높이고 싶은 경우 [`\set color`](#0be106bdb0518b8e) 명령을 통해 다음과 같이 row마다 색깔을 달리하여 출력할 수 있다. 자세한 내용은 [`\set color`](#0be106bdb0518b8e) 명령을 참조한다.
...
질의 결과를 출력할 때 row를 line 단위가 아닌 column 단위로 하나의 line에 출력하려면 `\set vertical on` 명령을 사용한다. 다음 예와 같이 column 단위로 출력 결과가 생성되는데 자세한 내용은 [`\set vertical`](#962c28f61f26211b) 명령을 참조한다.
...
<a id="9367eccdc8240e9c"></a>
#### 데이터 값의 출력 제어

gsql은 질의 결과에 포함되는 모든 데이터를 문자열로 변환하여 출력한다.

숫자형은 numsize (기본값 20)를 digit 단위로 출력하며 numsize 보다 큰 숫자값은 exponent 형태로 표현한다. 다음은 numsize를 이용해 출력값을 제어하는 예이며 자세한 내용은 [`\set numsize`](#2030abff10987b43) 명령을 참조한다.
...
숫자값 출력에 대한 다양한 제어가 필요한 경우, 다음과 같이 [TO_CHAR( number )](../part-03-sql-manual/11-sql-elements.md#d0f1dcb76f6a4884) 함수를 이용해 출력을 제어할 수 있다.
...
- [NLS_DATE_FORMAT](../part-02-administration-manual/10-server-property.md#39cb46c447575199)
- [NLS_TIMESTAMP_FORMAT](../part-02-administration-manual/10-server-property.md#db86968fe3d06d6e)
- [NLS_TIMESTAMP_WITH_TIME_ZONE_FORMAT](../part-02-administration-manual/10-server-property.md#3356e96838139b8a)
- [NLS_TIME_FORMAT](../part-02-administration-manual/10-server-property.md#2524063f770de1b8)
- [NLS_TIME_WITH_TIME_ZONE_FORMAT](../part-02-administration-manual/10-server-property.md#838ff7f06a1cc0ac)

해당 프로퍼티는 TO_CHAR(), TO_DATE() 등의 함수에서 format 정보를 입력하지 않았을 때 사용되는 프로퍼티이다. ODBC나 JDBC를 이용한 응용 프로그램이 SQL 구문에서 TO_CHAR(), TO_DATE() 등의 함수를 사용할 때만 위의 프로퍼티를 사용하는 반면에, gsql 은 날짜/ 시간 데이터 값을 출력하기 위해 최초로 접속할 때 해당 프로퍼티 정보를 획득하여 이를 이용한다. 대화형 모드에서 다음과 같은 SQL 구문으로 프로퍼티를 변경할 경우, gsql은 갱신된 프로퍼티 정보를 사용한다.
...
프로퍼티를 변경하지 않고 날짜/ 시간 출력값을 제어하려면 다음과 같이 [TO_CHAR( datetime )](../part-03-sql-manual/11-sql-elements.md#1aa9ef481e07ad64) 함수를 사용한다.
...
LONG VARCHAR, LONG VARBINARY 타입의 데이터는 아주 긴 문자열을 포함할 수 있어 colsize (기본값 8192) 만큼만 데이터를 출력한다. 다음은 colsize를 줄여 LONG VARCHAR 타입인 TEXT column의 일부 데이터를 출력한 예인데 colsize 값을 늘려 데이터를 모두 출력할 수 있다. 자세한 내용은 [`\set colsize`](#7fe81892940edbed)  명령을 참조한다.
...
<a id="83e71a73438f947b"></a>
#### SQL 구문 수행시간

SQL 구문을 수행할 때 수행 시간을 출력하려면 다음과 같이 `\set timing on` 명령을 사용한다. 자세한 내용은 [`\set timing`](#1fc85fd41709471a) 명령을 참조한다.
...
<a id="ebe05c282f3ea9cc"></a>
| gsql 명령 | 설명 |
| --- | --- |
| `\spool` | 출력 결과를 file에 저장한다. |

출력 결과 저장과 관련된 기능들은 [`\spool`](#662ffdd4290509bd)의 명령어들을 참조한다.
...
<a id="9332bdf64820ee6f"></a>
### SQL 객체 정보 조회

SQL 객체에 대한 정보는 [DICTIONARY_SCHEMA](../part-02-administration-manual/9-database-information.md#ef9643afcec18f44), [INFORMATION_SCHEMA](../part-02-administration-manual/9-database-information.md#ac6101a002bac098)의 각종 view들로 조회할 수 있다.

**참조 명령어**

<a id="86f6f9bc4641e8ae"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\desc`](#d521a9250087bc96) | 테이블 정보를 조회한다. |
| [`\idesc`](#a50caa0d971eac46) | 인덱스 정보를 조회한다. |
...
gsql은 다음과 같이 `\desc` 명령어를 이용하여 테이블과 관련된 정보를 쉽게 조회할 수 있다. 출력 결과 및 자세한 내용은 [`\desc`](#d521a9250087bc96) 명령을 참조한다.
...
명령어 `\idesc`는 다음 예와 같이 인덱스와 관련된 정보를 보여준다. 자세한 내용은 [`\idesc`](#a50caa0d971eac46)명령을 참조한다.
...
<a id="231b6f7f03c11bed"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\ddl_cluster`](#b20d97256f74061f) | 클러스터와 관련된 DDL을 출력한다. |
| [`\ddl_db`](#fcc52af08e8702f3) | 데이터베이스와 관련된 DDL을 출력한다. |
| [`\ddl_tablespace`](#0741a20c04d99f6b) | 테이블스페이스와 관련된 DDL을 출력한다. |
| [`\ddl_profile`](#a919e7df94be7534) | Profile과 관련된 DDL을 출력한다. |
| [`\ddl_audit_policy`](#921a93045ab9b362) | Audit policy와 관련된 DDL을 출력한다. |
| [`\ddl_auth`](#a28e24c873fc997c) | 계정과 관련된 DDL을 출력한다. |
| [`\ddl_schema`](#00572ce8bc1897b2) | 스키마와 관련된 DDL을 출력한다. |
| [`\ddl_publicsynonym`](#8ca35dd8ce460639) | Public synonym과 관련된 DDL을 출력한다. |
| [`\ddl_table`](#fea9e68b2130b346) | 테이블과 관련된 DDL을 출력한다. |
| [`\ddl_constraint`](#a7e58f81dbdcea98) | 제약 조건과 관련된 DDL을 출력한다. |
| [`\ddl_index`](#450a77eed5909177) | 인덱스와 관련된 DDL을 출력한다. |
| [`\ddl_view`](#623927873a26cc22) | View와 관련된 DDL을 출력한다. |
| [`\ddl_sequence`](#9cb17a405dbfdee1) | 시퀀스와 관련된 DDL을 출력한다. |
| [`\ddl_synonym`](#3a1393312f7bf07d) | Synonym과 관련된 DDL을 출력한다. |
| [`\ddl_procedure`](#55849ca03c97d972) | Procedure와 관련된 DDL을 출력한다. |
| [`\set ddlsize`](#2b6c88dca7920a01) | DDL 출력 버퍼의 크기를 제어한다. |
...
테이블에 생성된 제약 조건에 해당하는 DDL은 다음과 같이 `\ddl_table`의 CONSTRAINT 옵션을 사용하여 출력한다.
...
<a id="4fd5c1daf7ee88fd"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\\`](#02c0db7c0016eb98) | 이전 SQL을 수행한다. |
| [`\{n}`](#d0bf84e20995b49f) | SQL 이력 번호에 해당하는 구문을 실행한다. |
| [`\history`](#e66b2153e347a371) | SQL 수행 이력을 조회한다. |
| [`\set history`](#e80c3f97a399cc1c) | History 버퍼 개수를 제어한다. |
...
gsql의 history 관련 기능들은 [참조 명령어 ](#4fd5c1daf7ee88fd)의 명령어들을 참조한다.

<a id="3eabd0934259b890"></a>
### SQL 편집
...
<a id="91b455e58a734af4"></a>
| gsql 명령 | 설명 |
| --- | --- |
| `\edit` | Text editor를 사용하여 SQL 구문을 편집한다. |

편집 기능을 사용하면 gsql은 편집기를 실행하여 제어권을 넘긴다. 사용자는 편집기에서 SQL 구문 내용을 자유롭게 편집할 수 있으며 편집기를 종료하면 gsql에서 제어권을 돌려받은 후 해당 내용을 마지막 이력으로 추가한다.
...
gsql 편집 관련 기능에 대한 자세한 내용은 [`\edit`](#c81a9833a6ff5fd8)의 명령어들을 참조한다.
...
<a id="dc819189939aeefb"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\connect`](#e4c08e9ad8042e16) | 새로운 사용자로 접속한다. |
| [`\quit`](#a3ad7dc41be165ea) | 접속을 종료한다. |

대화형 모드에서 `\connect` 명령을 이용해 다음과 같이 새로운 사용자로 접속할 수 있다. `\connect ` 명령은 기존의 session을 종료하고 새로운 session을 생성한다. 자세한 내용은 [`\connect`](#e4c08e9ad8042e16) 명령을 참조한다.
...
다음과 같이 [SET SESSION AUTHORIZATION user_identifier](../part-03-sql-manual/16-sql-references.md#fc35dac87707c70d) 구문을 이용해 사용자를 변경할 수 있다. `\connect` 명령이 수행 중인 트랜잭션을 모두 COMMIT 하고 새로운 session을 생성하는 반면에 SET SESSION AUTHORIZATION 구문은 기존의 session을 그대로 유지한 채 사용자를 변경한다.
...
대화형 모드를 종료하려면 다음과 같이 `\quit` 명령을 사용한다. 자세한 내용은 [`\quit`](#a3ad7dc41be165ea) 명령을 참조한다.
...
<a id="9c479c9cab74a7a8"></a>
| gsql 명령 | 설명 |
| --- | --- |
|  [`\var`](#cb2c1c300622c188) | 호스트 변수를 선언한다. |
|  [`\exec :var := value`](#754c255a8858ed59) | 호스트 변수에 값을 할당한다. |
|  [`\print`](#4a8c91a8ae58a8ce) | 호스트 변수의 값을 출력한다. |
|  [`\dynamic sql :var`](#151e70d7243310db) | 호스트 변수에 저장된 SQL 문장을 수행한다. |

다음은 [`\var`](#cb2c1c300622c188) 명령을 이용하여 호스트 변수를 선언하고 [`\exec :var := value`](#754c255a8858ed59) 명령을 이용해 호스트 변수에 값을 대입하고 [`\print`](#4a8c91a8ae58a8ce) 명령을 이용해 호스트 변수의 값을 조회하는 예이다. 자세한 내용은 각 명령을 참조한다.
...
<a id="8c4b75fbda77a875"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\exec sql`](#35765f534b203586) | SQL 문장을 바로 실행한다. |
| [`\prepare sql`](#4da67701b4fdc1fc) | SQL 문장을 준비한다. |
| [`\exec`](#7fbfc7289468d89b) | 준비된 SQL 문장을 실행한다. |
| [`\dynamic sql :var`](#151e70d7243310db) | 호스트 변수에 저장된 SQL 문장을 실행한다. |
...
다음과 같이 [`\prepare sql`](#4da67701b4fdc1fc) 명령과 [`\exec`](#7fbfc7289468d89b) 명령을 이용하여 위에서 Java 코드의 PreparedStatement class를 이용하여 구현한 것을 simulation 할 수 있다.
...
> 
> 
> - [SELECT .. INTO](../part-03-sql-manual/16-sql-references.md#f1d3efe9cfddaa45)
> - [DECLARE cursor_name](../part-03-sql-manual/16-sql-references.md#c0f5909b51d661a3)
> - [OPEN cursor_name](../part-03-sql-manual/16-sql-references.md#275ead84ebffb434)
> - [FETCH cursor_name](../part-03-sql-manual/16-sql-references.md#f8b0914239210b68)
> - [CLOSE cursor_name](../part-03-sql-manual/16-sql-references.md#c718855fa5a651b0)
> 

<a id="313a623c1bfcd4be"></a>
### SQL 실행 계획 정보
...
<a id="7d9a3a0ea2b92eef"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\explain plan`](#88508324a1818005) | SQL 문장의 실행 계획을 출력한다. |
| [`\set autotrace`](#2372df662eef28fe) | 실행 계획 출력 여부를 설정한다. |

대화형 모드에서 다음과 같이 `\explain plan` 명령이나 `\set autotrace` 명령을 이용하여 SQL 구문의 실행 계획을 조회할 수 있다. 명령에 대한 자세한 내용은 [`\explain plan`](#88508324a1818005), [`\set autotrace`](#2372df662eef28fe) 를 참조하고 실행 계획에 대한 해석은 [SQL 실행 계획](../part-03-sql-manual/15-sql-tuning.md#343c5ce46c97bbbd)을 참조한다.
...
<a id="43473a7ea7a63469"></a>
### --as {SYSDBA|ADMIN}

<a id="f1fd8a50753e6987"></a>
#### 설명

SYSDBA role이나 ADMIN role로 GOLDILOCKS에 접속한다.  
해당 role에 대한 자세한 내용은 [서버 구동 및 종료](#27035efe8472921f)를 참조한다.

```
% gsql sys gliese --as sysdba
```

<a id="90fae429fdf55eed"></a>
#### 사용 예
...
<a id="f5a726055c723b39"></a>
### --conn-string

<a id="e024404218b695ab"></a>
#### 설명

Connection string을 통해 GOLDILOCKS에 접속한다.  
기술할 connection string의 내용은 ODBC 함수 [SQLDriverConnect](../part-05-developer-manual/25-odbc.md#8a2ef691d8c8517a)의 input string과 동일한 형태로 사용해야 한다. Connection string에 대한 자세한 내용은 [SQLDriverConnect](../part-05-developer-manual/25-odbc.md#8a2ef691d8c8517a) 함수를 참조한다.

<a id="350d35e53c052410"></a>
#### 사용 예
...
<a id="a42abf1325863d59"></a>
### --dsn

<a id="8c410c33d4c76977"></a>
#### 설명

odbc.ini 파일에 정의된 Data Source Name (DSN)을 이용하여 접속한다.  
DSN 이름은 odbc.ini 파일에 정의된 이름이어야 한다. odbc.ini 파일에 대한 자세한 내용은 [UNIX에서 DSN 구성](../part-05-developer-manual/25-odbc.md#2d6d293a128b42a1)을 참조한다.  
DSN이 생략된 경우 기본값은 GOLDILOCKS이다.

<a id="de3ded53551d846c"></a>
#### 사용 예
...
<a id="68dca1e8faaea67a"></a>
### --enable-color

<a id="d217aa238b586fb6"></a>
#### 설명

터미널 상에서 질의 결과의 각 row를 다른 색깔로 출력하여 쉽게 구분할 수 있도록 한다.  
대화형 모드로 질의를 수행할 경우 질의 결과를 row마다 다른 색깔로 출력하며 이는 [--import](#2cbe6ffb8b3af2ea) 옵션을 이용해 파일에 포함된 SELECT 구문의 결과에도 적용한다.

<a id="251e682aa688da8f"></a>
#### 사용 예
...
<a id="d696467b0e8fc963"></a>
### --help

<a id="343c2e199808f75f"></a>
#### 설명

gsql 프로그램의 옵션 목록을 간략히 보여준다.

<a id="b62c94aa67b8ef9b"></a>
#### 사용 예
...
<a id="2cd178091ca43fde"></a>
### --no-prompt

<a id="e6382f6fe07eaba8"></a>
#### 설명

대화형 모드를 수행할 때 프롬프트를 출력하지 않는다.

<a id="193f8b9c113eedde"></a>
#### 사용 예
...
<a id="fbc8ad011840c281"></a>
### --prompt

<a id="1150e5f9f0abc517"></a>
#### 설명
...
<a id="b1ae2abadd9555d8"></a>
### --silent

<a id="069df2b8f64a5c6f"></a>
#### 설명

SQL 구문의 수행 결과를 출력하지 않는다.  
[--import](#2cbe6ffb8b3af2ea) 명령을 이용해 파일을 읽어 대량으로 SQL 구문을 수행할 경우에 유용하다.  
--silent 옵션은 대화형 모드로 동작할 때도 적용된다.

<a id="99ec56add0b0ad52"></a>
#### 사용 예
...
<a id="6d387efaf97c8d97"></a>
### --version

<a id="b9632fe320668e1f"></a>
#### 설명

gsql 프로그램의 version 정보를 출력한다.  
GOLDILOCKS와 동일한 version의 gsql 프로그램을 사용할 것을 권장한다. GOLDILOCKS의 version은 다음과 같이 SQL 함수 [VERSION](../part-03-sql-manual/11-sql-elements.md#de2cfe412d1f521d)을 통해 확인할 수 있다.
...
<a id="1f1d922a97e257de"></a>
#### 설명

가장 최근에 성공한 SQL 구문을 실행한다.

<a id="c32b9c51cad21999"></a>
#### 사용 예
...
<a id="e4c08e9ad8042e16"></a>
### `\connect`

<a id="9af4ea1f8d87ba23"></a>
#### 구문
...
<a id="9a490a59f67fce36"></a>
#### 설명

입력된 username과 password로 새로 접속한다.  
종료되지 않은 트랜잭션이 존재하는 경우 COMMIT을 수행한다.

<a id="d4f60308776c5a11"></a>
#### 사용 예

다음은 test 계정으로 접속하는 예이다.
...
<a id="a26626f2caa96c6d"></a>
### `\cstartup`

<a id="fb04649ea4f0842a"></a>
#### 구문
...
<a id="c577ce8f308cfb95"></a>
#### 설명

Cluster 환경에서 GOLDILOCKS 서버를 구동한다.

`\cstartup` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#27035efe8472921f)를 참조한다.
...
`\cstartup local open` 명령어와 `\cstartup global open` 명령어는 해당 서버와 다른 서버들도 해당 단계까지 구동시킨다. 이 외의 명령어는 해당 서버에만 적용되므로 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8)를 이용하여 단계별로 구동하려면 직접 서버를 LOCAL OPEN 단계까지 구동해야 한다.

서버의 구동 단계는 NOMOUNT, MOUNT, LOCAL OPEN, GLOBAL OPEN 단계로 구분되며 자세한 내용은 [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#d48808b833e799ba)을 참조한다.

`\cstartup` 명령을 수행한 후에 다음 단계로 넘어가기 위해서는 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8) 구문을 수행해야 한다.

glocator를 이용한 `\cstartup`은 [CSTARTUP과 CSHUTDOWN](40-glocator.md#729941785f7cedfc)을 통해 확인할 수 있다.


> 
> - CS 환경에서만 사용하는 명령어로써 gsqlnet에서만 사용할 수 있다.
> - `\cstartup nomount/mount` 이외의 명령은 해당 서버와 함께 다른 서버에까지 영향을 미치는 명령어이다. 따라서 [odbc.ini 파일](../part-05-developer-manual/25-odbc.md#78b0fc955ee2c6e5)에 LOCATOR_DSN 속성이 기술되지 않았을 경우, 에러가 반환된다.
> 

<a id="7be6c9437e199330"></a>
#### 사용 예
...
<a id="104a75b925ad3bbe"></a>
### `\cshutdown`

<a id="577e22a60e81005b"></a>
#### 구문
...
<a id="a61c2b9c82484480"></a>
#### 설명

Cluster 환경에서 GOLDILOCKS 서버를 종료한다.

`\cshutdown` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#27035efe8472921f)를 참조한다.
...
glocator를 이용한 `\cshutdown`은 [CSTARTUP과 CSHUTDOWN](40-glocator.md#729941785f7cedfc)을 통해 확인할 수 있다.

> • CS 환경에서만 사용하는 명령어로써 gsqlnet에서만 사용할 수 있다.  
> • `\cshutdown` 명령은 해당 서버와 함께 다른 서버에까지 영향을 미치는 명령어이다. 따라서 [odbc.ini 파일](../part-05-developer-manual/25-odbc.md#78b0fc955ee2c6e5)에 LOCATOR_DSN 속성이 기술되지 않았을 경우, 에러가 반환된다.

<a id="1d986ad96b4a4475"></a>
#### 사용 예
...
<a id="b20d97256f74061f"></a>
### `\ddl_cluster`

<a id="a928600538ea759e"></a>
#### 구문
...
<a id="fcc52af08e8702f3"></a>
### `\ddl_db`

<a id="7071f96e93cab8bd"></a>
#### 구문
...
위의 dictionary 정보를 포함하는 schema에 대한 DDL 구문은 [`\ddl_schema`](#00572ce8bc1897b2) 명령을 이용해 출력할 수 있다.

<a id="b2590df16276bbc6"></a>
#### 사용 예
...
<a id="0741a20c04d99f6b"></a>
### `\ddl_tablespace`

<a id="840123ec035a407a"></a>
#### 구문
...
<a id="a919e7df94be7534"></a>
### `\ddl_profile`

<a id="eafb58c8c3b3c3f6"></a>
#### 구문
...
<a id="921a93045ab9b362"></a>
### `\ddl_audit_policy`

<a id="547b4cf9090b6956"></a>
#### 구문
...
<a id="a28e24c873fc997c"></a>
### `\ddl_auth`

<a id="0ff9f1fe3a0a23d8"></a>
#### 구문
...
<a id="00572ce8bc1897b2"></a>
### `\ddl_schema`

<a id="65aafbf3e80487f9"></a>
#### 구문
...
<a id="8ca35dd8ce460639"></a>
### `\ddl_publicsynonym`

<a id="fe6b28dfb8802352"></a>
#### 구문
...
<a id="fea9e68b2130b346"></a>
### `\ddl_table`

<a id="cf07df1beed38ac9"></a>
#### 구문
...
<a id="a7e58f81dbdcea98"></a>
### `\ddl_constraint`

<a id="02acfe3ec966de74"></a>
#### 구문
...
<a id="450a77eed5909177"></a>
### `\ddl_index`

<a id="21b6ceb973af5dc7"></a>
#### 구문
...
<a id="623927873a26cc22"></a>
### `\ddl_view`

<a id="be24c99db1cdffa6"></a>
#### 구문
...
<a id="9cb17a405dbfdee1"></a>
### `\ddl_sequence`

<a id="90d1df668e024b29"></a>
#### 구문
...
<a id="3a1393312f7bf07d"></a>
### `\ddl_synonym`

<a id="7fb2c1a34fa871c8"></a>
#### 구문
...
<a id="55849ca03c97d972"></a>
### `\ddl_procedure`

<a id="f667eeaf000e8b1b"></a>
#### 구문
...
<a id="d521a9250087bc96"></a>
### `\desc`

<a id="85ead0722b83f8b4"></a>
#### 구문
...
<a id="bc4e9ffcc2215e91"></a>
#### 설명

테이블의 구조 정보를 조회한다.

테이블은 다음과 같이 테이블 이름만 기술하거나 테이블 이름을 스키마 이름과 함께 기술할 수 있다. 스키마 이름을 명시하지 않은 경우 테이블의 스키마 이름은 사용자의 [Schema Path](../part-03-sql-manual/13-sql-objects.md#37870b8d44e548dc)에 의해 결정된다.

- `\desc` t1
- `\desc` public.t1

테이블을 생성할 때 다음과 같이 [delimited identifier](../part-03-sql-manual/11-sql-elements.md#f589d599cc92f25f)를 이용해 테이블 이름을 생성한 경우, double quote (")를 사용해 기술한다.
...
<a id="151e70d7243310db"></a>
### `\dynamic sql :var`

<a id="b9f4f65ca67bf940"></a>
#### 구문
...
<a id="b40bda354d9ae99e"></a>
#### 설명

호스트 변수 var에 저장된 SQL 문장을 수행한다.  
Embedded SQL의 정의되지 않은 SQL 문장을 수행하는 [Embedded Dynamic SQL](../part-05-developer-manual/27-embedded-sql.md#e652d9727ea939cd)과 유사한 개념으로써 정의되지 않은 SQL 문장을 수행할 때 사용한다.

다음과 같은 절차로 수행한다.

1. 호스트 변수를 선언한다. ([`\var`](#cb2c1c300622c188)를 참조한다.)  
   `\var` var_stmt VARCHAR(1024)
2. 호스트 변수의 값으로 SQL 문장을 대입한다. ([`\exec :var := value`](#754c255a8858ed59)를 참조한다.)  
   `\exec` :var_stmt := 'SELECT # FROM t1'
3. Dynamic SQL을 수행한다.  
   `\dynamic sql` :var_stmt

<a id="f7be8d770e0bff13"></a>
#### 사용 예
...
다음 INSERT 문장과 같이 SQL 문장에 문자열이 존재할 경우 명령어 [`\exec :var := value`](#754c255a8858ed59)를 사용할 때와 같이 single quote (')를 두 번 기술한다.
...
<a id="c81a9833a6ff5fd8"></a>
### `\edit`

<a id="282361377b7ec50b"></a>
#### 설명
...
<a id="08e8467bb2f04773"></a>
#### `\edit`

<a id="bacc9f2a73dca3f5"></a>
##### 구문
...
<a id="26865aec360581d5"></a>
#### `\edit 'file_name'`

<a id="3a5d9d6c24c55503"></a>
##### 구문
...
<a id="32ceb0af987e3a02"></a>
#### `\edit [history] {n}`

<a id="f2314f93f4c60a5c"></a>
##### 구문
...
<a id="bf379cf50c44253e"></a>
##### 설명

[`\history`](#e66b2153e347a371) 명령으로 조회할 수 있는 SQL 수행 이력 중에서 number에 해당하는 구문을 편집한다.  
number 값은 [`\history`](#e66b2153e347a371) 수행 결과의 ID 값이어야 한다.

<a id="603ad200ed0afdff"></a>
##### 사용 예

gsql의 이력에 저장된 SQL 구문을 편집하는 예이다.
...
<a id="7fbfc7289468d89b"></a>
### `\exec`

<a id="fe36ccb9ad6a0413"></a>
#### 구문
...
<a id="d9a49271e773c1ec"></a>
#### 설명

[`\prepare sql`](#4da67701b4fdc1fc) 명령에 의해 준비된 SQL 구문을 수행한다.  
`\`exec 명령은 ODBC의 [SQLExecute](../part-05-developer-manual/25-odbc.md#97979ad6741046e2) 함수와 JDBC의 PreparedStatement::[execute](../part-05-developer-manual/26-jdbc.md#2fa1e238e48cbe63) 함수와 유사하게 동작한다.  
준비된 SQL 구문은 `\exec` 명령을 통해 반복적으로 수행할 수 있다.

<a id="430b26bb184574d5"></a>
#### 사용 예
...
<a id="754c255a8858ed59"></a>
### `\exec :var := value`

<a id="67b689c78cd98fe7"></a>
#### 구문
...
<a id="15ccc3873d9f4307"></a>
#### 설명

호스트 변수에 값을 대입한다.  
해당 명령은 다음과 같은 [SELECT .. INTO](../part-03-sql-manual/16-sql-references.md#f1d3efe9cfddaa45) 구문을 수행한 것과 동일한 방식으로 호스트 변수에 값을 대입한다.
...
호스트 변수는 [`\var`](#cb2c1c300622c188) 명령을 통해 선언되어 있어야 한다.

호스트 변수는 colon (:)과 함께 사용하여야 하며, 대입 연산자 (:=)에서 (:)을 생략하면 안된다. 호스트 변수에 입력되는 &lt;value expression&gt;에는 단순한 값이나 연산을 사용할 수 있다. 자세한 내용은 [Expressions](../part-03-sql-manual/11-sql-elements.md#5fa008eecf52b881)를 참조한다.

호스트 변수에 입력되는 값은 호스트 변수의 데이터 타입에 입력 가능한 값이어야 한다. 자세한 내용은 [타입간 변환](../part-03-sql-manual/11-sql-elements.md#e2c21b3a3eaedfd1)을 참조한다.

입력된 호스트 변수의 값은 [`\print`](#4a8c91a8ae58a8ce) 명령을 이용해 조회할 수 있다.
...
다음은 [SELECT .. INTO](../part-03-sql-manual/16-sql-references.md#f1d3efe9cfddaa45) 구문을 통해 호스트 변수에 값을 얻어오는 예이다.
...
<a id="35765f534b203586"></a>
### `\exec sql`

<a id="67cb695c11cc57b0"></a>
#### 구문
...
SQL 구문을 수행할 때 [`\prepare sql`](#4da67701b4fdc1fc) 명령과 [`\exec`](#7fbfc7289468d89b) 명령을 이용하여 PREPARE/EXECUTE 방식으로 실행하는 것과 달리, `\exec sql` 명령은 ODBC의 [SQLExecDirect](../part-05-developer-manual/25-odbc.md#31acdaa3d1daea59) 함수와 JDBC 의 Statement::[execute](../part-05-developer-manual/26-jdbc.md#6c05a6599abb6e25) 함수에 대응하는 DIRECT EXECUTE 방식으로 SQL 구문을 수행한다.

<a id="15f9a7d63f87a840"></a>
#### 사용 예
...
<a id="88508324a1818005"></a>
### `\explain plan`

<a id="d8389be36ec5f27e"></a>
#### 구문
...
- `\explain plan`
    - on이나 only를 명시하지 않은 경우, 기본값은 on이다.

실행 계획에 대한 자세한 내용은 [SQL 실행 계획](../part-03-sql-manual/15-sql-tuning.md#343c5ce46c97bbbd)을 참조한다.

<a id="7626a16daca88aa7"></a>
#### 사용 예
...
<a id="1d4da2ee1a10931a"></a>
### `\help`

<a id="16e6b3e9c97ca406"></a>
#### 구문
...
<a id="0fec7bf5c3fdc51b"></a>
#### 설명

gsql 대화형 모드에서 `(\)`로 시작하는 명령어 목록을 간략히 보여준다.

<a id="56fecc696aeb1996"></a>
#### 사용 예
...
<a id="7d22e82a14322187"></a>
### `\host`

<a id="ee8b1b8960009c6f"></a>
#### 구문
...
<a id="e66b2153e347a371"></a>
### `\history`

<a id="35cf0aacc7461eaf"></a>
#### 구문
...
<a id="47d30e9c99d70fa1"></a>
#### 설명

gsql 프로그램을 수행한 이후의 SQL 구문 수행 목록을 보여준다.  
수행에 성공한 SQL 구문만 관리하며, gsql (`\`)로 시작하는 대화형 명령어로 수행한 명령어나 실패한 SQL 구문은 포함하지 않는다.  
해당 목록을 참조하여 [`\\`](#02c0db7c0016eb98) 명령이나 [`\{n}`](#d0bf84e20995b49f) 명령을 통해 이전에 수행한 SQL 구문을 다시 수행할 수 있다.  
관리할 수 있는 SQL 구문의 개수는 [`\set history`](#e80c3f97a399cc1c) 명령을 통해 제어한다.

<a id="b2a1fefee3313e34"></a>
#### 사용 예
...
<a id="53c60644a1c9756d"></a>
### `\import`

<a id="e6d4cd47e8701d34"></a>
#### 구문
...
<a id="a50caa0d971eac46"></a>
### `\idesc`

<a id="a4c6de103ba73921"></a>
#### 구문
...
<a id="c0e3538440346621"></a>
#### 설명

인덱스의 구조 정보를 조회한다.

인덱스 이름은 다음과 같이 인덱스 이름만 기술하거나 스키마 이름과 함께 기술할 수 있다. 스키마 이름을 명시하지 않은 경우, 인덱스의 스키마 이름은 사용자의 [Schema Path](../part-03-sql-manual/13-sql-objects.md#37870b8d44e548dc)에 의해 결정된다.
...
<a id="d0bf84e20995b49f"></a>
### `\{n}`

<a id="252a7a3b59a4470a"></a>
#### 구문
...
<a id="c336ea0a58730cf6"></a>
#### 설명

[`\history`](#e66b2153e347a371)명령으로 조회할 수 있는 SQL 수행 이력 중에 number에 해당하는 구문을 수행한다.  
number 값은 [`\history`](#e66b2153e347a371) 수행 결과의 ID 값이어야 한다.

<a id="01e6f8fab3c0317e"></a>
#### 사용 예
...
<a id="4da67701b4fdc1fc"></a>
### `\prepare sql`

<a id="a7f42c28080620b2"></a>
#### 구문
...
<a id="04c015a6827751e3"></a>
#### 설명

&lt;sql_statement&gt;에 기술된 SQL 구문을 준비한다. 준비된 SQL 구문은 [`\exec`](#7fbfc7289468d89b) 명령을 통해 반복적으로 수행할 수 있다.

`\prepare sql` 명령은 ODBC의 [SQLPrepare](../part-05-developer-manual/25-odbc.md#c6c32b154bed3d8b) 함수와 JDBC의 Connection::[prepareStatement](../part-05-developer-manual/26-jdbc.md#ad6afcd82012affb) 함수와 유사하게 동작한다.

<a id="6e814de262aeab9d"></a>
#### 사용 예
...
<a id="4a8c91a8ae58a8ce"></a>
### `\print`

<a id="cbe342a40d5f66d1"></a>
#### 구문
...
<a id="1717f92efcaf94c6"></a>
#### 설명

다음과 같이 [`\var`](#cb2c1c300622c188) 명령으로 선언한 호스트 변수 값을 조회한다. 호스트 변수 값이 설정되지 않은 경우 NULL 값을 가진다.
...
<a id="a3ad7dc41be165ea"></a>
### `\quit`

<a id="15e0d3f57eaf063d"></a>
#### 구문
...
<a id="17d803359a4651e6"></a>
#### 설명

gsql을 종료한다.  
종료할 때 COMMIT 되지 않은 트랜잭션을 모두 COMMIT 한다.

<a id="7f901368564deb2c"></a>
#### 사용 예
...
<a id="bbcc1db8d869ee1c"></a>
### `\set autocommit`

<a id="d314518c6a7e552e"></a>
#### 구문
...
<a id="2372df662eef28fe"></a>
### `\set autotrace`

<a id="f6e56d9d6cc4333a"></a>
#### 구문
...
실행 계획에 대한 자세한 내용은 [SQL 실행 계획](../part-03-sql-manual/15-sql-tuning.md#343c5ce46c97bbbd)를 참조한다.

<a id="89f77e579472bd21"></a>
#### 사용 예
...
<a id="0be106bdb0518b8e"></a>
### `\set color`

<a id="0e37337d1c29853c"></a>
#### 구문
...
<a id="7fe81892940edbed"></a>
### `\set colsize`

<a id="e1fb8459e1736eae"></a>
#### 구문
...
LONG VARCHAR column은 문자열 길이가 길기 때문에 다음과 같은 질의 수행 결과의 가독성이 떨어지게 된다. 이 때, colsize를 줄여 LONG VARCHAR column의 가독성을 높이거나, colsize를 크게 하여 원하는 길이만큼 출력할 수 있다.
...
<a id="2b6c88dca7920a01"></a>
### `\set ddlsize`

<a id="64babf7f0da08df7"></a>
#### 구문
...
<a id="0d801fe08ac45f8e"></a>
### `\set error`

<a id="e672d7a995e918a1"></a>
#### 구문
...
<a id="227e43038968397d"></a>
### `\set heading`

<a id="fa7927a520e48945"></a>
#### 구문
...
<a id="bac8d50bc1fd08e7"></a>
#### 설명

질의 결과에 헤더를 출력할지 여부를 설정한다.

<a id="b7adb34024206a4e"></a>
#### 사용 예
...
<a id="e80c3f97a399cc1c"></a>
### `\set history`

<a id="d0d08448d429ca58"></a>
#### 구문
...
history 정보는 [`\\`](#02c0db7c0016eb98) 명령이나 [`\{n}`](#d0bf84e20995b49f) 명령을 통해 이전에 수행한 SQL 구문을 다시 수행하려 할 때 사용한다.

<a id="afc96e89aa534ac4"></a>
#### 사용 예
...
<a id="8ad8e5ae9481dd53"></a>
### `\set linesize`

<a id="1a802f6e2d01204d"></a>
#### 구문
...
<a id="9062b99d831e6535"></a>
#### 설명

질의 결과를 출력할 때 한 line의 최대 크기를 설정한다.

- linesize 값은 1 ~ 10000 범위의 양의 정수값이다.
- linesize의 기본값은 80 이다.

질의 결과를 출력할 때 한 row는 하나의 line을 기준으로 출력된다. Row의 column 개수가 많거나 출력 길이가 linsize보다 클 경우 다음 예와 같이 여러 line에 걸쳐 출력되기 때문에 가독성이 떨어진다.
...
<a id="2030abff10987b43"></a>
### `\set numsize`

<a id="14e287b54345f5fd"></a>
#### 구문
...
<a id="f3d95aa99a1f9d27"></a>
### `\set pagesize`

<a id="99d5f1d7fcdbc52a"></a>
#### 구문
...
<a id="607f432333c2a230"></a>
### `\set serveroutput`

<a id="3a1e200d5d75cf83"></a>
#### 구문
...
<a id="8b667863d96a0034"></a>
#### 설명

gsql이나 gsqlnet에서 DBMS_OUTPUT package가 제공하는 함수들로 작성한 메시지들을 자동으로 출력하는 기능을 제어한다.
...
<a id="1e2ed68ed1d593fc"></a>
### `\set time`

<a id="8f2a94e03adb9f3f"></a>
#### 구문
...
<a id="1fc85fd41709471a"></a>
### `\set timing`

<a id="addb5a574e723ed5"></a>
#### 구문
...
<a id="962c28f61f26211b"></a>
### `\set vertical`

<a id="9b2a60020e58df12"></a>
#### 구문
...
<a id="b18e96eee57f8243"></a>
### `\shutdown`

<a id="e8aba9f173e50d01"></a>
#### 구문
...
<a id="755042fe00e2f989"></a>
#### 설명

GOLDILOCKS 서버를 종료한다.  
`\shutdown` 명령을 수행하기 위해서는 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#27035efe8472921f)를 참조한다.
...
- `\shutdown immediate`
    - 새로운 단위 연산 (예: FETCH, EXECUTE 등)의 수행을 차단하고 현재 수행 중인 모든 단위 연산이 종료될 때까지 기다린 후, 모든 transaction들을 rollback 하고, checkpoint를 수행하고 서버를 종료한다.
- `\shutdown abort`
    - 접속 중인 session들의 상태와 관계없이 바로 서버를 강제 종료한다.
- `\shutdown`
    - `\shutdown normal`과 동일하다.

<a id="b1ba2689edeb6264"></a>
#### 사용 예
...
<a id="662ffdd4290509bd"></a>
### `\spool`

<a id="046556971a7b89f5"></a>
#### 설명

gsql을 사용하여 출력되는 모든 결과를 terminal과 file에 기록한다.

<a id="43432ae93623a577"></a>
#### `\spool 'filename'`

<a id="0f8d34d2e205e300"></a>
##### 구문
...
> Spool 기능이 동작하는 도중에 새로운 spool이 시작되면 기존 spool은 종료되고 새 spool이 시작된다.

<a id="9aac16fa601a3c39"></a>
##### 사용 예
...
<a id="8f2ba2043aa14602"></a>
#### `\spool OFF`

<a id="324cc7031388e89a"></a>
##### 구문
...
<a id="4eae8c4003837275"></a>
#### `\spool`

<a id="c168f1648ac536fe"></a>
##### 구문
...
<a id="f8f8557c1ad2c1a6"></a>
### `\startup`

<a id="b2477b92e9c78c36"></a>
#### 구문
...
<a id="b814699d4970e5c8"></a>
#### 설명

GOLDILOCKS 서버를 구동한다.  
`\startup` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#27035efe8472921f)를 참조한다.
...
서버의 구동 단계는 NOMOUNT, MOUNT, OPEN 단계로 구분되며 자세한 내용은 [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#d48808b833e799ba)을 참조한다.  
`\startup` 명령을 수행한 후에 다음 단계로 전이하려면는 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/16-sql-references.md#f434c9481edc0db8) 구문을 수행해야 한다.

<a id="59cc1a8dfea49f44"></a>
#### 사용 예
...
<a id="cb2c1c300622c188"></a>
### `\var`

<a id="b19eaa5590891fc2"></a>
#### 구문
...
<a id="04c2fcf003c47bde"></a>
#### 설명

호스트 변수를 선언한다.  
variable_name과 동일한 호스트 변수가 선언되어 있을 경우, 기존의 호스트 변수를 삭제하고 새로 선언한다. variable_name의 최대 길이는 128 byte 이다.  
호스트 변수의 data_type은 GOLDILOCKS의 데이터 타입과 동일한데 자세한 내용은 [Data Type](../part-03-sql-manual/11-sql-elements.md#ff81d005bda1af76)을 참조한다.  
호스트 변수는 [`\exec :var := value`](#754c255a8858ed59) 명령을 이용하여 값을 대입하며 [`\print`](#4a8c91a8ae58a8ce) 명령을 통해 값을 조회할 수 있다.
...
호스트 변수는 SQL 구문의 입력 또는 출력 인자로 사용할 수 있으며 SQL 구문이나 [`\exec :var := value`](#754c255a8858ed59) 명령에서 사용할 때는 colon (:) 기호를 호스트 변수 앞에 붙여 호스트 변수임을 명시해야 한다.

<a id="269112cf2e864b04"></a>
#### 사용 예
...
[UPDATE name RETURNING .. INTO](../part-03-sql-manual/16-sql-references.md#9430037a3767eba7) 구문을 수행하는 다음 예에서 SET 절과 WHERE 절의 호스트 변수 :v_id는 입력 인자로 사용되었으며 INTO 절의 :v_id는 출력 인자로 사용되었다.

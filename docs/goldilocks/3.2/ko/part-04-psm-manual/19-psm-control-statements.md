<a id="22faf83282da6521"></a>

# 19. PSM Control Statements

> 원본: [GOLDILOCKS 3.2 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/3_2_x/manual/ko/22faf83282da6521)  
> 태그: `GOLDILOCKS_Venus_3_2_14_retagged`

[← 18. PSM DataTypes](18-psm-datatypes.md) · [전체 목차](../README.md) · [20. PSM Cursor Statements →](20-psm-cursor-statements.md)

<a id="8715af88b117993a"></a>
## Assignment

Assignment 연산자 (':=')를 사용하여 오른쪽 expression이 계산된 결과 값을 왼쪽 변수에 삽입한다.

<a id="896b979fc0f49686"></a>
### Assignment Target
...
<a id="1d64ba0d497f7b62"></a>
| Target 타입 | Expression의 최종 결과 타입 |
| --- | --- |
| Scalar 타입  * Scalar 타입 변수 * 레코드 타입 변수의 특정 필드 * Key 값을 가지는 collection 변수의 scalar element | Target의 타입으로 conversion 가능한 값을 가진 Scalar 타입인 경우에만 허용된다. |
| Attribute 레코드 타입 (%ROWTYPE) | 필드의 개수가 같고, 각 필드의 값이 target의 해당 순번의 필드로 conversion 가능한 경우에만 허용된다. |
...
<a id="a040311cb37878a3"></a>
#### 실행부 (Executable Part)

실행부에서는 선언된 item들을 조작하고, 포함하고 있는 여러 statement들을 수행한다. 실행부에 올 수 있는 statement 종류는 다음과 같다.

- Control statements ([PSM Control Statements](#22faf83282da6521) 참조)
- Cursor statements ([PSM Cursor Statements](20-psm-cursor-statements.md#6f2dba85390de5e4) 참조)
- SQL statements (Static/ dynamic) ([Using SQLs In PSM](22-using-sqls-in-psm.md#8b691252fb3da255) 참조)

<a id="039788b7e21ec691"></a>
...
사용자 정의 exception 선언, built-in handler의 종류 및 handler의 설치 등에 대한 자세한 내용은 [Error Handling](#0c4c13a0b739ec43)을 참조한다.

<a id="bc46e042430c7a8c"></a>
## NULL Statement

PSM 내에서 no-op와 같이 아무런 역할도 하지 않는 statement를 명시할 때 사용한다.   
자세한 내용은 [NULL Statement](23-psm-language-element-references.md#b6a3e5f9f88fbaba)를 참조한다.
...
자세한 내용은 [IF Statement](23-psm-language-element-references.md#a840c60f2fa6fdae)를 참조한다.
...
자세한 내용은 [CASE Statement](23-psm-language-element-references.md#d542d13e5026a54e)를 참조한다.
...
자세한 내용은 [Basic LOOP Statement](23-psm-language-element-references.md#7bc0e49d642adbd1)를 참조한다.

다음은 EXIT WHEN 구문을 사용하여 해당 위치에서 가장 가까운 scope를 가지는 basic loop를 탈출하는 예이다.  
자세한 내용은 [EXIT Statement](23-psm-language-element-references.md#c02a4714e78afa89)를 참조한다.
...
<a id="c084e2b1c69899bf"></a>
### FOR Loop

FOR loop 구문은 주어진 범위의 정수 개수 만큼 일련의 statement들을 반복 수행한다.   
자세한 내용은 [FOR LOOP Statement](23-psm-language-element-references.md#287941cf9ced1e16)를 참조한다.

사용자가 FOR 키워드 뒤에 명시한 이름으로 인덱스 변수를 생성하고, IN 키워드 뒤의 범위 연산자 ('..') 왼쪽에 명시된 시작값을 초기값으로 하여 오른쪽에 명시된 종료값과 같아질 때까지 loop 한 번당 1씩 증가시켜 주어진 일련의 statement들을 수행한다.
...
범위 앞에 REVERSE 키워드를 명시하면 인덱스 변수의 값을 1씩 감소시키면서 loop를 수행한다.
...
<a id="1a54356861a9ddc5"></a>
### WHILE Loop

WHILE loop 구문은 주어진 조건이 TRUE인 동안 주어진 일련의 statement들을 반복해서 수행한다.  
자세한 내용은 [WHILE LOOP Statement](23-psm-language-element-references.md#46a44ab4f95a5507)를 참조한다.
...
자세한 내용은 [GOTO Statement](23-psm-language-element-references.md#712fd8282104fa49)를 참조한다.
...
<a id="0da87a1ab242ac84"></a>
### CONTINUE

CONTINUE 구문은 FOR나 WHILE 등의 loop 구문 내에서 현재 iteration을 종료하고 다음 iteration을 시작한다.   
자세한 내용은 [CONTINUE Statement](23-psm-language-element-references.md#6dc86c314905ec09)를 참조한다.
...
다음은 오류가 발생하는 위치를 exception handling 하여 중단없이 사용자 프로그램이 수행되도록 정의한 예이다.
...
자세한 내용은 [EXCEPTION Handling](#dd9b7918975b1430), [SQLCODE Function](23-psm-language-element-references.md#5dae88c607af81c9), [SQLERRM Function](23-psm-language-element-references.md#3b7cf148a615e7e0)을 참조한다.

<a id="87b09507bdf16ec9"></a>
### Cursor Attributes
...
- Cursor는 다음과 같이 분류된다.
    ...
    - Explicit cursor
        - 사용자가 명시적으로 declaration/ definition한 후에 OPEN/ FETCH/ CLOSE 하는 cursor

<a id="9239ae8d09dfcb32"></a>
...
> GOLDILOCKS에서 제공하는 predefined exception이나 user-defined exception에 대한 자세한 내용은 [Exception Declaration](23-psm-language-element-references.md#480c66a8f8725127)을 참조한다.

<a id="5dc6e9dd7df858b0"></a>
#### Exception 전파

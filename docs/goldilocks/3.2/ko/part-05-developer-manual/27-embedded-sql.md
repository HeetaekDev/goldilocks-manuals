<a id="3e9fa31b23d53521"></a>

# 27. Embedded SQL

> 원본: [GOLDILOCKS 3.2 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/3_2_x/manual/ko/3e9fa31b23d53521)  
> 태그: `GOLDILOCKS_Venus_3_2_14_retagged`

[← 26. JDBC](26-jdbc.md) · [전체 목차](../README.md) · [28. PDO →](28-pdo.md)

<a id="d1fbec386d34d7c6"></a>
## Precompiler

<a id="3f3e42539f5230e6"></a>
### 개요
...
<a id="aa2568234acd39f6"></a>
| Directory or File | 설명 |
| --- | --- |
...
gpec에 주어지는 옵션에 대한 자세한 내용은 [Precompiler Options](#ced85a7e81880433)를 참조한다.

<a id="33d691c7c60acfed"></a>
##### Example
...
<a id="ced85a7e81880433"></a>
### Precompiler Options

본 절에서는 gpec의 option들을 설명한다.

<a id="40cfda5e53dc4c75"></a>
#### --no-prompt, -n

<a id="6abd469fe7426379"></a>
##### 설명

Version 정보를 출력하지 않는다.

<a id="aeafd55f720c98ac"></a>
##### 사용 예
...
<a id="5905e5853b6e933d"></a>
#### --version, -v

<a id="1f92306268cef924"></a>
##### 설명

Version 정보만 출력하고 종료한다.

<a id="b89336e51b42b419"></a>
##### 사용 예
...
<a id="f1ced9160836b36d"></a>
#### --help, -h

<a id="7579879ec8f1b0c6"></a>
##### 설명

Help message를 출력한다. gpec에 아무런 option이나 &lt;input file&gt;을 주지 않을 때도 동일하게 동작한다.

<a id="381c9e5b2d57b6b6"></a>
##### 사용 예
...
<a id="671f0f7ca41c1942"></a>
#### --include-path, -I

<a id="6168cef5d1dbf100"></a>
##### 설명
...
<a id="76627f368a89b6b2"></a>
#### --no-lineinfo

<a id="d727b76864f205ef"></a>
##### 설명
...
<a id="ef410629a23a918a"></a>
#### --char_map, -c

<a id="770df2370df15498"></a>
##### 설명
...
<a id="44a841c799689778"></a>
#### --define, -D

<a id="a176943d5b3b5e9c"></a>
##### 설명

gpec에서 사용되는 define 이름으로써 1로 설정된다.

<a id="35b811180c189c7a"></a>
##### 사용 예
...
<a id="36b9dbffe1044ae4"></a>
## Embedded SQL

<a id="a58ad5abffc9257c"></a>
### Preprocessing

<a id="ec7b8803816f8027"></a>
#### 개요
...
<a id="63a02b4a4da97566"></a>
#### 적용 범위

gpec에 대한 SQL precompiler 기능은 declare section 내에서만 host 변수를 적용하고 EXEC SQL 문에 사용된다. (Declare section 외부에 있는 host 변수는 EXEC SQL 문에 사용할 수 없다.)  
gpec preprocessor는 source 전체에 적용된다.  
Include 파일에는 EXEC SQL INCLUDE로 선언한 header 파일만 적용된다.
...
> 1 _DEV1_가 DECLARE SECTION의 위치와 상관없이 정의되어 있기 때문에 gpec에서 host 변수로 처리된다.  
> 2 _DEV1_가 정의되어 있기 때문에 해당 위치는 false가 되어 gpec에서 공백으로 처리된다.

gpec을 이용하여 위 파일을 c 파일로 생성하면 다음과 같이 conn_str[20]은 변환되고, #ifdef, #elif, #endif preprocessor 문장과 conn_str[30]은 공백으로 바뀐 것을 확인할 수 있다.
...
> 1 _DEV1_가 DECLARE SECTION 위치와 상관없이 정의되어 있기때문에 gpec에서 host 변수로 처리된다.  
> 2 _DEV1_가 정의되어 있기 때문에 해당 위치는 false가 되어 gpec에서 공백으로 처리된다.

<a id="190810047e8cb8c9"></a>
#### 유형

<a id="a381f33688c95397"></a>
##### #if
...
> 2 _DEV2_는 #ifdef _DEV1_이 true이므로 gpec에서 공백으로 처리된다.  
> 3 _DEV2_가 undef 되었으므로 gpec에서 공백으로 처리된다.
...
<a id="b73a31bdf1039180"></a>
#### 확장

MACRO는 c 선언문의 중간이나 EXEC SQL 문의 중간에 사용할 수 있다.
...
gpec이 preprocessor를 선행 처리하여 다음과 같이 c code를 생성할 수 있다. 다음은 SQL statement가 c code로 변환되는 것이 생략된 예이다.
...
<a id="cffb128ca6709fe8"></a>
### 연결

<a id="e5645f7d6a6ae9d0"></a>
#### Database 연결
...
GOLDILOCKS는 shared memory에 직접 attach하여 구동하는 D/A 모드와, TCP 통신을 사용하여 database에 접속하는 C/S 모드 둘 다 지원한다. D/A 모드로 동작할 때는 database의 동일한 호스트에서 직접 접근하게 되므로 위와 같이 별도의 서버 정보를 포함하지 않아도 사용할 수 있지만 C/S 모드로 동작하기를 원할 때는 Data Source Name (DSN)을 지정하여 접근하도록 해야 한다. DSN에 대한 자세한 내용은 [데이터 원본 구성](25-odbc.md#99c2f1f58b73deeb)을 참조한다.
...
<a id="89b686734b8c5695"></a>
| NUMBER type 선언 | 설명 |
| --- | --- |
| NUMBER | NUMBER(38, 0)와 동일하다. |
| NUMBER(p) | NUMBER(p, 0)와 동일하다. |
...
<a id="5cb7dae7254f139a"></a>
###### **TIME**

TIME type은 시간을 다루는 datatype으로써 ODBC의 SQL_TIME_STRUCT를 구조로 갖는다.
...
<a id="2d6114c6c048f6d1"></a>
#### Indicator Variable

<a id="3879b78742b028fd"></a>
##### Scalar Indicator
...
<a id="20f03e02e0cd15c4"></a>
### Embedded SQL

<a id="a2e86a6cb94aaf54"></a>
#### Host Variable
...
<a id="94e27d8fe6808bd0"></a>
#### Basic SQL statement

Embedded SQL에서는 GOLDILOCKS에서 제공하는 모든 SQL 문을 사용할 수 있는데 SQL 문에 대한 자세한 내용은 [SQL Manual](../part-03-sql-manual/README.md#a7c30cddfb019671)을 참조한다. Embedded SQL에서 SQL 문을 사용할 때는 EXEC SQL 키워드 뒤에 SQL 문을 기술하는데 본 장에서는 Data Definition Language (DDL)이나 Data Manipulation Language (DML)에 대한 내용을 기술하고 query와 같이 반복적으로 data를 가져오는 구문에 대해서는 다음 장에서 설명한다.

SQL 문을 실행한 이후에는 SQLCA를 검사하여 실행한 SQL 문의 성공 여부를 판단할 수 있으며, 자세한 내용은 [Handling Run-time Error](#dfe33827e7a9ebbe) 부분을 참조한다.

<a id="0d8cbf03d0a04208"></a>
##### DDL Statement
...
이에 대한 자세한 내용은 [Embedded Dynamic SQL](#e652d9727ea939cd)을 참조한다.

<a id="da8fc9f8d52bfffd"></a>
##### Select Into Statement

GOLDILOCKS에서 data를 가져 오기 위해서 query를 사용한다. 일반적인 경우에는 해당 결과의 개수를 알지 못하는 경우가 많기 때문에, query를 사용하기 위해서는 cursor라는 객체를 선언하여 open, fetch, close라는 복잡한 과정을 거치게 된다. 이러한 과정의 실행 방식은 다음 [Cursor](#46c8f4b1a093575d) 부분에서 기술할 것이다.
...
> Host array를 사용하면 select into 구문을 사용하여 여러 개의 결과를 가져올 수 있다. Host array 사용법은 [Host Arrays](#49a5d76de39b5b7c) 를 참조한다.

<a id="eda6bd755693c77b"></a>
##### Insert Statement
...
> Host array를 사용하면 한 번에 여러 개의 row를 삽입할 수 있다. 자세한 내용은 [Host Array](#49a5d76de39b5b7c)를 참조한다.

<a id="a2ff4eaa795df4a5"></a>
##### Update Statement
...
> Host array를 사용하면 한 번에 여러 개의 row를 갱신할 수 있다. 자세한 내용은 [Host Array](#49a5d76de39b5b7c)를 참조한다.

<a id="123a41c95e46fc97"></a>
##### Delete Statement
...
> Host array를 사용하면 한 번에 여러 개의 row를 삭제할 수 있다. 자세한 내용은 [Host Arrays](#49a5d76de39b5b7c)를 참조한다.

<a id="1464952637ba6db2"></a>
##### PSM Statement

PSM 구문은 서버 내부에 procedure나 function을 만들어서 사용한다.  
PSM에 대한 자세한 내용은 [PSM Manual](../part-04-psm-manual/README.md#8eab24313cb0b51b)을 참조한다.
...
<a id="838b2b2603a9a3c9"></a>
##### Declare Cursor

Cursor를 선언한다. Cursor를 선언할 때는 cursor 이름과 그에 부합하는 질의를 기술해야 한다. 여기서 선언된 cursor 이름은 이후에 다른 cursor 조작 명령에 사용된다. 다음은 cursor 선언문의 예이다.
...
Cursor 이름은 precompiler에 의해 인식되고 사용되는 식별자로써 실제 C 프로그램의 변수와는 무관하다. 그러므로 cursor 선언문의 위치는 프로그램 소스 코드 상에서 해당 cursor에 접근하는 구문 앞에 위치해야만 하며 다른 파일에 정의된 cursor 선언문은 사용할 수 없다. 즉, cursor를 open/ fetch/ close 하는 구문은 모두 하나의 소스 파일 내에 위치해야 하며 cursor 선언문은 모든 구문들보다 앞서 위치해야 한다. 또한, 하나의 소스 파일 내에서 서로 다른 질의에 대해 동일한 cursor 이름을 중복하여 사용할 수 없다.
...
위와 같이 cursor 선언문이 뒤에 위치하면 precompile 과정에서 cursor가 선언되지 않았다는 에러가 발생한다.
...
커서를 open하면 선언된 커서의 질의를 수행한 결과 집합을 가져올 준비가 된다. 여기서는 실제로 결과를 가져오는 것은 아니기 때문에 실제 결과를 얻어오려면 [Fetch Cursor](#f8e5fefb29750c02)를 수행해야 한다.  
...
커서 정의 구문과 속성에 대한 자세한 내용은 [DECLARE cursor_name](../part-03-sql-manual/16-sql-references.md#c0f5909b51d661a3)을 참조한다.

<a id="8fe69684bd3e314e"></a>
##### Scrollable Cursor
...
<a id="0a610ed927f88321"></a>
<table><thead><tr><th align="center">Fetch orientation</th><th align="center">설명</th></tr></thead><tbody><tr><td align="left" valign="middle">NEXT</td><td align="left" valign="middle">현재 위치의 다음 row를 fetch 한다.</td></tr><tr><td align="left" valign="middle">PRIOR</td><td align="left" valign="middle">현재 위치의 이전 row를 fetch 한다.</td></tr><tr><td align="left" valign="middle">FIRST</td><td align="left" valign="middle">결과 집합의 첫 번째 row를 fetch 한다.</td></tr><tr><td align="left" valign="middle">LAST</td><td align="left" valign="middle">결과 집합의 마지막 row를 fetch 한다.</td></tr><tr><td align="left" valign="middle">CURRENT</td><td align="left" valign="middle">현재 위치의 row를 fetch 한다.</td></tr><tr><td align="left" valign="middle">ABSOLUTE &lt;position&gt;</td><td align="left" valign="middle"><ul><li>결과 집합에서 position의 위치에 해당하는 row를 fetch 한다.</li><li>Position 값이 음수일 경우, AFTER THE LAST ROW로부터 이전 위치에 해당하는 row를 fetch한다.</li></ul></td></tr><tr><td align="left" valign="middle">RELATIVE &lt;position&gt;</td><td align="left" valign="middle">현재 위치에서 position 만큼 떨어진 위치에 해당하는 row를 fetch 한다.</td></tr></tbody></table>
...
Static cursor의 fetch 방법은 [Scrollable Cursor](#8fe69684bd3e314e)를 참조한다

<a id="f972e17527a69d1d"></a>
...
Keyset driven 커서 역시 scroll 기능을 가지고 있으므로 fetch 방법은 [Scrollable Cursor](#8fe69684bd3e314e)를 참조한다.

<a id="ebd394825b40e034"></a>
#### Positioned DML
...
> 이 옵션은 precompiler의 command-line option을 통해서도 부여할 수 있다. 자세한 내용은 [Precompiler Options](#ced85a7e81880433)의 [--include-path, -I](#671f0f7ca41c1942)를 참조한다.

<a id="49a5d76de39b5b7c"></a>
### Host Array
...
<a id="f4d6fd1463f9a4ae"></a>
#### Host Array 사용

<a id="1ba293612cb09e5d"></a>
##### Host Array 접근
...
<a id="3e5f0bb3a676a144"></a>
##### SELECT INTO에서의 Array

가져와야 할 row의 개수를 정확히 알고 있을 경우에는 [Select Into Statement](#da8fc9f8d52bfffd)를 사용하여 간단하게 구현할 수 있다. [Select Into Statement](#da8fc9f8d52bfffd)에서는 결과가 없거나 한 개일 경우에 대해서만 언급하였지만 여기에 host array를 사용하면 다수의 row를 가져올 수 있다.
...
<a id="f6b3b5aabe1481c7"></a>
##### Cursor 사용할 때의 Array

현재 질의에 대한 결과 집합 개수를 알 수 없는 경우에는 커서를 사용해야만 한다. 커서를 사용하는 방법은 [Cursor](#46c8f4b1a093575d) 부분을 참조한다. 커서를 선언한 이후에 FETCH 구문에서 INTO 절의 host 변수를 array로 사용하는 것만으로도 다수의 row를 한 번에 가져올 수 있다.
...
sqlca에 대한 자세한 내용은 [Handling Runtime Errors](#dfe33827e7a9ebbe)를 참조한다.

<a id="53f8ed366b817154"></a>
#### Insert 구문에서의 Array
...
<a id="a3ba389600923b0a"></a>
#### 구조체 Array

일반적인 scalar 변수들을 배열로 사용하면 한 번에 다수의 row를 처리할 수 있어 편리하지만 한 개의 변수당 한 개의 column만 표시할 수 있다는 한계가 있다. 한 개의 host 변수에서 다수의 column에 접근하려면 [Host structure](#2edc5c2aa36ae96f)를 사용하면 되는데 이렇게 host 변수를 구조체로 선언하여 그 구조체를 array로 사용하면 다수의 column을 갖는 다수의 row를 한 번에 처리할 수 있다.
...
<a id="69f55c7665bdce40"></a>
##### 구조체 배열 Indicator

[Structure Indicator](#123f05e8bf6affc0)에 기술된 것과 같이 host 변수가 구조체이면 indicator 변수도 구조체이어야 한다. 따라서 host 변수가 구조체 배열일 경우 indicator 변수도 대응하는 구조체 배열이어야 한다.
...
<a id="dfe33827e7a9ebbe"></a>
### Handling Run-time Error

<a id="afeac5cabd70d410"></a>
#### 개요
...
<a id="7c5176e5f09e030e"></a>
#### Run-time Error 감지

<a id="a1c37dafc6a09b7b"></a>
##### SQLCA

Embedded SQL에서 발생하는 모든 종류의 에러는 SQL Communication Area (SQLCA)라고 하는 영역에 보고된다. 따라서 응용 프로그램에서 SQLCAS의 내용을 확인하면 현재 연산의 성공 여부와 함께 에러가 발생했을 경우 그 에러의 종류까지 파악할 수 있다.
...
GOLDILOCKS의 embedded SQL 응용 프로그램은 기본적으로 sqlca를 전역으로 가지고 있다. 따라서 single thread program에서는 별다른 선언 없이 sqlca를 사용할 수 있다. 그러나 multi thread program에서는 동시에 sqlca에 접근할 경우 동시성 문제가 발생하기 때문에 별도의 선언 방법을 가져야 한다. 이에 대한 내용은 [Multi-threaded Application](#44cfc7d706612a32) 에서 설명한다.

<a id="83ef014354d36920"></a>
##### SQLCA 구조
...
<a id="9beb65d2bdbe9043"></a>
## Advanced Topic

<a id="e652d9727ea939cd"></a>
### Embedded Dynamic SQL

<a id="44df4f6ae488a51d"></a>
#### 개요
...
현재 GOLDILOCKS에서는 method 3까지만 지원하고 있다.

<a id="99de2c1715ad0431"></a>
##### Method 1
...
Method 2에서 동일한 SQL 문을 반복 실행할 때, prepare는 한 번만 수행한 뒤에 execute를 반복 실행할 수 있다. 이 prepare된 statement는 현재 소스 코드 내에서 같은 statement name으로 다른 SQL 문을 prepare 하거나 disconnect할 때까지 유효하다.

<a id="7ef2e8c7b457659d"></a>
##### Method 3
...
&lt;cursor properties&gt;는 standing cursor와 의미 및 사용 이름이 동일하므로 이와 관련된 내용은 [Cursor Property](#0f4ba19b2f1f923e)를 참조한다.
...
Cursor를 open한다. Standing cursor의 [Open](#1c75405517b14b2b)과 비교하여 기본적인 면에서는 동일하지만 dynamic cursor의 open만 갖는 중요한 차이점이 있다. Dynamic cursor는 run-time에 SQL 문을 자유롭게 변경하여 사용할 수 있기 때문에 declare된 statement의 SQL에 따라 host variable이 결정된다. 따라서 dynamic cursor를 open할 때는 open 시점에 USING 절을 사용하여 host variable을 전달한다.
...
Cursor로부터 fetch한다. Dynamic cursor의 fetch는 [Standing cursor의 Fetch](#f8e5fefb29750c02)와 동일하다.

- Close cursor

Cursor를 close한다. Dynamic cursor의 close는 [Standing cursor의 Close](#ba37fd30fc7f0cb1)와 동일하다.

<a id="863693205873a3d5"></a>
#### Example Program
...
<a id="9a5432eb55e25524"></a>
![D/A 모드에서 각 thread가 각자의 connection을 가지는 경우](../assets/images/82027e500f8b6813.png)

<a id="51ce8a41b09f20b2"></a>
...
<a id="f2a6898b8f5f6b0a"></a>
![다수의 thread가 한 개의 connection을 공유하는 경우](../assets/images/f74619fdc5a91a94.png)

<a id="e9ff5baadf3da0b1"></a>
![한 개의 thread가 다수의 connection을 갖는 경우](../assets/images/1118e7e2e1cca0d2.png)

<a id="605a867483f171b4"></a>
#### Guideline
...
<a id="cab3eba0cfed96d2"></a>
### C++ Application

<a id="3af31b8b01b0a145"></a>
...
<a id="5759ed5bf5682d0f"></a>
#### SQLCA_STORAGE_CLASS

C++ application에서 동일한 symbol을 global 변수로 선언한 경우, symbol에 대한 충돌이 발생한다. GOLDILOCKS의 embedded SQL에서는 default로 sqlca 변수를 가지고 있는데 다수의 C++ file을 link할 때 이 변수의 충돌이 문제가 된다.
...
<a id="27d92dd79c118d4d"></a>
### XA

<a id="79b774c930684f28"></a>
#### xa_open string 정의

xa_open string은 Resource Manager (RM)에 접속하기 위한 정보를 포함하고 있는데 자세한 내용은 [SQLDriverConnect 인자설명](25-odbc.md#796ea077469c021a)을 참조한다.
...
<a id="1da657982b58d72b"></a>
### EXEC SQL AT

<a id="4ec9a94afda02ad4"></a>
#### 기능

Embedded SQL 문에 적용할 connection 이름을 지정한다.

<a id="82b18f0e39c28da9"></a>
#### 구문
...
<a id="3413988fbd98056a"></a>
#### 참조

관련 내용은 [연결](#cffb128ca6709fe8)을 참조한다.

<a id="ddf48370ce631b61"></a>
### EXEC SQL ATOMIC INSERT

<a id="65fdb4406ea1ad7a"></a>
#### 기능

Atomic array insert를 수행한다.

<a id="aa92b291a541203d"></a>
#### 구문
...
<a id="fc4472840529591e"></a>
#### 참조

관련 내용은 [Atomic Insert](#414cc4df6a221ab8)를 참조한다.

<a id="8160aab168e49be9"></a>
### EXEC SQL AUTOCOMMIT

<a id="afdf09279517f969"></a>
#### 기능

Autocommit 설정을 변경한다.

<a id="d46109d0b5c70493"></a>
#### 구문
...
<a id="5115738c513f63b1"></a>
#### 참조

관련 내용은 [Auto Commit](#6e76dd6fc1fe18e9)을 참조한다.

<a id="7f8ce746d588e911"></a>
### EXEC SQL BEGIN DECLARE SECTION

<a id="8f6ceaebc9081c79"></a>
#### 기능

Precompiler의 지시자로써 host variable 선언 영역을 지정한다.

<a id="3ee034657d1f983e"></a>
#### 구문
...
<a id="1babba37cc1e6bf8"></a>
#### 참조

관련 내용은 다음을 참조한다.

- [Host Variable의 선언](#bc95c06fdc74277b)
- [EXEC SQL END DECLARE SECTION](#64d31fb53b6fd5c7)

<a id="2d5762b064e62225"></a>
### EXEC SQL COMMIT RELEASE

<a id="104133b639a4a3b8"></a>
#### 기능

트랜잭션을 완료한 후에 connection을 종료한다.

<a id="296643e4a7f69761"></a>
#### 구문
...
<a id="9e7916e83db68ca2"></a>
#### 참조

관련 내용은 [RELEASE option](#b5fd9302d216eb3d)을 참조한다.

<a id="47106607b326a116"></a>
### EXEC SQL CONNECT

<a id="9d3c8415e62c0260"></a>
#### 기능

GOLDILOCKS와 connection을 맺는다.

<a id="f8162ef2b2c08f41"></a>
#### 구문
...
<a id="655f5816e82f7a17"></a>
#### 설명

GOLDILOCKS에 연결을 설정한다.

<a id="38b4b0267cabc8f4"></a>
#### 사용 예
...
<a id="9456f0bb4a148a45"></a>
#### 참조

관련 내용은 [Database 연결](#e5645f7d6a6ae9d0)을 참조한다.

<a id="f297ac5236b9f5f5"></a>
### EXEC SQL CONTEXT ALLOCATE

<a id="ef5dffb7f781d85c"></a>
#### 기능

Run-time context 메모리를 할당한다.

<a id="cac71c13496e2afa"></a>
#### 구문
...
<a id="3bc59fe6ed2ede7a"></a>
#### 참조

관련 내용은 [SQL_CONTEXT](#bd1772fd74c1b4a7)를 참조한다.

<a id="92aff0ce7622def3"></a>
### EXEC SQL CONTEXT FREE

<a id="43626f50c02d20f4"></a>
#### 기능

Run-time context 메모리를 해제한다.

<a id="f8aee8bbf85fd3a6"></a>
#### 구문
...
<a id="9433faf99671beef"></a>
#### 참조

관련 내용은 [SQL_CONTEXT](#bd1772fd74c1b4a7)를 참조한다.

<a id="ac60b14025fb429e"></a>
### EXEC SQL CONTEXT USE

<a id="dfadf1a24fb6d75c"></a>
#### 기능

Run-time context 사용을 알린다.

<a id="e62111940e783385"></a>
#### 구문
...
<a id="d156107c86596484"></a>
#### 참조

관련 내용은 [SQL_CONTEXT](#bd1772fd74c1b4a7)를 참조한다.

<a id="27d67b6ab6f39bf0"></a>
### EXEC SQL DISCONNECT

<a id="07f3cd9f24242357"></a>
#### 기능

GOLDILOCKS와의 connection을 종료한다.

<a id="7de247a3ca1d32d9"></a>
#### 구문
...
<a id="465738d1140699ac"></a>
#### 참조

관련 내용은 [Database 연결 해제](#06a5fba748175a0e)를 참조한다.

<a id="64d31fb53b6fd5c7"></a>
### EXEC SQL END DECLARE SECTION

<a id="dd20fa30a5061e61"></a>
#### 기능

Precompiler 지시자로써 host variable 선언 영역을 지정한다.

<a id="548eaa9337e6b154"></a>
#### 구문
...
<a id="52623495f1696319"></a>
#### 참조

관련 내용은 다음을 참조한다.

- [Host Variable의 선언](#bc95c06fdc74277b)
- [EXEC SQL BEGIN DECLARE SECTION](#7f8ce746d588e911)

<a id="5563049ba92a8dcd"></a>
### EXEC SQL FOR

<a id="8995829cea678c15"></a>
#### 기능

Array operation에서 array 개수를 지정한다.

<a id="b6bb18c1f4c39050"></a>
#### 구문
...
<a id="c5415fa094a5ae4e"></a>
#### 참조

관련 내용은 [FOR 절 사용](#8d2121a64a29953a)을 참조한다.

<a id="49e8f9f9ff6789e5"></a>
### EXEC SQL GET GROUPID INTO

<a id="f56112019c97221f"></a>
#### 기능

SQL statement의 group ID를 얻는다.

<a id="67ad551604d16872"></a>
#### 구문
...
<a id="6e1c2c6b108c4456"></a>
### EXEC SQL INCLUDE

<a id="17d56fe401b6dc76"></a>
#### 기능

Embedded SQL header file을 포함시킨다.

<a id="aa2f1b527e60167b"></a>
#### 구문
...
<a id="fbb6d1c55a1aed77"></a>
#### 참조

관련 내용은 [Precompiled Header File](#12cd93bab14a2119)을 참조한다.

<a id="8aa5bd4e8921f9f4"></a>
### EXEC SQL INCLUDE SQLCA

<a id="bb2ddb83b35795c5"></a>
#### 기능

*sqlca.h* header file을 포함시킨다.

<a id="7c1a963fcd9f4f15"></a>
#### 구문
...
<a id="9c5c10f2e458fa5c"></a>
#### 참조

관련 내용은 [Run-time Error 감지](#7c5176e5f09e030e)를 참조한다.

<a id="56f8b45ee45d40d4"></a>
### EXEC SQL OPTION

<a id="3adfb71d2fc0e7fe"></a>
#### 기능

Embedded SQL 소스 코드를 precompile 하는 과정에서 option을 적용한다.

<a id="5ce0dc1a70876891"></a>
#### 구문
...
<a id="561cdf05ace5854f"></a>
#### 설명

Embedded SQL 소스 코드를 precompiler 하는 과정에 적용할 option을 기술한다. 현재 version에서는 INCLUDE 경로 지정만 지원하고 있으며 이 option은 EXEC SQL INCLUDE에서 precompile 할 header file이 위치한 디렉토리를 기술한다.

<a id="34db1e5e45406de7"></a>
#### 사용 예
...
<a id="f2507fb32a149981"></a>
#### 참조

관련 내용은 [Header Files 경로 지정](#2e89bde8a945b278)을 참조한다.

<a id="23126d9bb989e599"></a>
### EXEC SQL ROLLBACK RELEASE

<a id="a3990efdd3097037"></a>
#### 기능

트랜잭션을 rollback 한 후에 connection을 종료한다.

<a id="9fbb17fd780d246b"></a>
#### 구문
...
<a id="a971644b7cc1b0e3"></a>
#### 참조

관련 내용은 [RELEASE option](#b5fd9302d216eb3d)을 참조한다.

<a id="28b9deae10b3b4a2"></a>
### EXEC SQL WHENEVER

<a id="af871bece7030a13"></a>
#### 기능

Embedded SQL 응용 프로그램에서 run-time exception handling을 실행한다.

<a id="c8d77e1859fbff19"></a>
#### 구문
...
<a id="7d7ae3b4d3ec9d5d"></a>
#### 설명

Embedded SQL 응용 프로그램에서 run-time exception handling을 자동화하여 처리한다. 네 가지 condition이 있고 condition마다 한 개의 action을 지정할 수 있는데 이 action은 필요에 따라 다시 지정할 수도 있다. 자세한 내용은 [Handling Implicit Error](#4984a048dd8e457b)를 참조한다.

<a id="d254a68ea74a6fbb"></a>
#### 사용 예
...
<a id="cfab58ceb2ad54d5"></a>
#### 참조

관련 내용은 [Handling Implicit Error](#4984a048dd8e457b)를 참조한다.

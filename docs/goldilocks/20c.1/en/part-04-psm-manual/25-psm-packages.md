...
<a id="d9178f7a869fd442"></a>
### Package State

A package is instantiated (instantiation) when it is referred or called from each session for the first time. In this case, the following operations occur.

- Setting the initial value which was specified when declaring the open variable
- Execute an initializing code if the code is defined in the body

A package whose package specification has one or more open variable or cursor exist, then it is called as a stateful package, otherwise it is called as a stateless package.
...
- When a package specification is recompiled by executing ALTER PACKAGE statement
- When DDL operation which alters the package instance form occurred on any of an object referred by a package specification
    - For example, if executing CREATE INDEX on a table which was referred to declare a variable type, then it is not invalidated. However, if executing ADD COLUMN it may be invalidated case by case.
- When any instantiated package in a session is invalidated

A stateless package does not have a state, so it is automatically recompiled even when the related object's form is altered. However, an instance can be invalidated in a stateful package, so be cautious to handle the situational exception when using a package in an application.

...
<a id="d4a79cea374540c6"></a>
### 제어 파일 내용

**GOLDILOCKS 데이터베이스 시스템 정보**

<a id="85a01c130206cef8"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="43ef885c8dbc66f0"></a>
### 제어 파일 훼손 시 대처 방안

데이터베이스 비정상 종료 등으로 인해 제어 파일이 훼손되었을 경우 다중화된 제어 파일 중 정상 제어 파일을 이용하여 훼손된 제어 파일을 복원한 후에 다시 시작한다.

만약 다중화된 제어 파일들이 모두 훼손되었을 경우, 백업된 제어 파일을 복원하여 archive redo log file과 redo log file까지 불완전 미디어 복구를 수행하여 재시작한다. 백업된 제어 파일을 이용한 불완전 복구는 시나리오별 복구 예제 중에 [다중화된 모든 제어 파일이 훼손된 경우](7-goldilocks-데이터베이스의-백업과-복구.md#e1f9de541cbdbff0)를 참조한다.

<a id="09bfa2def1c4c192"></a>
### 제어 파일 정보
...
GOLDILOCKS의 dump tool인 [gdump](../part-06-utility-manual/39-gdump.md#43afe5439b14a376)를 이용하면 제어 파일에 기록된 정확한 정보를 확인할 수 있다.

<a id="0562788626465737"></a>
## Redo Log File 관리
...
<a id="0f2f48c351214075"></a>
#### 로그 그룹 및 로그 멤버 추가

<a id="eb424a803a777ee6"></a>
##### 로그 그룹 추가
...
만약 로그 그룹의 로그 파일들이 모두 훼손되었거나 로그 멤버가 하나밖에 없는 경우, [불완전 복구](7-goldilocks-데이터베이스의-백업과-복구.md#57398a7883451215)를 수행하여 정상적인 로그 파일까지만 복구하여 재시작할 수 있다.

<a id="a486d78be675c6f4"></a>
### Redo Log File 정보
...
<a id="27aa988fec619d91"></a>
## Archive Redo Log File 관리

GOLDILOCKS의 redo log file은 circular 로그 그룹을 이용하여 사용된 로그 그룹을 재사용하므로 백업을 이용한 미디어 복구를 위해서는 데이터베이스를 archive 로그 모드로 운용해야 하는데 이 때 기록이 완료된 redo log file은 archive redo log file로 복사된다. GOLDILOCKS 데이터베이스의 archive log mode에 대한 자세한 내용은 [archivelog 모드](7-goldilocks-데이터베이스의-백업과-복구.md#0499326c18977254)를 참조한다.

<a id="8d8ae7b189d0b16f"></a>
...
<a id="17f2e86d9b50e156"></a>
#### Non SYSTEM 테이블스페이스

데이터를 저장하기 위한 테이블과 인덱스를 저장하기 위해 사용자가 임의로 생성하고 삭제할 수 있는 테이블스페이스들이다.

<a id="d6e1157b3890f03a"></a>
### 테이블스페이스 및 데이터 파일 관리

<a id="5041348ac140039b"></a>
#### 테이블스페이스 관리

<a id="c7cbd8e543f888d9"></a>
##### 테이블스페이스 상태 관리
...
<a id="49158436ea1bc9c9"></a>
#### 테이블스페이스 정보

데이터베이스에서 생성된 테이블스페이스의 정보를 보려면 [V$TABLESPACE](9-database-information.md#5b2bfa3724ad700f)를 참조한다.
...
<a id="8ca15ca023f416cf"></a>
## 데이터 파일 관리

<a id="6ca5c362c17fd914"></a>
### 데이터 파일 정합성

데이터 파일의 정합성은 디스크 고장이나 데이터베이스 결함, 사용자의 실수로 인해 깨어질 수 있다. 데이터 파일의 정합성이 깨어진 것을 인식하지 못한 채 데이터베이스를 이용하여 서비스하면 심각한 문제가 발생한다.

이를 위해 GOLDILOCKS 데이터베이스는 데이터 파일의 각 페이지 별 checksum을 이용하여 데이터 파일의 정합성을 보장한다. LSN과 CRC 값을 이용하여 GOLDILOCKS 데이터베이스의 페이지 checksum을 생성하고 각 페이지에 저장한다. 사용자는 [PAGE_CHECKSUM_TYPE](10-server-property.md#b6ea0cd5dd02fd7c) 값 설정을 통해 페이지 checksum 종류를 설정하고 기본값으로 LSN을 사용한다.
...
<a id="44c76086e3afce77"></a>
### 데이터 파일 정보

데이터베이스에서 사용 중인 데이터 파일의 정보를 보려면 [V$DATAFILE](9-database-information.md#0a060ac257671be4)을 참조한다.
...
<a id="c9d558c6fc674ab3"></a>
## 버퍼 캐쉬

<a id="26ed460eb66fd635"></a>
### GOLDILOCKS 버퍼 캐쉬 구조
...
| 버퍼 플러쉬 리스트 | 버퍼 LRU 리스트에서 재사용하기 위한 갱신된 페이지를 데이터파일에 반영하기 위한 버퍼 프레임의 리스트 | BUFFER_FLUSH_THREADS |
...
버퍼 체크포인트 리스트를 제외한 버퍼 프리/ LRU/ 플러쉬 리스트는 어떤 페이지 프레임도 중복해서 연결될 수 없다. 즉, 버퍼 프리 리스트에 존재하는 페이지 프레임은 동시에 버퍼 LRU/ 플러쉬 리스트에 존재할 수 없고, 버퍼 LRU 리스트에 존재하는 페이지 프레임 역시 버퍼 프리/ 플러쉬 리스트에 존재할 수 없다.

버퍼 LRU 리스트는 hot/ cold 영역으로 구분되어 cold 영역에 있는 페이지 프레임의 터치 카운트가 정해진 값 ([BUFFER_HOT_REGION_CRITERIA](10-server-property.md#d3646878a9120d28))과 같거나 크면 hot 영역으로 옮겨진다. 버퍼 LRU 리스트의 페이지 프레임들이 접근할 때마다 터치 카운트가 증가되다가 모두 hot 영역에 속하게 되면 대체할 페이지 프레임을 구할 수 없기 때문에 hot 영역의 크기를 [BUFFER_HOT_REGION_PERCENT](10-server-property.md#10e1956399f865fb) 프로퍼티 값으로 제한한다.

<a id="6ca07b7767a5c208"></a>
### 페이지 프레임 상태

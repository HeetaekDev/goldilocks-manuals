...
<a id="bdce2f26ce395971"></a>
## 백업과 복구

<a id="7ad48ebfa52b203d"></a>
### 백업

<a id="77db51072cd96c5e"></a>
#### 백업/ 복구 목적
...
체인지 트래킹이 enable 되면 체인지 트래킹 파일과 공유 메모리를 생성한다. 체인지 트래킹 파일과 공유 메모리의 구조는 동일하고 [CHANGE_TRACKING_EXTENT_SIZE](10-server-property.md#2c74cdeb62faf3ef) 프로퍼티에 설정된 개수만큼의 페이지 묶음에 대한 변경 플래그를 저장하는 블록들로 구성된다.
...
체인지 트래킹이 enable 되면 기본적으로 [CHANGE_TRACKING_FILE](10-server-property.md#972bd916aa3076a6) 프로퍼티에 설정된 위치에 파일이 생성된다. enable 할 때 체인지 트래킹을 위한 파일 이름이나 저장 위치까지 설정하면 설정된 위치에 파일이 생성되고, 저장 위치가 설정되지 않은 경우에는 BACKUP_DIR 프로퍼티에 설정된 위치에 체인지 트래킹 파일이 생성된다.
...
<a id="dbadcfde64feb4ba"></a>
#### 복구 예

<a id="65045b3c7307c7a6"></a>
##### 제어 파일 훼손

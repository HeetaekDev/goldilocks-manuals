...
<a id="f0212009119cec92"></a>
## GOLDILOCKS PyDBC

<a id="769b2a4885104e4a"></a>
### 개요
...
PyDBC의 내부 연산은 ODBC 드라이버를 사용하기 때문에 [ODBC 구성요소 개요](29-odbc.md#ccfdb2d55b665493)와 동일하다. 응용 프로그램이 드라이버 관리자로 링크하는 아키텍처와 응용 프로그램이 GOLDILOCKS ODBC driver 라이브러리로 링크하는 아키텍처가 있다.

<a id="2a4226a5f3e7f2d3"></a>
### 버전 체계
...
<a id="a482b809111f5a57"></a>
### 사용 예

<a id="a120148eb5cb020b"></a>
#### connection 클래스 획득

PyDBC는 내부적으로 ODBC 라이브러리를 호출한다. 따라서 커넥션을 얻기 위해 [데이터 원본](29-odbc.md#95f39b6bab8f3af7)을 구성해야 한다.
...
> DSN을 사용하기 위해서는 [데이터 원본](29-odbc.md#95f39b6bab8f3af7)이 사전에 구성되어 있어야 한다.
...
- [데이터 원본 구성](29-odbc.md#95f39b6bab8f3af7)에 CHARSET 프로퍼티를 변경한다.
...
<a id="0b0e0f397070dab2"></a>
## API Reference

<a id="14d40a21d93ca772"></a>
### pygoldilocks 모듈
...
- attrs_before
    - 연결 전에 설정되는 옵션을 지정한다. 이 옵션들은 [SQLSetConnectAttr](29-odbc.md#3d062855e30a3891)을 사용하여 설정한다. 속성과 값은 딕셔너리 타입으로 받는다. 설정에 대한 자세한 내용은 [ODBC 속성](29-odbc.md#3eaad8090a5f88bb)을 참조한다.

```
cnxn =  pygoldilocks.connect( "DSN=GOLDILOCKS", attr_before={ pygoldilocks.SQL_ATTR_MAX_ROWS : 1000 })
```

<a id="68cc7e1b406ce9a8"></a>
#### Date
...
주어진 값에 해당하는 datetime 객체를 생성한다. [Timestamp](#8f816b1da37b14d5)와 동일하다.

<a id="361b96031b24da48"></a>
#### Binary
...
주어진 값에 해당하는 bytearray 객체를 생성한다. [BINARY](#965d09aad13dd361)와 동일하다.

<a id="965d09aad13dd361"></a>
#### BINARY
...
- getinfo( info )
    - ODBC의 SQLGetInfo 함수를 이용하여 연결 관련 속성을 얻을 수 있다. 자세한 내용은 [SQLGetInfo](29-odbc.md#5d7a5877a8a7f4a9)를 참조한다.
...
- set_attr( attr_id, value )
    - [SQLSetConnectAttr](29-odbc.md#3d062855e30a3891) 함수를 실행하여 연결 속성을 설정할 수 있다.
...
<a id="721b900eca7ef17a"></a>
#### 속성

<a id="164d8a2d94de8284"></a>
##### Description
...
<a id="c0d9b893b9b5f34a"></a>
##### rowcount

마지막에 수행한 SQL 구문이 갱신한 row의 개수이다.

<a id="ebfc505ec9308c7c"></a>
##### arraysize

[fetchmany( [size = cursor.arraysize] )](#7149047e90855298) 함수를 사용하여 한 번에 가져올 수 있는 row의 개수이다. 기본값은 1이다.

<a id="e700eb234faf8625"></a>
##### connection

읽기 전용 속성으로써 해당 cursor 객체를 생성한 connection 객체를 가리킨다.

<a id="cda359e44e29c4a3"></a>
##### fast_executemany

True로 설정되면 [executemany( sql, [*params] )](#790cbac88a52d410) 함수를 실행할 때 매개 변수를 배열로 구성하여 한 번의 execute로 처리한다. False로 설정되면 매개 변수마다 개별적으로 execute를 실행한다.

<a id="fc5be91f57d10b22"></a>
#### 함수

<a id="87b1ba8ecf650b87"></a>
##### execute( sql, [*params] )
...
지정된 조건을 만족하는 데이터베이스의 테이블 정보를 반환한다. 문자 '_'와 '%'는 와일드 카드로 해석된다. 각 row는 다음의 column 정보를 가지는데 자세한 내용은 [SQLTables](29-odbc.md#b0e27282ee2ba34d)를 참조한다.
...
<a id="587c59f2c5a19944"></a>
##### columns( table=None, catalog=None, schema=None, tableType=None )

[SQLColumns](29-odbc.md#3c2c072a0b2c8e67) 함수를 통해 지정된 테이블의 column 정보를 얻는다. 각 row는 다음과 같은 column 정보를 포함한다.
...
<a id="537da3ca7f953405"></a>
##### rowIdColumns( table, catalog=None, schema=None, nullable=True )

SQL_BEST_ROWID로 [SQLSpecialColumns](29-odbc.md#7eca868c87e34ea0)를 실행하여 row를 고유하게 식별하는 column의 결과 집합을 반환한다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="2074437bf6588912"></a>
##### rowVerColumns( table, catalog=None, schema=None, nullable=True )

SQL_ROWVER으로 [SQLSpecialColumns](29-odbc.md#7eca868c87e34ea0)를 실행하여 row가 업데이트 될 때 자동으로 업데이트 되는 column의 결과 집합을 반환한다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="16264651cf744b8d"></a>
##### primaryKeys( table, catalog=None, schema=None )

[SQLPrimaryKeys](29-odbc.md#882b9d56a7de45f1) 함수를 실행하여 테이블의 주요 키를 구성하는 column의 결과 집합을 반환한다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="7da6db3a677cbb85"></a>
##### foreignKeys( table=None, catalog=None, schema=None, foreignTable=None, foreignCatalog=None, foreignSchema=None )

[SQLForeignKeys](29-odbc.md#3924281e746a9619) 함수를 실행하여 지정된 테이블 또는 지정된 테이블의 기본 키를 참조하는 다른 테이블의 외래 키인 column 이름의 결과 집합을 만든다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="7251c3627cd9f0b5"></a>
##### procedures( procedure=None, catalog=None, schema=None )

[SQLProcedures](29-odbc.md#6a4b4bff48745722)를 실행하여 프로시저에 대한 정보의 결과 집합을 만든다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="29ebb1ab6861558f"></a>
##### getTypeInfo( sqlType=None )

[SQLGetTypeInfo](29-odbc.md#ba2decd3ca6b28c6) 함수를 실행하여 지정된 데이터 타입 또는 GOLDILOCKS ODBC가 지원하는 모든 데이터 타입에 대한 정보의 결과 집합을 만든다. 각 row는 다음과 같은 column을 갖는다.
...
<a id="a53eac12ec97d383"></a>
#### 속성

- cursor_description

해당 row를 생성한 cursor 객체의 속성 description의 복사본이다. 자세한 내용은 [Cursor.description](#721b900eca7ef17a)을 참조한다.

<a id="5cf8f65f64c8886b"></a>
## Exception
...
<a id="0c117d551dd94529"></a>
## Data Type

<a id="d867503f480e97a2"></a>

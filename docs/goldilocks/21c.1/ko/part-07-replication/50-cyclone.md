<a id="c5201c95d739d30c"></a>

# 50. CYCLONE

> 원본: [GOLDILOCKS 21c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/21c_1/manual/ko/c5201c95d739d30c)  
> 태그: `21c.1_35_tag`

[← 49. 개요](49-개요.md) · [전체 목차](../README.md) · [51. LOGMIRROR →](51-logmirror.md)

<a id="29e1de9a3b12af15"></a>
## CYCLONE

CYCLONE은 Change Data Capture (CDC) 방식을 사용하는 이중화 툴이다.

<a id="074cf81a4c674cc4"></a>
### 개요
...
- 이중화가 진행 중인 table에 Data Definition Language (DDL)이 수행된 경우 give up 될 수 있다.
    - Table1의 [DDL 구문에 따른 give up 발생 및 절차에 따른 허용 여부](#85f345e9fde1d288)를 참조한다.
...
> 사용자의 실수를 방지하기 위해, 이중화 give up을 유발하는 DDL을 수행하지 못하도록 [DISABLE_DDL_CDC_GIVEUP](../part-02-administration-manual/10-server-property.md#6f92112033620276) 서버 프로퍼티를 이용해 제어할 수 있다. 또한 [DISABLE_UPDATE_PK_CDC_GIVEUP](../part-02-administration-manual/10-server-property.md#cd17713f26634775) 서버 프로퍼티를 사용하여 primary key 갱신을 비활성화 할 수 있다.

<a id="acd99ca42bafee10"></a>
### 이중화 운영 중 DDL 처리
...
** 자세한 내용은 [DDL 구문에 따른 give up 발생 및 절차에 따른 허용 여부](#85f345e9fde1d288)를 참조한다.
...
- 다른 DBMS와 연동할 경우 SYNC 기능과 허용 가능한 DDL 처리 기능을 사용할 수 없다.

<a id="2ecef6830ee0ba4b"></a>
#### Oracle

**Oracle 연동 datatype**

<a id="d29ae1eb15a3712b"></a>
...
<a id="0e878d0c97d06af6"></a>
#### MySQL

**MySQL 연동 datatype**

<a id="4951fca3ec31fd0b"></a>
| GOLDILOCKS | MySQL | 비고 |
| --- | --- | --- |
...
<a id="d16fa8647d3e3f7d"></a>
#### DB2

**DB2 연동 datatype**

<a id="f28574f7294afa6a"></a>
| GOLDILOCKS | DB2 | 비고 |
| --- | --- | --- |
...
<a id="699f3139cb245757"></a>
#### 데이터베이스 권한

<a id="5606405701208ee7"></a>
##### 사용자의 접속 권한을 설정
...
<a id="f656d4587d028c83"></a>
## 환경설정

<a id="17ae0458b5b0c128"></a>
### 환경설정 파일
...
| DB2_DRIVER | DB2에서 제공하는 DB2 ODBC driver의 파일 위치를 기술한다. | Slave |
| DB2_DATABASE | 이중화하려는 DB2의 database 이름을 기술한다. | Slave |

<a id="6abab92b6c382ed7"></a>
### 환경설정 옵션

<a id="9883ea52a41c632b"></a>
#### COMM_CHUNK_COUNT
...
<a id="c951278980e5b928"></a>
#### TRANS_SORT_AREA_SIZE

- Redo log file을 캡쳐할 때 필요한 메모리 공간을 설정한다.
- Master에서만 설정할 수 있다.
- 단위는 MB (Megabyte) 이다.
- 기본값은 500 MB 이다.
    - 최소값은 10 MB 이다.
...
<a id="1df293ff63679580"></a>
| 구분 | 파일 |
| --- | --- |
| Cyclone Master | $GOLDILOCKS_DATA/trc/cyclone_master.trc |
| Cyclone Slave | $GOLDILOCKS_DATA/trc/cyclone_slave.trc |

> Trace log에 저장되는 에러 메시지와 대처 방안은 [CYCLONE 에러 메시지와 처리 방법](../part-02-administration-manual/8-goldilocks-데이터베이스-이중화.md#f530b8d0750ff028)을 참조한다.

<a id="2a44f253dfb70b2a"></a>
### GOLDILOCKS 접속 정책
...
<a id="c942513c6d0322cf"></a>
#### 올바르게 설정된 예

- D/A로 접속하였기 때문에 USER_ID, USER_PW만으로도 가능하다.

**DA로 접속함으로 USER_ID, USER_PW만으로 가능함.**

<a id="baa12e896c88da6c"></a>
| 구분 | 파일 |
| --- | --- |
...
**CONFIG나 odbc.ini둘중에 한군데만 정보가 있으면 됨. DA로 동작함으로 HOST_IP, HOST_PORT는 무시됨.**

<a id="0b8fd58257a6d872"></a>
| 구분 | 파일 |
| --- | --- |
...
**TCP로 접속하고 CONFIG가 우선임으로 USER_ID는 test 계정으로 동작함.**

<a id="8bff7991cee56a7d"></a>
| 구분 | 파일 |
| --- | --- |
...
<a id="e6cad846a2c75599"></a>
#### 잘못 설정된 예

- D/A로 접속하는데 USER_PW가 없다.

**DA로 접속하는데 USER_PW가 없다.**

<a id="b061c7442d1df95b"></a>
| 구분 | 파일 |
| --- | --- |
...
> 그룹 또는 노드의 추가/삭제와 관련된 예는 [노드 추가와 삭제](../part-02-administration-manual/8-goldilocks-데이터베이스-이중화.md#715946ed7259fe87)를 참조한다.  
> 이중화 초기화와 관련된 예는 [이중화 초기화](../part-02-administration-manual/8-goldilocks-데이터베이스-이중화.md#b7a397a18cffe073)를 참조한다.

<a id="4d324226c5e67f42"></a>
## CYCLONE 운영 예

CYCLONE 운영 구조는 다음과 같다.

<a id="b8c5f3e0c7ac4f78"></a>
![CYCLONE 운영 구조도](../assets/images/2a055666c8c10e95.png)
...
<a id="5eff4cef1deff25a"></a>
### 원본 GOLDILOCKS 환경 설정

기술된 [준비사항](#a6fd21ff9afd2ef2)을 모두 수행한다.
...
<a id="26ca674f7ef503f9"></a>
### 원격 GOLDILOCKS 환경 설정

[사용자 등록 및 권한 설정](#958162c4a33061d0)을 수행한다.

<a id="e5c6db5f94b3deca"></a>
...
> Slave의 target DB가 Oracle이나 MySQL인 경우에는 SYNC 기능이 동작하지 않는다.
...
<a id="eb5598cff32a632c"></a>
## Cluster 환경에서 CYCLONE 운영

<a id="110c1df9764b453e"></a>
### 준비사항
...
<a id="b81df94906306238"></a>
### 운영

<a id="4376e88d5d122a3e"></a>
#### 실행과 종료
...
<a id="20041fc19c528334"></a>
### 수행

<a id="d16bd138e0ad8766"></a>
#### 환경 설정 파일
...
<a id="8d7f55422a49b881"></a>
#### 실행 방법

<a id="5b6243909b20635e"></a>
##### Master에서 수행
...
<a id="ecb1895e0366fd7d"></a>
### SYNC

<a id="b6a6130eee74f880"></a>
#### Master에서 수행
...
<a id="1002938ddf07ba97"></a>
### 기타

--reset 옵션은 standalone 환경에서 쓰이는 것과 동일하게 사용할 수 있다.

<a id="1c1fa62101f376bb"></a>
## 모니터링 (CYMON)
...
<a id="30d711471aa7366a"></a>
| 파일 이름 | 구분 |
| --- | --- |
| cymon | cyclone용 monitor |

<a id="a5b4abfc2f82d195"></a>
### 환경 설정 파일
...
<a id="a48c999e55835e0e"></a>
### 모니터링 내용

<a id="a70ccdac5a07d89b"></a>
#### CYMON
...
<a id="ea7dcbedb254e606"></a>
### 실행 및 모니터링

<a id="ecdd38974a41d824"></a>
#### 실행 옵션

**실행옵션**

<a id="5cba4abdac49cbe9"></a>
| 옵션 | 설명 | 비고 |
| --- | --- | --- |
| --conf \| -c | 환경파일의 경로를 설정한다. | --conf CONFIG_FILE 형식으로 입력한다. * CYCLONE을 master 환경에서<br>  실행할 때 사용한 환경 파일과 동일한 파일로 설정해야 한다. |
| --start \| -s | CYMON을 실행한다. | - |
| --stop \| -t | CYMON을 종료한다. | - |

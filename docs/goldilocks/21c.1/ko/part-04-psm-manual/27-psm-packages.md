<a id="9e61063b0fcbfa6b"></a>

# 27. PSM Packages

> 원본: [GOLDILOCKS 21c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/21c_1/manual/ko/9e61063b0fcbfa6b)  
> 태그: `21c.1_35_tag`

[← 26. Using SQLs in PSM](26-using-sqls-in-psm.md) · [전체 목차](../README.md) · [28. PSM Language Element References →](28-psm-language-element-references.md)

본 장에서는 package를 이용하여 여러 응용 프로그램에서 공통으로 사용되는 데이터와 PSM 코드를 모듈화하는 방법에 대해 설명한다.

<a id="61b0f4cb92386b61"></a>
## 정의
...
<a id="f3d4f3b0a9dd879b"></a>
### Package 상태

Package는 stateful package와 stateless package로 구분한다.

Stateful package는 하나 이상의 변수나 커서를 포함하여 구성한 package이고, stateless package는 procedure나 function과 같은 subprogram으로만 구성한 package이다.

Stateful package는 package의 변수나 커서를 처음으로 참조할 때 package instance를 session에 생성한다. 이 때 아래와 같은 작업을 수행한다.

- Package 변수의 초기화
- Initialize section 실행

일반적으로 package 상태는 session의 lifetime과 동일하게 유지되지만 stateful package의 경우, 다음과 같은 경우에 invalidate되어 instance가 초기화된다.

- REPLACE PACKAGE 구문을 실행하여 package를 다시 생성한 경우
- ALTER PACKAGE 구문을 실행하여 package를 recompile 한 경우
- Package가 참조한 객체들 중 어느 하나에라도 package instance의 형상에 영향을 줄 수 있는 변경 (DDL) 작업이 발생한 경우

Stateless package는 상태를 가지지 않기 때문에 관련 객체의 형상이 변경되더라도 자동으로 recompile 되어 수행한다. 그러나 stateful package는 기존의 package instance를 제거하고 변경된 package의 정보를 기반으로 다시 package instance를 생성한다.

따라서 응용 프로그램에서 package를 사용할 때는 이런 상황에 따른 예외 처리에 유의해야 한다.

<a id="a4db741136ec0d0f"></a>

# 35. PyDBC

> 원본: [GOLDILOCKS 21c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/21c_1/manual/ko/a4db741136ec0d0f)  
> 태그: `21c.1_35_tag`

[← 34. PDO](34-pdo.md) · [전체 목차](../README.md) · [36. Hibernate →](36-hibernate.md)

<a id="e8aaa93e48d1bd43"></a>
## GOLDILOCKS PyDBC

<a id="4ed9545ea38ead69"></a>
### 개요
...
PyDBC의 내부 연산은 ODBC 드라이버를 사용하기 때문에 [ODBC 구성요소 개요](31-odbc.md#ed2d662ce675803d)와 동일하다. 응용 프로그램이 드라이버 관리자로 링크하는 아키텍처와 응용 프로그램이 GOLDILOCKS ODBC driver 라이브러리로 링크하는 아키텍처가 있다.

<a id="4c16fb3d7e2f64ec"></a>
### 버전 체계
...
<a id="2774aae03c298179"></a>
### 사용 예

<a id="7bbaec4b57d6f666"></a>
#### connection 클래스 획득

PyDBC는 내부적으로 ODBC 라이브러리를 호출한다. 따라서 커넥션을 얻기 위해 [데이터 원본](31-odbc.md#e877d5722faabf1c)을 구성해야 한다.
...
> DSN을 사용하기 위해서는 [데이터 원본](31-odbc.md#e877d5722faabf1c)이 사전에 구성되어 있어야 한다.
...
- [데이터 원본 구성](31-odbc.md#e877d5722faabf1c)의 CHARSET 프로퍼티를 변경한다.
...
<a id="c42274ecebd0b986"></a>
## API Reference

<a id="61eb33463398010e"></a>
### pygoldilocks 모듈
...
- attrs_before
    - 연결 전에 설정되는 옵션을 지정한다. 이 옵션들은 [SQLSetConnectAttr](31-odbc.md#7f245b8c490c610f)을 사용하여 설정한다. 속성과 값은 딕셔너리 타입으로 받는다. 설정에 대한 자세한 내용은 [ODBC 속성](31-odbc.md#85f8ba29fceda051)을 참조한다.

```
cnxn =  pygoldilocks.connect( "DSN=GOLDILOCKS", attr_before={ pygoldilocks.SQL_ATTR_MAX_ROWS : 1000 })
```

<a id="ab81e5a3e24feec0"></a>
#### Date
...
주어진 값에 해당하는 datetime 객체를 생성한다. [Timestamp](#59e71675b138e29a)와 동일하다.

<a id="22a53397a54a2fd4"></a>
#### Binary
...
주어진 값에 해당하는 bytearray 객체를 생성한다. [BINARY](#5da6c8077854258c)와 동일하다.

<a id="5da6c8077854258c"></a>
#### BINARY
...
- getinfo( info )
    - ODBC의 SQLGetInfo 함수를 이용하여 연결 관련 속성을 얻을 수 있다. 자세한 내용은 [SQLGetInfo](31-odbc.md#4e1ebd2ba41b3b9f)를 참조한다.
...
- set_attr( attr_id, value )
    - [SQLSetConnectAttr](31-odbc.md#7f245b8c490c610f) 함수를 실행하여 연결 속성을 설정할 수 있다.
...
<a id="2f0eb74d2a8ee6dd"></a>
#### 속성

<a id="f07dab5b83abfa3e"></a>
##### Description
...
<a id="1df0425c75174a70"></a>
##### rowcount

마지막에 수행한 SQL 구문이 갱신한 row의 개수이다.

<a id="0996cd94259f4665"></a>
##### arraysize

[fetchmany( [size = cursor.arraysize] )](#8d0f1d90d6262e48) 함수를 사용하여 한 번에 가져올 수 있는 row의 개수이다. 기본값은 1이다.

<a id="257a406331aaeab0"></a>
##### connection

읽기 전용 속성으로써 해당 cursor 객체를 생성한 connection 객체를 가리킨다.

<a id="8fb270bdad194073"></a>
##### fast_executemany

True로 설정되면 [executemany( sql, [*params] )](#52c664cb04c5ae64) 함수를 실행할 때 매개 변수를 배열로 구성하여 한 번의 execute로 처리한다. False로 설정되면 매개 변수마다 개별적으로 execute를 실행한다.

<a id="bb4bf2a74a7d58d9"></a>
#### 함수

<a id="095f66ed1c4b303b"></a>
##### execute( sql, [*params] )
...
<a id="e97ae743a8dc9568"></a>
##### tables( table=None, catalog=None, schema=None, tableType=None )

지정된 조건을 만족하는 데이터베이스의 테이블 정보를 반환한다. 문자 '_'와 '%'는 와일드 카드로 해석된다. 각 row는 다음의 column 정보를 가지는데 자세한 내용은 [SQLTables](31-odbc.md#a7f3e70032096411)를 참조한다.
...
<a id="78f2feb40da20649"></a>
##### columns( table=None, catalog=None, schema=None, tableType=None )

[SQLColumns](31-odbc.md#abd07469434aef22) 함수를 통해 지정된 테이블의 column 정보를 얻는다. 각 row는 다음과 같은 column 정보를 포함한다.
...
<a id="11d7e231236d6192"></a>
##### statistics( table, catalog=None, schema=None, unique=False, quick=True )

[SQLStatistics](31-odbc.md#682ce1fb31cad5f4) 함수를 통해 지정된 테이블과 관련된 정보를 얻는다.  
...
<a id="9ce044e43f2fee43"></a>
##### rowIdColumns( table, catalog=None, schema=None, nullable=True )

SQL_BEST_ROWID로 [SQLSpecialColumns](31-odbc.md#aaec5cbf2cb1e0b4)를 실행하여 row를 고유하게 식별하는 column의 결과 집합을 반환한다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="2645a409f2960604"></a>
##### rowVerColumns( table, catalog=None, schema=None, nullable=True )

SQL_ROWVER으로 [SQLSpecialColumns](31-odbc.md#aaec5cbf2cb1e0b4)를 실행하여 row가 업데이트 될 때 자동으로 업데이트 되는 column의 결과 집합을 반환한다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="1cd1fa8551828a13"></a>
##### primaryKeys( table, catalog=None, schema=None )

[SQLPrimaryKeys](31-odbc.md#b6c2c47d9d3d7a1f) 함수를 실행하여 테이블의 주요 키를 구성하는 column의 결과 집합을 반환한다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="285d60502fb1241b"></a>
##### foreignKeys( table=None, catalog=None, schema=None, foreignTable=None, foreignCatalog=None, foreignSchema=None )

[SQLForeignKeys](31-odbc.md#9277776e18955d63) 함수를 실행하여 지정된 테이블 또는 지정된 테이블의 기본 키를 참조하는 다른 테이블의 외래 키인 column 이름의 결과 집합을 만든다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="101d24f468f3d8f3"></a>
##### procedures( procedure=None, catalog=None, schema=None )

[SQLProcedures](31-odbc.md#88d404d471dfbba4)를 실행하여 프로시저에 대한 정보의 결과 집합을 만든다. 각 row는 다음과 같은 column 정보를 갖는다.
...
<a id="3a9abb59c9203187"></a>
##### getTypeInfo( sqlType=None )

[SQLGetTypeInfo](31-odbc.md#3c85b592da97e379) 함수를 실행하여 지정된 데이터 타입 또는 GOLDILOCKS ODBC가 지원하는 모든 데이터 타입에 대한 정보의 결과 집합을 만든다. 각 row는 다음과 같은 column을 갖는다.
...
<a id="1485795b6a30b3c5"></a>
#### 속성

- cursor_description

해당 row를 생성한 cursor 객체의 속성 description의 복사본이다. 자세한 내용은 [Cursor.description](#2f0eb74d2a8ee6dd)을 참조한다.

<a id="b80eac4d5424385e"></a>
## Exception
...
<a id="76d7d0f338babe9b"></a>
## Data Type

<a id="67622261e59acf77"></a>

<a id="bfd3dc5b043dfd7b"></a>

# 6. GOLDILOCKS 데이터베이스의 구조 및 저장 구조

> 원본: [GOLDILOCKS 21c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/21c_1/manual/ko/bfd3dc5b043dfd7b)  
> 태그: `21c.1_35_tag`
...
<a id="b6e12e8609d5eb81"></a>
### 제어 파일 내용

**GOLDILOCKS 데이터베이스 시스템 정보**

<a id="a718cd469ce65d19"></a>
| 항목 | 설명 |
| --- | --- |
...
<a id="dde818095adb1e87"></a>
### 제어 파일 훼손 시 대처 방안

데이터베이스 비정상 종료 등으로 인해 제어 파일이 훼손되었을 경우 다중화된 제어 파일 중 정상 제어 파일을 이용하여 훼손된 제어 파일을 복원한 후에 다시 시작한다.

만약 다중화된 제어 파일들이 모두 훼손되었을 경우, 백업된 제어 파일을 복원하여 archive redo log file과 redo log file까지 불완전 미디어 복구를 수행하여 재시작한다. 백업된 제어 파일을 이용한 불완전 복구는 시나리오별 복구 예제 중에 [다중화된 모든 제어 파일이 훼손된 경우](7-goldilocks-데이터베이스의-백업과-복구.md#08bddeb78aea19a3)를 참조한다.

<a id="39c20adbfd0e18ea"></a>
### 제어 파일 정보
...
GOLDILOCKS의 dump tool인 [gdump](../part-06-utility-manual/41-gdump.md#c780202d1c9738d6)를 이용하면 제어 파일에 기록된 정확한 정보를 확인할 수 있다.

<a id="d44ad8aebdf36488"></a>
## Redo Log File 관리
...
<a id="024b7539e890836a"></a>
![GOLDILOCKS redo log file, 로그 buffer 구조](../assets/images/3bb499f583be1ed3.png)

<a id="b86509813dde88fa"></a>
### Redo Log 그룹과 멤버
...
<a id="63ee12f23b55cfec"></a>
#### 로그 그룹 및 로그 멤버 추가

<a id="f9eed2a9fb297cff"></a>
##### 로그 그룹 추가
...
만약 로그 그룹의 로그 파일들이 모두 훼손되었거나 로그 멤버가 하나밖에 없는 경우, [불완전 복구](7-goldilocks-데이터베이스의-백업과-복구.md#3afe7ebf8a9ce75e)를 수행하여 정상적인 로그 파일까지만 복구하여 재시작할 수 있다.

<a id="0239118bf55f0782"></a>
### Redo Log File 정보
...
<a id="3e6c18e516912bd5"></a>
## Archive Redo Log File 관리

GOLDILOCKS의 redo log file은 circular 로그 그룹을 이용하여 사용된 로그 그룹을 재사용하므로 백업을 이용한 미디어 복구를 위해서는 데이터베이스를 archive 로그 모드로 운용해야 하는데 이 때 기록이 완료된 redo log file은 archive redo log file로 복사된다. GOLDILOCKS 데이터베이스의 archive log mode에 대한 자세한 내용은 [archivelog 모드](7-goldilocks-데이터베이스의-백업과-복구.md#b91c176428c6dee8)를 참조한다.

<a id="3fb6cf2f07e97b3b"></a>
...
<a id="ee35ce6ef43bfa15"></a>
#### Non SYSTEM 테이블스페이스

데이터를 저장하기 위한 테이블과 인덱스를 저장하기 위해 사용자가 임의로 생성하고 삭제할 수 있는 테이블스페이스들이다.

<a id="00f99937218d0006"></a>
### 테이블스페이스 및 데이터 파일 관리

<a id="6a1961cda62dda67"></a>
#### 테이블스페이스 관리

<a id="a52237bbb950c3cd"></a>
##### 테이블스페이스 상태 관리
...
<a id="3b558124ba92e841"></a>
#### 테이블스페이스 정보

데이터베이스에서 생성된 테이블스페이스의 정보를 보려면 [V$TABLESPACE](9-database-information.md#539ad1dc865736db)를 참조한다.
...
<a id="53959da91542ccda"></a>
## 데이터 파일 관리

<a id="abc9859219a506d0"></a>
### 데이터 파일 정합성

데이터 파일의 정합성은 디스크 고장이나 데이터베이스 결함, 사용자의 실수로 인해 깨어질 수 있다. 데이터 파일의 정합성이 깨어진 것을 인식하지 못한 채 데이터베이스를 이용하여 서비스하면 심각한 문제가 발생한다.

이를 위해 GOLDILOCKS 데이터베이스는 데이터 파일의 각 페이지 별 checksum을 이용하여 데이터 파일의 정합성을 보장한다. LSN과 CRC 값을 이용하여 GOLDILOCKS 데이터베이스의 페이지 checksum을 생성하고 각 페이지에 저장한다. 사용자는 [PAGE_CHECKSUM_TYPE](10-server-property.md#f280f24d30552558) 값 설정을 통해 페이지 checksum 종류를 설정하고 기본값으로 LSN을 사용한다.
...
<a id="ba05442509c88f6e"></a>
### 데이터 파일 정보

데이터베이스에서 사용 중인 데이터 파일의 정보를 보려면 [V$DATAFILE](9-database-information.md#c6c26cc666daf137)을 참조한다.
...
<a id="851ffaba98494121"></a>
## 버퍼 캐쉬

<a id="eee44f9625ec8a53"></a>
### GOLDILOCKS 버퍼 캐쉬 구조

디스크 테이블스페이스에 저장된 테이블과 인덱스 페이지에 접근하기 위해 디스크 데이터파일에서 필요한 페이지를 버퍼에 캐싱한다. GOLDILOCKS는 [BUFFER_CACHE_SIZE](10-server-property.md#af23e99f2a820376) 프로퍼티에 설정된 크기만큼 버퍼 캐쉬를 할당하고 [BUFFER_HASH_BUCKETS](10-server-property.md#b3b2d5003f2527d3) 프로퍼티에 설정된 크기만큼의 해쉬 테이블을 이용하여 버퍼 캐쉬에 캐싱된 페이지에서 요청된 페이지를 버퍼 캐쉬에서 검색한다. 페이지에 접근할 때마다 터치 카운트를 증가시키고, 버퍼 캐쉬에 여유공간이 없을 경우, 터치 카운트 값이 작은 페이지를 대체하는 LRU 정책을 사용한다.

<a id="2b41501e4cbdf93f"></a>
![GOLDILOCKS buffer cache 구조](../assets/images/d8718030b1133ee6.png)

<a id="1f802859d024bfb8"></a>
### 버퍼 캐쉬 리스트
...
버퍼 LRU 리스트는 hot/ cold 영역으로 구분되어 cold 영역에 있는 페이지 프레임의 터치 카운트가 정해진 값 ([BUFFER_HOT_REGION_CRITERIA](10-server-property.md#003129e5e4ca7c3f))과 같거나 크면 hot 영역으로 옮겨진다. 버퍼 LRU 리스트의 페이지 프레임들이 접근할 때마다 터치 카운트가 증가되다가 모두 hot 영역에 속하게 되면 대체할 페이지 프레임을 구할 수 없기 때문에 hot 영역의 크기를 [BUFFER_HOT_REGION_PERCENT](10-server-property.md#baff3957de2c84ff) 프로퍼티 값으로 제한한다.

<a id="53feff1bf2f6e615"></a>
### 페이지 프레임 상태

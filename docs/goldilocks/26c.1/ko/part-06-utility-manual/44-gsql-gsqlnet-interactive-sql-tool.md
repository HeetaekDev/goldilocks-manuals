<a id="775292360ce2ccd9"></a>

# 44. gsql/gsqlnet (Interactive SQL Tool)

> 원본: [GOLDILOCKS 26c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/26c_1/manual/ko/775292360ce2ccd9)  
> 태그: `26c.1_0_tag`

[← 43. glsnr](43-glsnr.md) · [전체 목차](../README.md) · [45. gloader/gloadernet (Upload/download Tool) →](45-gloader-gloadernet-upload-download-tool.md)

<a id="268e326ae9999952"></a>
## gsql 개요
...
다음과 같이 [CREATE TABLE](../part-03-sql-manual/19-sql-references-c-g.md#ce6ecbcf1ea593ea) 구문을 수행하여 테이블을 생성하고 트랜잭션을 COMMIT 한다.
...
생성한 테이블에 [INSERT INTO](../part-03-sql-manual/20-sql-references-h-z.md#4b00ba4f4c0eed30) 구문을 수행하여 데이터를 추가하고 트랜잭션을 COMMIT 한다. 두 번째 INSERT 구문은 두 개의 row를 추가하는 예이다.
...
다음은 테이블에 addr column을 추가하고 [UPDATE](../part-03-sql-manual/20-sql-references-h-z.md#d07bac444b3a8009) 구문을 수행하여 데이터를 갱신하는 예이다.
...
다음과 같이 입력된 데이터를 [SELECT](../part-03-sql-manual/20-sql-references-h-z.md#2070458035e417b9) 질의를 이용하여 조회한다. 질의 결과는 column 이름을 표현하는 header와 각 row를 하나의 line에 출력한 정보로 구성되어 있다. NULL 값은 소문자 null로 표현된다.
...
[`\prepare sql`](#b6b45d20d8520418) 명령을 이용해 SQL 구문을 준비하고 호스트 변수의 값을 바꿔가며 [`\exec`](#d6c532ff1c99e2c0) 명령을 이용해 위의 SELECT 구문을 반복 수행한다. 다음과 같이 gsql 대화형 모드에서 ODBC와 JDBC의 prepare/ execute 동작을 simulation 할 수 있다.
...
다음은 embedded SQL, ODBC, JDBC에서 scrollable 커서를 선언하고 gsql을 통해 커서를 이용한 데이터 검색을 simulation 하는 예이다. [DECLARE cursor_name](../part-03-sql-manual/19-sql-references-c-g.md#d0200d8897a107da) 구문을 이용해 선언한 커서 cur1은 스크롤이 가능한 KEYSET 커서로써 [FETCH cursor_name](../part-03-sql-manual/19-sql-references-c-g.md#189c41bd169bad81) 구문을 이용해 다양한 fetch orientation을 사용할 수 있다.
...
<a id="2aec520418604e25"></a>
## gsql 실행

<a id="9ad117764f9e65d5"></a>
### gsql 정보
...
<a id="2e7c33d932af9c3d"></a>
| 옵션 | 설명 |
| --- | --- |
| [--help](#44befb69c3c45b80) | 옵션 목록 |
| [--version](#c4860a64a890e709) | gsql 버전 정보 |

Shell 프롬프트 상에서 수행할 수 있는 gsql의 command option은 다음과 같이 [--help](#44befb69c3c45b80) 옵션을 통해 확인할 수 있다.
...
gsql 프로그램은 GOLDILOCKS 서버와 달리 별도의 version을 가지는데 다음과 같이 [--version](#c4860a64a890e709) 옵션을 사용하여 확인할 수 있다. GOLDILOCKS 서버의 version과 동일한 gsql 프로그램을 사용할 것을 권장한다.
...
참고로 GOLDILOCKS 서버의 version은 다음과 같이 [VERSION](../part-03-sql-manual/17-built-in-function-references.md#75eb00197e439b14) 함수를 통해 확인할 수 있다.
...
<a id="e8495d3735dbd8d6"></a>
| 옵션 | 설명 |
| --- | --- |
| [Username Password](#70e550bb73dc99f0) | 접속 사용자와 패스워드이다. |
| [--as {SYSDBA\|ADMIN}](#c40db372e54be750) | 접속 role을 지정한다. |
| [--conn-string](#be8090b08ac48074) | Connection string을 지정한다. |
| [--dsn](#e6e1883972f06d69) | Data Source Name (DSN)을 지정한다. |

<a id="941e0ca8b6a5f53a"></a>
#### 사용자 접속
...
- [--conn-string](#be8090b08ac48074)
- [--dsn](#e6e1883972f06d69)

<a id="577bb5e5d86c6508"></a>
#### SYSDBA 접속

서버를 구동하거나 database에 대한 모든 권한을 가진 SYSDBA role 등에 접속하려면 [--as {SYSDBA|ADMIN}](#c40db372e54be750) 옵션을 사용해야 한다.
...
서버 구동 및 종료에 대한 자세한 내용은 [서버 구동 및 종료](#1cd0f4e1950f2f7f)를 참조한다.

<a id="187ebf96f4ef0d3d"></a>
#### gsql 종료

gsql은 다음과 같이 `\quit` 또는` \q` 명령을 이용하여 종료한다.   
자세한 내용은 [`\quit`](#d8d224101441a6bd)을 참조한다.
...
<a id="b68bd970363c20a8"></a>
| 옵션 | 설명 |
| --- | --- |
| [--prompt](#5bd7d5046ff98eaa) | 프롬프트를 지정한다. |
| [--no-prompt](#4e89971a2caae913) | 프롬프트를 제거한다. |
| [--import](#861a8b236fe734d2) | 입력으로 SQL 파일을 수행한다. |
| [--silent](#fb6b3462597d147a) | SQL 파일을 실행할 때 명령어와 실행 결과를 출력하지 않는다. |
| [--enable-color](#785c6983115aefd5) | 질의 결과를 row 별로 구분하여 출력한다. |

<a id="7a495b339b21f22c"></a>
#### 프롬프트 제어
...
- [--prompt](#5bd7d5046ff98eaa)
- [--no-prompt](#4e89971a2caae913)

<a id="3379608933ed60f7"></a>
#### 파일로부터 수행
...
- [--import](#861a8b236fe734d2)
- [--silent](#fb6b3462597d147a)
- [--enable-color](#785c6983115aefd5)

<a id="f5ef1e8d65444093"></a>
### gsql 설정 저장

<a id="b9158d08844e96e1"></a>
#### gsql 환경 파일 (gsql.ini)
...
<a id="8a0fdc86c70e9ef9"></a>
| 옵션 | 최소값 | 최대값 | 기본값 |
| --- | --- | --- | --- |
| [AUTOCOMMIT](#c4acc79598200530) | OFF | ON | OFF |
| [AUTOTRACE](#a08c5074419aff63) | OFF | ON/TRACEONLY | OFF |
| [LINESIZE](#0aafbad106cb96b3) | 1 | 10000 | 80 |
| [PAGESIZE](#4e095418cd281bd8) | 1 | 10000 | 20 |
| [VERTICAL](#7716ae4d86e07740) | OFF | ON | OFF |
| [TIME](#1765ef6b96453876) | OFF | ON | OFF |
| [TIMING](#8577f76f19ef19c0) | OFF | ON | OFF |
| [ERROR](#56808516791920e5) | ON | OFF | ON |
| [COLSIZE](#441592ff185b225b) | 1 | 10485760 | 8192 |
| [NUMSIZE](#efaea7c6ca4602a2) | 1 | 50 | 20 |
| [DDLSIZE](#40bc02d751263b98) | 1 | 10485760 | 10000 |
| [HISTORY](#ea8f4db66f5c3aca) | 0 | 100000 | 128 |

> .gsql.ini가 없거나 옵션값이 유효하지 않은 경우에는 이를 무시한다.

<a id="b268a1859950d24d"></a>
#### glogin.sql
...
<a id="2fdfa6d8b5de6b03"></a>
## Interactive Command 사용

<a id="04255a14007e4141"></a>
### gsql 대화형 모드 명령어
...
<a id="be1772b51c6e77e5"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\help`](#bb1b80043eaac0f6) | gsql 명령어 목록 |

gsql 프로그램은 [서버 접속](#dc15494856a39cfe)을 통해 다음과 같은 대화형 모드로 동작한다.
...
다음은 gsql 명령어 [`\exec sql`](#6bc575182160adbb)을 사용하여 동일한 SELECT 구문을 사용하는 예이다. `\exec sql` 명령은 [Embedded SQL](../part-05-developer-manual/36-embedded-sql.md#174fa5eee5426e21)의 SQL 구문 시작을 의미하는 EXEC SQL과 구문이 동일할 뿐 gsql 고유의 명령어이다.
...
[`\set time`](#1765ef6b96453876)을 통해 현재 시간을 프롬프트에 출력하도록 설정할 수 있다.
...
gsql 대화형 모드의 고유 명령어들은 다음과 같이 [`\help`](#bb1b80043eaac0f6) 명령을 이용해 조회할 수 있다.
...
<a id="f7c2e3003a7794c0"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\startup`](#9eec5d5c59b4ab1a) | 서버를 구동한다. |
| [`\shutdown`](#90bad908cd317516) | 서버를 종료한다. |
| [`\cstartup`](#ec7292699e09973e) | Cluster 환경을 위한 서버를 구동한다. |
| [`\cshutdown`](#a5bcb10148ff886a) | Cluster 환경을 위한 서버를 종료한다. |

<a id="9e47445d2b970c1e"></a>
#### Standalone
...
gsql은 GOLDILOCKS 서버가 없는 상태에서 idle instance에 접속하고 [`\startup`](#9eec5d5c59b4ab1a) 명령을 통해 GOLDILOCKS 서버를 특정 단계로 구동하여 서버에 접속한다. 다음은 NOMOUNT 단계로 GOLDILOCKS 서버를 구동하는 예이다.
...
GOLDILOCKS를 최초로 구동할 때만 [`\startup`](#9eec5d5c59b4ab1a) 명령을 사용하며 GOLDILOCKS 서버를 구동하고 서버에 접속한 후에는 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13) 구문을 이용해 GOLDILOCKS 서버의 각 단계로 전환한다.
...
- 서버 구동 단계: [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#1b66f042f1c4f4be)
- 최초 서버 구동 명령: [`\startup`](#9eec5d5c59b4ab1a)
- 서버 단계 변경: [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13)

서버를 종료하려면 다음과 같이 `\shutdown` 명령을 사용한다. 서버 종료에 대한 자세한 내용은 [`\shutdown`](#90bad908cd317516) 명령을 참조한다.
...
다음은 [odbc.ini 파일](../part-05-developer-manual/34-odbc.md#1edb504add88d834) 에 LOCATOR_DSN 속성과 LOCATOR DSN이 정의된 예이다.
...
`\CSTARTUP {LOCAL | GLOBAL} OPEN` 이외의 명령어는 해당 GOLDILOCKS 서버에만 적용된다. 따라서 `\CSTARTUP NOMOUNT, MOUNT` 명령어 이후 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13) 을 이용하여 단계별로 구동하기 위해서는 다른 서버들도 LOCAL OPEN 상태까지 직접 구동해야 한다.
...
- 서버 구동 단계: [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#1b66f042f1c4f4be)
- 최초 서버 구동 명령: [`\cstartup`](#ec7292699e09973e)
- 서버 단계 변경: [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13)

전체 서버를 종료하려면 다음 예와 같이 `\cshutdown` 명령을 사용한다. 전체 서버 종료에 대한 자세한 내용은 [`\cshutdown`](#a5bcb10148ff886a) 명령을 참조한다.
...
<a id="b0da70cd272c870f"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\import`](#593235c21c0d4913) | SQL 파일을 실행한다. |
| [`\set autocommit`](#c4acc79598200530) | SQL 구문을 실행할 때마다 자동으로 COMMIT 한다. |
| [SQL References](../part-03-sql-manual/18-sql-references-a-b.md#7e5e0c9e12e61d15) | SQL 구문의 종류이다. |

<a id="2fa6929892f3b8fa"></a>
#### SQL 구문 입력
...
<a id="8c62a2b3d5fbc277"></a>
#### 파일로부터 SQL 구문 입력

대화형 모드에서 SQL 구문을 직접 입력하지 않고 파일로부터 수행할 경우, `\import` 명령을 사용한다. 다음은 sample.sql 파일을 읽어 다수의 SQL 구문들을 수행하는 예이다. 자세한 내용은 [`\import`](#593235c21c0d4913) 명령을 참조한다.
...
<a id="b3024e54194aaba4"></a>
#### gsql 주석

대화형 모드에서 사용되는 주석은 SQL 구문의 주석과 동일하다. [--import](#861a8b236fe734d2) 옵션을 통해 파일로부터 입력되는 주석도 SQL 구문의 주석과 동일하다. SQL 구문의 주석에 대한 자세한 내용은 [Comments](../part-03-sql-manual/11-sql-elements.md#0c0fe841b325908b)를 참조한다.
...
gsql 대화형 모드에서 사용할 수 있는 주석은 SQL 구문의 주석과 동일하며 주석에 대한 자세한 내용은 SQL 구문의 [Comments](../part-03-sql-manual/11-sql-elements.md#0c0fe841b325908b)를 참조한다.

<a id="8e71a099be3b8cf4"></a>
#### SQL 수행 결과
...
SQL 질의 이외의 [Data Definition Language](../part-03-sql-manual/12-sql-languages.md#61d92470d561a61e), [Data Manipulation Language](../part-03-sql-manual/12-sql-languages.md#51926b73d01bf67d), [Control Language](../part-03-sql-manual/12-sql-languages.md#530b05967cf0f67b) 등의 구문은 각 SQL 구문의 특성에 맞는 수행 결과를 요약한 정보를 출력한다.
...
<a id="29ed8a33fa6d5452"></a>
#### 대화형 명령어의 수행 결과

SQL 구문과 함께 사용하는 대화형 명령어들은 SQL 구문을 수행한 것과 동일한 결과와 에러를 출력한다.

다음은 [`\prepare sql`](#b6b45d20d8520418) 명령을 이용해 SQL 구문을 준비하고 [`\exec`](#d6c532ff1c99e2c0) 명령을 이용해 준비된 SQL 구문을 수행하는 예이다.
...
다음은 [`\prepare sql`](#b6b45d20d8520418) 명령을 사용할 때 SQL 구문에 에러가 존재하는 경우로써 SQL 구문을 수행한 것과 동일한 에러를 출력한다.
...
대화형 명령어의 수행 결과는 [Interactive Command References](#9658aa01a8ba64e3)의 각 명령어 설명과 사용 예를 참조한다.

<a id="baa1cd13d3f8e524"></a>
#### SQL 구문의 자동 COMMIT

gsql의 대화형 모드에서 수행하는 SQL 구문은 [COMMIT](../part-03-sql-manual/19-sql-references-c-g.md#9d9942a1324d8ced) 또는 [ROLLBACK](../part-03-sql-manual/20-sql-references-h-z.md#1ba3b433854d9411) 구문을 통해 트랜잭션을 완료하거나 철회할 수 있다.
...
SQL 구문을 수행할 때마다 자동으로 트랜잭션을 COMMIT 하려면 다음과 같이 [`\set autocommit`](#c4acc79598200530) 명령을 이용하여 제어할 수 있다. 위의 예와 동일한 SQL 구문을 수행한 경우, 각 SQL 구문이 자동으로 COMMIT 되어 두 번째, 세 번째 INSERT 구문의 수행 결과도 ROLLBACK과 관계없이 완료되었음을 알 수 있다.
...
자동 COMMIT 제어에 대한 자세한 내용은 [`\set autocommit`](#c4acc79598200530) 명령어를 참조한다.

<a id="05eb37dd09b05681"></a>
#### 수행 중인 SQL 구문의 강제종료
...
<a id="3f524fb56cc9c9b9"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\set color`](#dd290c169cfac9e1) | 각 row를 다른 색깔로 구별한다. |
| [`\set colsize`](#441592ff185b225b) | Column 결과의 최대 크기를 제어한다. |
| [`\set error`](#56808516791920e5) | 에러 메시지 출력 여부를 제어한다. |
| [`\set linesize`](#0aafbad106cb96b3) | Row의 최대 출력 길이를 제어한다. |
| [`\set numsize`](#efaea7c6ca4602a2) | 숫자값의 최대 digit 개수를 제어한다. |
| [`\set pagesize`](#4e095418cd281bd8) | Page에 포함할 row의 개수를 제어한다. |
| [`\set timing`](#8577f76f19ef19c0) | SQL 구문 수행 시간을 출력한다. |
| [`\set vertical`](#7716ae4d86e07740) | 각 column을 한 line에 출력한다. |

<a id="97dac7c8146fc86d"></a>
#### Page 구성 제어

앞서 [SQL 수행 결과](#8e71a099be3b8cf4)에 기술한대로 질의 결과의 상단부에는 column 이름을 출력하고 데이터를 출력한 후, 하단부에 요약 결과를 출력한다.

질의 결과가 많을 경우 pagesize (기본값 20) 단위로 나누어 각 page마다 column 이름과 데이터를 출력하고 모든 page의 출력이 완료된 후 하단부에 요약 결과를 출력한다.

다음은 pagesize를 5로 변경하여 질의 결과를 출력한 예이다. Pagesize 제어에 대한 자세한 내용은 [`\set pagesize`](#4e095418cd281bd8)명령을 참조한다.
...
다음은 linesize를 200으로 조정하기 전과 후의 질의 결과이다. Linesize 제어에 대한 자세한 내용은 [`\set linesize`](#0aafbad106cb96b3) 명령을 참조한다.
...
터미널 상에서 각 row를 구별하여 가독성을 높이고 싶은 경우 [`\set color`](#dd290c169cfac9e1) 명령을 통해 다음과 같이 row 마다 색깔을 달리하여 출력할 수 있다. 자세한 내용은 [`\set color`](#dd290c169cfac9e1) 명령을 참조한다.
...
질의 결과를 출력할 때 row를 line 단위가 아닌 column 단위로 하나의 line에 출력하려면 `\set vertical on` 명령을 사용한다. 다음 예와 같이 column 단위로 출력 결과가 생성되는데 자세한 내용은 [`\set vertical`](#7716ae4d86e07740) 명령을 참조한다.
...
<a id="15fab4dc7297a042"></a>
#### 데이터 값의 출력 제어

gsql은 질의 결과에 포함되는 모든 데이터를 문자열로 변환하여 출력한다.

숫자형은 numsize (기본값 20)를 digit 단위로 출력하며 numsize 보다 큰 숫자값은 exponent 형태로 표현한다. 다음은 numsize를 이용해 출력값을 제어하는 예이며 자세한 내용은 [`\set numsize`](#efaea7c6ca4602a2) 명령을 참조한다.
...
숫자값 출력에 대한 다양한 제어가 필요한 경우, 다음과 같이 [TO_CHAR( number )](../part-03-sql-manual/17-built-in-function-references.md#69fde4657a756aff) 함수를 이용해 출력을 제어할 수 있다.
...
- [NLS_DATE_FORMAT](../part-02-administration-manual/10-server-property.md#09118aefc816172d)
- [NLS_TIMESTAMP_FORMAT](../part-02-administration-manual/10-server-property.md#b83b0daa42003bc4)
- [NLS_TIMESTAMP_WITH_TIME_ZONE_FORMAT](../part-02-administration-manual/10-server-property.md#8cf319a54a7fec34)
- [NLS_TIME_FORMAT](../part-02-administration-manual/10-server-property.md#d78680ceba6107a9)
- [NLS_TIME_WITH_TIME_ZONE_FORMAT](../part-02-administration-manual/10-server-property.md#d8ed241efdbfc8ee)
...
프로퍼티를 변경하지 않고 날짜/ 시간 출력값을 제어하려면 다음과 같이 [TO_CHAR( datetime )](../part-03-sql-manual/17-built-in-function-references.md#49e380b1da61d32b) 함수를 사용한다.
...
LONG VARCHAR, LONG VARBINARY 타입의 데이터는 아주 긴 문자열을 포함할 수 있어 colsize (기본값 8192) 만큼만 데이터를 출력한다. 다음은 colsize를 줄여 LONG VARCHAR 타입인 TEXT column의 일부 데이터를 출력한 예인데 colsize 값을 늘려 데이터를 모두 출력할 수 있다. 자세한 내용은 [`\set colsize`](#441592ff185b225b)  명령을 참조한다.
...
<a id="c47b627fba18d071"></a>
#### SQL 구문 수행시간

SQL 구문을 수행할 때 수행 시간을 출력하려면 다음과 같이 `\set timing on` 명령을 사용한다. 자세한 내용은 [`\set timing`](#8577f76f19ef19c0) 명령을 참조한다.
...
<a id="6315976affc2f593"></a>
| gsql 명령 | 설명 |
| --- | --- |
| `\spool` | 출력 결과를 file에 저장한다. |

출력 결과 저장과 관련된 기능들은 [`\spool`](#78d64fbff9f6ca0d)의 명령어들을 참조한다.
...
<a id="c6a503f958db5801"></a>
### SQL 객체 정보 조회

SQL 객체에 대한 정보는 [DICTIONARY_SCHEMA](../part-02-administration-manual/9-database-information.md#72c0cc66453855b4), [INFORMATION_SCHEMA](../part-02-administration-manual/9-database-information.md#83a1c3ba584ee938)의 각종 view들로 조회할 수 있다.

**참조 명령어**

<a id="47b2c662c6005e12"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\desc`](#224062403e0a51c9) | 테이블 정보를 조회한다. |
| [`\idesc`](#21cb66ede83065c4) | 인덱스 정보를 조회한다. |
...
gsql 은 다음과 같이 `\desc` 명령어를 이용해 테이블과 관련된 정보를 쉽게 조회할 수 있다. 출력 결과 및 자세한 내용은 [`\desc`](#224062403e0a51c9) 명령을 참조한다.
...
명령어 `\idesc`는 다음 예와 같이 인덱스와 관련된 정보를 보여준다. 자세한 내용은 [`\idesc`](#21cb66ede83065c4)명령을 참조한다.
...
<a id="8b1b8145e5643eff"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\ddl_cluster`](#7c6a0753e6498297) | 클러스터와 관련된 DDL을 출력한다. |
| [`\ddl_db`](#827a37294723a5c4) | 데이터베이스와 관련된 DDL을 출력한다. |
| [`\ddl_tablespace`](#45762104ffbe6d7f) | 테이블스페이스와 관련된 DDL을 출력한다. |
| [`\ddl_profile`](#a6b1bd36dd7d15f3) | Profile과 관련된 DDL을 출력한다. |
| [`\ddl_audit_policy`](#03fe02faad2b2db7) | Audit policy와 관련된 DDL을 출력한다. |
| [`\ddl_user`](#150860f08466e92d) | 계정과 관련된 DDL을 출력한다. |
| [`\ddl_role`](#30ba33e56e23343e) | Role과 관련된 DDL을 출력한다. |
| [`\ddl_schema`](#ca99c8979e5b645b) | 스키마와 관련된 DDL을 출력한다. |
| [`\ddl_public_synonym`](#e6cbc10575e98125) | Public synonym과 관련된 DDL을 출력한다. |
| [`\ddl_table`](#d0aa23d7ba9eb24f) | 테이블과 관련된 DDL을 출력한다. |
| [`\ddl_constraint`](#d4693b108e5a700d) | 제약 조건과 관련된 DDL을 출력한다. |
| [`\ddl_index`](#003e2263f989eeaf) | 인덱스와 관련된 DDL을 출력한다. |
| [`\ddl_view`](#1b40b4ba908c5405) | View와 관련된 DDL을 출력한다. |
| [`\ddl_sequence`](#535ae310e8b9868d) | 시퀀스와 관련된 DDL을 출력한다. |
| [`\ddl_synonym`](#e9299e301c836f51) | Synonym과 관련된 DDL을 출력한다. |
| [`<code>\ddl_library</code>`](#107f7fa3d4eeca93) | Library와 관련된 DDL을 출력한다. |
| [`\ddl_procedure`](#991cc99c8760ed8e) | Procedure와 관련된 DDL을 출력한다. |
| [`\ddl_package`](#ea0fe986b3fe0845) | Package과 관련된 DDL을 출력한다. |
| [`\ddl_trigger`](#99c01d5516325b59) | Trigger와 관련된 DDL을 출력한다. |
| [`\set ddlsize`](#40bc02d751263b98) | DDL 출력 버퍼의 크기를 제어한다. |
...
<a id="af8c9b81bf8ac42f"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\\`](#cd404cd8f381035e) | 이전 SQL을 수행한다. |
| [`\{n}`](#c2358dd22431286d) | SQL 이력 번호에 해당하는 구문을 실행한다. |
| [`\history`](#1663b40a11a4fbca) | SQL 수행 이력을 조회한다. |
| [`\set history`](#ea8f4db66f5c3aca) | History 버퍼 개수를 제어한다. |
...
gsql의 history 관련 기능들은 [참조 명령어 ](#af8c9b81bf8ac42f)의 명령어들을 참조한다.

<a id="5cf80001c1bc7832"></a>
### SQL 편집
...
gsql 편집 관련 기능에 대한 자세한 내용은 [`\edit`](#1ba24e3dc5e10478)의 명령어들을 참조한다.
...
<a id="caf62104457f409c"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\connect`](#949cabf85f6107bd) | 새로운 사용자로 접속한다. |
| [`\quit`](#d8d224101441a6bd) | 접속을 종료한다. |

대화형 모드에서 `\connect` 명령을 이용해 다음과 같이 새로운 사용자로 접속할 수 있다. `\connect ` 명령은 기존의 session을 종료하고 새로운 session을 생성한다. 자세한 내용은 [`\connect`](#949cabf85f6107bd) 명령을 참조한다.
...
다음과 같이 [SET SESSION AUTHORIZATION user_identifier](../part-03-sql-manual/20-sql-references-h-z.md#784d7fcc74c83d43) 구문을 이용해 사용자를 변경할 수 있다. `\connect` 명령이 수행 중인 트랜잭션을 모두 COMMIT 하고 새로운 session을 생성하는 반면에 SET SESSION AUTHORIZATION 구문은 기존의 session을 그대로 유지한 채 사용자를 변경한다.
...
대화형 모드를 종료하려면 다음과 같이 `\quit` 명령을 사용한다. 자세한 내용은 [`\quit`](#d8d224101441a6bd) 명령을 참조한다.
...
<a id="fa0f645c3c3d28cf"></a>
| gsql 명령 | 설명 |
| --- | --- |
|  [`\var`](#f4b746e481fc748e) | 호스트 변수를 선언한다. |
|  [`\exec :var := value`](#90a8efde9c6fcbff) | 호스트 변수에 값을 할당한다. |
|  [`\print`](#4eefbe4e20e65218) | 호스트 변수의 값을 출력한다. |
|  [`\dynamic sql :var`](#ea53a6d3061862f3) | 호스트 변수에 저장된 SQL 문장을 수행한다. |

다음은 [`\var`](#f4b746e481fc748e) 명령을 이용하여 호스트 변수를 선언하고 [`\exec :var := value`](#90a8efde9c6fcbff) 명령을 이용해 호스트 변수에 값을 대입하고 [`\print`](#4eefbe4e20e65218) 명령을 이용해 호스트 변수의 값을 조회하는 예이다. 자세한 내용은 각 명령을 참조한다.
...
<a id="7c16c34f3ffe2674"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\exec sql`](#6bc575182160adbb) | SQL 문장을 바로 실행한다. |
| [`\prepare sql`](#b6b45d20d8520418) | SQL 문장을 준비한다. |
| [`\exec`](#d6c532ff1c99e2c0) | 준비된 SQL 문장을 수행한다. |
| [`\dynamic sql :var`](#ea53a6d3061862f3) | 호스트 변수에 저장된 SQL 문장을 수행한다. |
...
다음과 같이 [`\prepare sql`](#b6b45d20d8520418) 명령과 [`\exec`](#d6c532ff1c99e2c0) 명령을 이용하여 위에서 Java 코드의 PreparedStatement class를 이용하여 구현한 것을 simulation 할 수 있다.
...
> 
> 
> - [SELECT .. INTO](../part-03-sql-manual/20-sql-references-h-z.md#410ad949d2455fb3)
> - [DECLARE cursor_name](../part-03-sql-manual/19-sql-references-c-g.md#d0200d8897a107da)
> - [OPEN cursor_name](../part-03-sql-manual/20-sql-references-h-z.md#674c9a50006ee689)
> - [FETCH cursor_name](../part-03-sql-manual/19-sql-references-c-g.md#189c41bd169bad81)
> - [CLOSE cursor_name](../part-03-sql-manual/19-sql-references-c-g.md#98a34ae5c82860ff)
> 

<a id="26510c24c4bbbaa5"></a>
### SQL 실행 계획 정보
...
<a id="bffccfdbb1987a2d"></a>
| gsql 명령 | 설명 |
| --- | --- |
| [`\explain plan`](#ad4a7aa57a521888) | SQL 문장의 실행 계획을 출력한다. |
| [`\set autotrace`](#a08c5074419aff63) | 실행 계획 출력 여부를 설정한다. |

대화형 모드에서 다음과 같이 `\explain plan` 명령이나 `\set autotrace` 명령을 이용하여 SQL 구문의 실행 계획을 조회할 수 있다. 명령에 대한 자세한 내용은 [`\explain plan`](#ad4a7aa57a521888), [`\set autotrace`](#a08c5074419aff63)를 참조하고 실행 계획에 대한 해석은 [SQL 실행 계획](../part-03-sql-manual/15-sql-tuning.md#d840f5576fcd85fa)을 참조한다.
...
<a id="c40db372e54be750"></a>
### --as role_name

<a id="9c2de827d1a7eb94"></a>
#### 설명

role_name으로 GOLDILOCKS에 접속한다.

SYSDBA role이나 ADMIN role로 GOLDILOCKS에 접속하여 서버를 구동하거나 종료할 수 있다.  
해당 role에 대한 자세한 내용은 [서버 구동 및 종료](#1cd0f4e1950f2f7f)를 참조한다.

```
% gsql sys gliese --as sysdba
```

<a id="28ebc4a7dbf9472c"></a>
#### 사용 예
...
<a id="be8090b08ac48074"></a>
### --conn-string

<a id="63d52603b2305b2a"></a>
#### 설명

Connection string을 통해 GOLDILOCKS에 접속한다.  
기술할 connection string의 내용은 ODBC 함수 [SQLDriverConnect](../part-05-developer-manual/34-odbc.md#a1dbe82655dd4df4)의 input string과 동일한 형태로 사용해야 한다. Connection string에 대한 자세한 내용은 [SQLDriverConnect](../part-05-developer-manual/34-odbc.md#a1dbe82655dd4df4) 함수를 참조한다.

<a id="2e642c5b1f16193e"></a>
#### 사용 예
...
<a id="e6e1883972f06d69"></a>
### --dsn

<a id="b34d84dfa1d9ec05"></a>
#### 설명

odbc.ini 파일에 정의된 Data Source Name (DSN)을 이용하여 접속한다.  
DSN 이름은 odbc.ini 파일에 정의된 이름이어야 한다. odbc.ini 파일에 대한 자세한 내용은 [UNIX에서 DSN 구성](../part-05-developer-manual/34-odbc.md#f03b93616bd53082) 을 참조한다.  
DSN이 생략된 경우 기본값은 GOLDILOCKS이다.

<a id="7664b34751162369"></a>
#### 사용 예
...
<a id="785c6983115aefd5"></a>
### --enable-color

<a id="2706ece459e801a2"></a>
#### 설명

터미널 상에서 질의 결과의 각 row를 다른 색깔로 출력하여 쉽게 구분할 수 있도록 한다.  
대화형 모드로 질의를 수행할 경우 질의 결과를 row 마다 다른 색깔로 출력하며 이는 [--import](#861a8b236fe734d2) 옵션을 이용해 파일에 포함된 SELECT 구문의 결과에도 적용된다.

<a id="38e06c63f558badd"></a>
#### 사용 예
...
<a id="44befb69c3c45b80"></a>
### --help

<a id="10bd03c5823e6fac"></a>
#### 설명

gsql 프로그램의 옵션 목록을 간략히 보여준다.

<a id="2063551bdf839c5a"></a>
#### 사용 예
...
<a id="4e89971a2caae913"></a>
### --no-prompt

<a id="0c3863286104d7e4"></a>
#### 설명

대화형 모드를 수행할 때 프롬프트를 출력하지 않는다.

<a id="a2fca7ad357b9f41"></a>
#### 사용 예
...
<a id="5bd7d5046ff98eaa"></a>
### --prompt

<a id="1b24ccc62ac7e716"></a>
#### 설명
...
<a id="fb6b3462597d147a"></a>
### --silent

<a id="e0fbe0e9aba008d3"></a>
#### 설명

SQL 구문의 수행 결과를 출력하지 않는다.  
[--import](#861a8b236fe734d2) 명령을 이용해 파일을 읽어 대량의 SQL 구문을 수행할 경우에 유용하다.  
--silent 옵션은 대화형 모드로 동작할 때도 적용된다.

<a id="a6f50bc4740f1087"></a>
#### 사용 예
...
<a id="c4860a64a890e709"></a>
### --version

<a id="b87183b61c0fcb09"></a>
#### 설명

gsql 프로그램의 version 정보를 출력한다.  
GOLDILOCKS와 동일한 version의 gsql 프로그램을 사용할 것을 권장한다. GOLDILOCKS의 version은 다음과 같이 SQL 함수 [VERSION](../part-03-sql-manual/17-built-in-function-references.md#75eb00197e439b14)을 통해 확인할 수 있다.
...
<a id="d658a462b24c51ff"></a>
#### 설명

가장 최근에 성공한 SQL 구문을 실행한다.

<a id="8d77b26d7e9bc985"></a>
#### 사용 예
...
<a id="949cabf85f6107bd"></a>
### `\connect`

<a id="c826d896f3a1b4ab"></a>
#### 구문
...
<a id="ec7292699e09973e"></a>
### `\cstartup`

<a id="df6e4aab0f7f349a"></a>
#### 구문
...
<a id="5a2a2399657a6051"></a>
#### 설명

Cluster 환경에서 GOLDILOCKS 서버를 구동한다.

`\cstartup` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#1cd0f4e1950f2f7f)를 참조한다.
...
`\cstartup local open` 명령어와 `\cstartup global open` 명령어는 해당 서버와 다른 서버들도 해당 단계까지 구동 시킨다. 이 외의 명령어는 해당 서버에만 적용되므로 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13)를 이용하여 단계별로 구동하려면 직접 서버를 LOCAL OPEN 단계까지 구동해야 한다.

서버의 구동 단계는 NOMOUNT, MOUNT, LOCAL OPEN, GLOBAL OPEN 단계로 구분되며 자세한 내용은 [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#1b66f042f1c4f4be)을 참조한다.

`\cstartup` 명령을 수행한 후에 다음 단계로 넘어가기 위해서는 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13) 구문을 수행해야 한다.

glocator를 이용한 `\cstartup`은 [CSTARTUP과 CSHUTDOWN](51-glocator.md#3cc1f447dd5bc362)을 통해 확인할 수 있다.


> 
> - CS 환경에서만 사용하는 명령어로써 gsqlnet에서만 사용할 수 있다.
> - `\cstartup nomount/mount` 이외의 명령은 해당 서버와 함께 다른 서버에까지 영향을 미치는 명령어이다. 따라서 [odbc.ini 파일](../part-05-developer-manual/34-odbc.md#1edb504add88d834)에 LOCATOR_DSN 속성이 기술되지 않았을 경우, 에러가 반환된다.
> 

<a id="4595d097e91649f6"></a>
#### 사용 예
...
<a id="a5bcb10148ff886a"></a>
### `\cshutdown`

<a id="196de751a26811fa"></a>
#### 구문
...
<a id="f7fd1bea916bc0cf"></a>
#### 설명

Cluster 환경에서 GOLDILOCKS 서버를 종료한다.

`\cshutdown` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#1cd0f4e1950f2f7f)를 참조한다.
...
glocator를 이용한 `\cshutdown`은 [CSTARTUP과 CSHUTDOWN](51-glocator.md#3cc1f447dd5bc362)을 통해 확인할 수 있다.

> • CS 환경에서만 사용하는 명령어로써 gsqlnet에서만 사용할 수 있다.  
> • `\cshutdown` 명령은 해당 서버와 함께 다른 서버에까지 영향을 미치는 명령어이다. 따라서 [odbc.ini 파일](../part-05-developer-manual/34-odbc.md#1edb504add88d834) LOCATOR_DSN 속성이 기술되지 않았을 경우, 에러가 반환된다.

<a id="bdf0c3013e80d971"></a>
#### 사용 예
...
<a id="7c6a0753e6498297"></a>
### `\ddl_cluster`

<a id="9462884d12f8b68f"></a>
#### 구문
...
<a id="827a37294723a5c4"></a>
### `\ddl_db`

<a id="bca7974b21cb2dd5"></a>
#### 구문
...
위의 dictionary 정보를 포함하는 schema에 대한 DDL 구문은 [`\ddl_schema`](#ca99c8979e5b645b) 명령을 이용해 출력할 수 있다.

<a id="3cac02c97deb7044"></a>
#### 사용 예
...
<a id="45762104ffbe6d7f"></a>
### `\ddl_tablespace`

<a id="2976888c456a5d53"></a>
#### 구문
...
<a id="a6b1bd36dd7d15f3"></a>
### `\ddl_profile`

<a id="e2bbd35750b6b915"></a>
#### 구문
...
<a id="03fe02faad2b2db7"></a>
### `\ddl_audit_policy`

<a id="d004a0439f224620"></a>
#### 구문
...
<a id="150860f08466e92d"></a>
### `\ddl_user`

<a id="af84e3df4c43bab3"></a>
#### 구문
...
<a id="d460595c993eb52f"></a>
#### 설명

사용자 객체의 현재 상태에 해당하는 DDL 구문을 출력한다.

> 휴지통에 보관된 객체들은 출력되지 않는다.

**`\ddl_user` 명령**

<a id="4a664165ca179608"></a>
| 명령어 | 설명 |
| --- | --- |
| `\ddl_user name` | 아래의 모든 옵션을 수행한다. |
| `\ddl_user name CREATE` | 사용자 객체의 CREATE USER 구문을 출력한다. |
| `\ddl_user name SCHEMA` | 사용자가 소유한 schema들의 CREATE SCHEMA 구문을 출력한다. |
| `\ddl_user name SCHEMA PATH` | 사용자의 schema path에 해당하는 ALTER USER 구문을 출력한다. |
| `\ddl_user name SEQUENCE` | 사용자가 소유한 sequence들의 CREATE SEQUENCE 구문을 출력한다. |
| `\ddl_user name TABLE` | 사용자가 소유한 table들의 CREATE TABLE 구문을 출력한다. |
| `\ddl_user name CONSTRAINT` | 사용자가 소유한 constraint들의 ALTER TABLE 구문을 출력한다. |
| `\ddl_user name INDEX` | 사용자가 소유한 index들의 CREATE INDEX 구문을 출력한다. |
| `\ddl_user name VIEW` | 사용자가 소유한 view들의 CREATE VIEW 구문을 출력한다. |
| `\ddl_user name SYNONYM` | 사용자가 소유한 synonym들의 CREATE SYNONYM 구문을 출력한다. |
| `\ddl_user name LIBRARY` | 사용자가 소유한 library들의 CREATE LIBRARY 구문을 출력한다. |
| `\ddl_user name PROCEDURE` | 사용자가 소유한 procedure와 function들의 CREATE PROCEDURE/FUNCTION 구문을 출력한다. |
| `\ddl_user name PACKAGE` | 사용자가 소유한 package들의 CREATE PACKAGE/PACKAGE BODY 구문을 출력한다. |
| `\ddl_user name TRIGGER` | 사용자가 소유한 trigger들의 CREATE TRIGGER 구문을 출력한다. |
| `\ddl_user name COMMENT` | 사용자의 주석에 해당하는 COMMENT ON AUTHORIZATION 구문을 출력한다. |

<a id="7acc72329f4cb85c"></a>
#### 사용 예

다음은 `\ddl_user CREATE` 명령을 수행하는 예이다.
...
다음은 `\ddl_user SCHEMA` 명령을 수행하는 예이다.
...
다음은 `\ddl_user TABLE` 명령을 수행하는 예이다.
...
<a id="30ba33e56e23343e"></a>
### `\ddl_role`

<a id="11d84814a7dfb334"></a>
#### 구문
...
<a id="e37f7218d1593bda"></a>
#### 설명

Role 객체의 현재 상태에 해당하는 DDL 구문을 출력한다.

**`\ddl_role` 명령**

<a id="b230ad94facd7cb0"></a>
| 명령어 | 설명 |
| --- | --- |
| `\ddl_role name` | 아래의 모든 옵션을 수행한다. |
| `\ddl_role name CREATE` | Role 객체의 CREATE ROLE 구문을 출력한다. |
| `\ddl_role name GRANT` | Role을 부여한 GRANT role 구문을 출력한다. |
| `\ddl_role name COMMENT` | Role의 주석에 해당하는 COMMENT ON AUTHORIZATION 구문을 출력한다. |

<a id="a513a35b7afa8f58"></a>
#### 사용 예

다음은 `\ddl_role name` 명령을 수행하는 예이다.
...
<a id="ca99c8979e5b645b"></a>
### `\ddl_schema`

<a id="758855a6afeeb2d9"></a>
#### 구문
...
| `\ddl_schema name LIBRARY` | 스키마에 속한 library들의 CREATE LIBRARY 구문을 출력한다. |
...
| `\ddl_schema name TRIGGER` | 스키마에 속한 tirgger들의 CREATE TRIGGER 구문을 출력한다. |
...
<a id="e6cbc10575e98125"></a>
### `\ddl_public_synonym`

<a id="d57b0d29cf25fe39"></a>
#### 구문
...
<a id="107f7fa3d4eeca93"></a>
### `\ddl_library`

<a id="bb0cfcd88dd04d41"></a>
#### 구문
...
<a id="a50ad620a0c0cfbd"></a>
#### 설명

라이브러리 객체의 현재 상태에 해당하는 DDL 구문을 출력한다.

**`\ddl_library` 명령**

<a id="484186dbfdc4867d"></a>
| 명령어 | 설명 |
| --- | --- |
| `\ddl_library name` | 아래의 모든 옵션을 수행한다. |
| `\ddl_library name CREATE` | 라이브러리 객체의 CREATE LIBRARY 구문을 출력한다. |
| `\ddl_library name GRANT` | 라이브러리의 권한 정보에 대한 GRANT .. ON LIBRARY 구문을 출력한다. |
| `\ddl_library name COMMENT` | 라이브러리의 주석에 해당하는 COMMENT ON LIBRARY 구문을 출력한다. |

<a id="1d14c4186b1f120c"></a>
#### 사용 예

다음은 `\ddl_library CREATE` 명령을 수행하는 예이다.
...
<a id="d0aa23d7ba9eb24f"></a>
### `\ddl_table`

<a id="407c081f4be493f9"></a>
#### 구문
...
| `\ddl_table name TRIGGER` | 테이블에 생성된 trigger들의 CREATE TRIGGER 구문을 출력한다. |
...
<a id="d4693b108e5a700d"></a>
### `\ddl_constraint`

<a id="b44adf6f1487f249"></a>
#### 구문
...
<a id="003e2263f989eeaf"></a>
### `\ddl_index`

<a id="08bf1700757bd1b7"></a>
#### 구문
...
<a id="1b40b4ba908c5405"></a>
### `\ddl_view`

<a id="0f4508000ee328f9"></a>
#### 구문
...
<a id="535ae310e8b9868d"></a>
### `\ddl_sequence`

<a id="856890ea4d3a7bfb"></a>
#### 구문
...
<a id="e9299e301c836f51"></a>
### `\ddl_synonym`

<a id="3d60ff62d71eede9"></a>
#### 구문
...
<a id="991cc99c8760ed8e"></a>
### `\ddl_procedure`

<a id="67066120d967443d"></a>
#### 구문
...
<a id="f52737696f2a1d65"></a>
| 명령어 | 설명 |
| --- | --- |
| `\ddl_procedure` | 아래의 모든 옵션을 수행한다. |
| `\ddl_procedure name CREATE` | Stored procedure/ function 객체의 CREATE PROCEDURE/FUNCTION 구문을 출력한다. |
| `\ddl_procedure name GRANT` | Stored procedure/ function에 대한 GRANT .. ON PROCEDURE 구문을 출력한다. |
| `\ddl_procedure name COMMENT` | Stored procedure/ function에 해당하는 COMMENT ON PROCEDURE 구문을 출력한다. |

<a id="54019d19c07ccaf5"></a>
#### 사용 예
...
<a id="ea0fe986b3fe0845"></a>
### `\ddl_package`

<a id="7c05aa5266a6152d"></a>
#### 구문
...
<a id="f25a8d9f478355b1"></a>
| 명령어 | 설명 |
| --- | --- |
| `\ddl_package` | 아래의 모든 옵션을 수행한다. |
| `\ddl_package name CREATE` | Package spec 및 package body 객체의 CREATE PACKAGE/PACKAGE BODY 구문을 출력한다. |
| `\ddl_package name GRANT` | Package spec의 권한 정보에 대한 GRANT .. ON PACKAGE 구문을 출력한다. |
| `\ddl_package name COMMENT` | Package spec에 해당하는 COMMENT ON PACKAGE 구문을 출력한다. |

<a id="f3e7cbbf13a082e4"></a>
#### 사용 예
...
<a id="99c01d5516325b59"></a>
### `\ddl_trigger`

<a id="de69fd07d4283fcb"></a>
#### 구문
...
<a id="38d684e71e316371"></a>
#### 설명

Trigger 의 현재 상태에 해당하는 DDL 구문을 출력한다.

> 휴지통에 보관된 객체들은 출력되지 않는다.

**`\ddl_trigger` 명령**

<a id="3f1927641c10ec48"></a>
| 명령어 | 설명 |
| --- | --- |
| `\ddl_trigger` | 아래의 모든 옵션을 수행한다. |
| `\ddl_trigger name CREATE` | Trigger 객체의 CREATE TRIGGER 구문을 출력한다. |
| `\ddl_trigger name COMMENT` | Trigger 의 주석에 해당하는 COMMENT ON TRIGGER 구문을 출력한다. |

<a id="49f8b5d8b684a2d2"></a>
#### 사용 예

다음은 \ddl_trigger CREATE 명령을 수행하는 예이다.
...
<a id="224062403e0a51c9"></a>
### `\desc`

<a id="8b8f61151e59cade"></a>
#### 구문
...
<a id="232d3eb4041b8c9c"></a>
#### 설명

테이블의 구조 정보를 조회한다.

테이블은 다음과 같이 테이블 이름만 기술하거나 테이블 이름을 스키마 이름과 함께 기술할 수 있다. 스키마 이름을 명시하지 않은 경우 테이블의 스키마 이름은 사용자의 [Schema Path](../part-03-sql-manual/13-sql-objects.md#3d5355367b16fe83)에 의해 결정된다.

- `\desc` t1
- `\desc` public.t1

테이블을 생성할 때 다음과 같이 [delimited identifier](../part-03-sql-manual/11-sql-elements.md#a6538337c848447e)를 이용해 테이블 이름을 생성한 경우, double-quote (")를 사용해 기술한다.
...
<a id="ea53a6d3061862f3"></a>
### `\dynamic sql :var`

<a id="c5b8665d2396f00b"></a>
#### 구문
...
<a id="cd37814a52e010ef"></a>
#### 설명

호스트 변수 var에 저장된 SQL 문장을 수행한다.  
Embedded SQL의 정의되지 않은 SQL 문장을 수행하는 [Embedded Dynamic SQL](../part-05-developer-manual/36-embedded-sql.md#70d95d21f1b0a78f)과 유사한 개념으로써 정의되지 않은 SQL 문장을 수행할 때 사용한다.

다음과 같은 절차로 수행한다.

1. 호스트 변수를 선언한다. ([`\var`](#f4b746e481fc748e)를 참조한다.)  
   `\var` var_stmt VARCHAR(1024)
2. 호스트 변수의 값으로 SQL 문장을 대입한다. ([`\exec :var := value`](#90a8efde9c6fcbff)를 참조한다.)  
   `\exec` :var_stmt := 'SELECT # FROM t1'
3. Dynamic SQL을 수행한다.  
   `\dynamic sql` :var_stmt

<a id="0d97ddcdb9348643"></a>
#### 사용 예
...
다음 INSERT 문장과 같이 SQL 문장에 문자열이 존재할 경우 명령어 [`\exec :var := value`](#90a8efde9c6fcbff)를 사용할 때와 같이 single-quote (')를 두 번 기술한다.
...
<a id="1ba24e3dc5e10478"></a>
### `\edit`

<a id="677422cb10654af9"></a>
#### 설명
...
<a id="66da3b14b440ecbd"></a>
#### `\edit`

<a id="a1cb7433577b8090"></a>
##### 구문
...
<a id="d58a18b86936c490"></a>
#### `\edit 'file_name'`

<a id="8b225de9c6adcc96"></a>
##### 구문
...
<a id="df3fd21cc55067af"></a>
#### `\edit [history] {n}`

<a id="d278cb38b4b4d3a0"></a>
##### 구문
...
<a id="b8fccb51c2aa1967"></a>
##### 설명

[`\history`](#1663b40a11a4fbca) 명령으로 조회할 수 있는 SQL 수행 이력 중에서 number에 해당하는 구문을 편집한다.  
number 값은 [`\history`](#1663b40a11a4fbca) 수행 결과의 ID 값이어야 한다.

<a id="dc80cec9b19732a3"></a>
##### 사용 예
...
<a id="d6c532ff1c99e2c0"></a>
### `\exec`

<a id="8db7f5ee017c4af0"></a>
#### 구문
...
<a id="755b5e097e9d6186"></a>
#### 설명

[`\prepare sql`](#b6b45d20d8520418) 명령에 의해 준비된 SQL 구문을 수행한다.  
`\exec` 명령은 ODBC의 [SQLExecute](../part-05-developer-manual/34-odbc.md#dd8e6f5e5058c6a2) 함수와 JDBC의 PreparedStatement::[execute](../part-05-developer-manual/35-jdbc.md#f9fd32f5a8afe021) 함수와 유사하게 동작한다.  
준비된 SQL 구문은 `\exec` 명령을 통해 반복적으로 수행할 수 있다.

<a id="b08fbdef79a1c44e"></a>
#### 사용 예
...
<a id="90a8efde9c6fcbff"></a>
### `\exec :var := value`

<a id="0851033e311aefc7"></a>
#### 구문
...
<a id="f8c1538bc662e092"></a>
#### 설명

호스트 변수에 값을 대입한다.  
해당 명령은 다음과 같은 [SELECT .. INTO](../part-03-sql-manual/20-sql-references-h-z.md#410ad949d2455fb3) 구문을 수행한 것과 동일한 방식으로 호스트 변수에 값을 대입한다.
...
호스트 변수는 [`\var`](#f4b746e481fc748e) 명령을 통해 선언되어 있어야 한다.

호스트 변수는 colon (:)과 함께 사용하여야 하며, 대입 연산자 (:=)에서 (:)을 생략하면 안된다. 호스트 변수에 입력되는 &lt;value expression&gt;에는 단순한 값이나 연산을 사용할 수 있다. 자세한 내용은 [Expressions](../part-03-sql-manual/11-sql-elements.md#26c6918438c54a30)를 참조한다.

호스트 변수에 입력되는 값은 호스트 변수의 데이터 타입에 입력 가능한 값이어야 한다. 자세한 내용은 [타입간 변환](../part-03-sql-manual/11-sql-elements.md#d9c1b23c9cbab71c)을 참조한다.

입력된 호스트 변수의 값은 [`\print`](#4eefbe4e20e65218) 명령을 이용해 조회할 수 있다.
...
다음은 [SELECT .. INTO](../part-03-sql-manual/20-sql-references-h-z.md#410ad949d2455fb3) 구문을 통해 호스트 변수에 값을 얻어오는 예이다.
...
<a id="6bc575182160adbb"></a>
### `\exec sql`

<a id="5fe71c0ee3f278bb"></a>
#### 구문
...
SQL 구문을 수행할 때 [`\prepare sql`](#b6b45d20d8520418) 명령과 [`\exec`](#d6c532ff1c99e2c0) 명령을 이용하여 PREPARE/EXECUTE 방식으로 실행하는 것과 달리, `\exec sql` 명령은 ODBC의 [SQLExecDirect](../part-05-developer-manual/34-odbc.md#c630bcae3c3e259b) 함수와 JDBC의 Statement::[execute](../part-05-developer-manual/35-jdbc.md#19c89209ce4eb78f) 함수에 대응하는 DIRECT EXECUTE 방식으로 SQL 구문을 수행한다.

<a id="77a7e318a2d547db"></a>
#### 사용 예
...
<a id="ad4a7aa57a521888"></a>
### `\explain plan`

<a id="d0e032aefa4bd794"></a>
#### 구문
...
실행 계획에 대한 자세한 설명은 [SQL 실행 계획](../part-03-sql-manual/15-sql-tuning.md#d840f5576fcd85fa)을 참조한다.

<a id="45adb1de5be3a734"></a>
#### 사용 예
...
<a id="bb1b80043eaac0f6"></a>
### `\help`

<a id="b5162f00292728a1"></a>
#### 구문
...
<a id="3e13a5d77090f135"></a>
#### 설명

gsql 대화형 모드에서 `(\)`로 시작하는 명령어 목록을 간략히 보여준다.

<a id="cdca738fbc1e293d"></a>
#### 사용 예
...
<a id="4567f53922789606"></a>
### `\host`

<a id="64deddbeab456b35"></a>
#### 구문
...
<a id="1663b40a11a4fbca"></a>
### `\history`

<a id="412ea5a155a5f8ad"></a>
#### 구문
...
<a id="4f37fcdb47b72343"></a>
#### 설명

gsql 프로그램을 수행한 이후의 SQL 구문 수행 목록을 보여준다.  
수행에 성공한 SQL 구문만 관리하며, gsql (`\`)로 시작하는 대화형 명령어로 수행한 명령어나 실패한 SQL 구문은 포함하지 않는다.  
해당 목록을 참조하여 [`\\`](#cd404cd8f381035e) 명령이나 [`\{n}`](#c2358dd22431286d) 명령을 통해 이전에 수행한 SQL 구문을 다시 수행할 수 있다.  
관리할 수 있는 SQL 구문의 개수는 [`\set history`](#aee5297afda7a6bc) 명령을 통해 제어한다.

<a id="131f3a65b0a5deb1"></a>
#### 사용 예
...
<a id="593235c21c0d4913"></a>
### `\import`

<a id="6e042be448766705"></a>
#### 구문
...
<a id="21cb66ede83065c4"></a>
### `\idesc`

<a id="206d1950f56ac0fc"></a>
#### 구문
...
<a id="cea6d4c5ad03e77c"></a>
#### 설명

인덱스의 구조 정보를 조회한다.

인덱스 이름은 다음과 같이 인덱스 이름만 기술하거나 스키마 이름과 함께 기술할 수 있다. 스키마 이름을 명시하지 않은 경우, 인덱스의 스키마 이름은 사용자의 [Schema Path](../part-03-sql-manual/13-sql-objects.md#3d5355367b16fe83)에 의해 결정된다.
...
<a id="c2358dd22431286d"></a>
### `\{n}`

<a id="eaca5a8e3247b1b9"></a>
#### 구문
...
<a id="6cba4dd309ab07da"></a>
#### 설명

[`\history`](#1663b40a11a4fbca) 명령으로 조회할 수 있는 SQL 수행 이력 중에 number에 해당하는 구문을 수행한다.  
number 값은 [`\history`](#1663b40a11a4fbca) 수행 결과의 ID 값이어야 한다.

<a id="65fc65b0bc47dc1e"></a>
#### 사용 예
...
<a id="b6b45d20d8520418"></a>
### `\prepare sql`

<a id="81ba2fd4ab10f93d"></a>
#### 구문
...
<a id="87f5f0e5ccce90ac"></a>
#### 설명

&lt;sql_statement&gt;에 기술된 SQL 구문을 준비한다. 준비된 SQL 구문은 [`\exec`](#d6c532ff1c99e2c0) 명령을 통해 반복적으로 수행할 수 있다.

`\prepare sql` 명령은 ODBC의 [SQLPrepare](../part-05-developer-manual/34-odbc.md#e3672320cde879bd) 함수와 JDBC의 Connection::[prepareStatement](../part-05-developer-manual/35-jdbc.md#d89e5433919f5cb8) 함수와 유사하게 동작한다.

<a id="dadfd55281a5f802"></a>
#### 사용 예
...
<a id="4eefbe4e20e65218"></a>
### `\print`

<a id="2d8eae7f5d8127e3"></a>
#### 구문
...
<a id="1f2f97febe56b040"></a>
#### 설명

다음과 같이 [`\var`](#f4b746e481fc748e) 명령으로 선언한 호스트 변수 값을 조회한다. 호스트 변수 값이 설정되지 않은 경우 NULL 값을 가진다.
...
<a id="d8d224101441a6bd"></a>
### `\quit`

<a id="b1123bfc9a2d7469"></a>
#### 구문
...
<a id="3effaa544e2a7041"></a>
#### 설명

gsql을 종료한다.   
종료할 때 COMMIT 되지 않은 트랜잭션을 모두 COMMIT 한다.

<a id="74007b088df9e2a9"></a>
#### 사용 예
...
<a id="c4acc79598200530"></a>
### `\set autocommit`

<a id="3a8f6cb51e16f5f2"></a>
#### 구문
...
<a id="a08c5074419aff63"></a>
### `\set autotrace`

<a id="080352fa906ace4d"></a>
#### 구문
...
실행 계획에 대한 자세한 내용은 [SQL 실행 계획](../part-03-sql-manual/15-sql-tuning.md#d840f5576fcd85fa)을 참조한다.

<a id="fa3bf7083dc51be2"></a>
#### 사용 예
...
<a id="dd290c169cfac9e1"></a>
### `\set color`

<a id="f2d5c7786b0030a6"></a>
#### 구문
...
<a id="441592ff185b225b"></a>
### `\set colsize`

<a id="0822522484fc270f"></a>
#### 구문
...
<a id="40bc02d751263b98"></a>
### `\set ddlsize`

<a id="a6ee7fad0aab310e"></a>
#### 구문
...
<a id="56808516791920e5"></a>
### `\set error`

<a id="210d284a0c0539a8"></a>
#### 구문
...
<a id="aee5297afda7a6bc"></a>
### `\set heading`

<a id="699eb55ae56d9a74"></a>
#### 구문
...
<a id="f368e6ade4217067"></a>
#### 설명

질의 결과에 헤더를 출력할지 여부를 결정한다.

<a id="bcb90ed8687583aa"></a>
#### 사용 예
...
<a id="ea8f4db66f5c3aca"></a>
### `\set history`

<a id="e3584772649fa96d"></a>
#### 구문
...
history 정보는 [`\\`](#cd404cd8f381035e) 명령이나 [`\{n}`](#c2358dd22431286d) 명령을 통해 이전에 수행한 SQL 구문을 다시 수행하려 할 때 사용한다.

<a id="e93b807ddd1e0cbc"></a>
#### 사용 예
...
<a id="0aafbad106cb96b3"></a>
### `\set linesize`

<a id="68fd31f9b9b55d22"></a>
#### 구문
...
<a id="efaea7c6ca4602a2"></a>
### `\set numsize`

<a id="5e9199b25cd026d4"></a>
#### 구문
...
<a id="4e095418cd281bd8"></a>
### `\set pagesize`

<a id="a3440901583b7d0f"></a>
#### 구문
...
<a id="fe16c585ae081840"></a>
### `\set serveroutput`

<a id="54007657a980b167"></a>
#### 구문
...
<a id="1abae99308a1b9bd"></a>
### `\set sqlprompt`

<a id="1f46c5b6125e9cd3"></a>
#### 구문
...
<a id="1765ef6b96453876"></a>
### `\set time`

<a id="ec229d6e30f96e93"></a>
#### 구문
...
<a id="8577f76f19ef19c0"></a>
### `\set timing`

<a id="99598314c33f3792"></a>
#### 구문
...
<a id="7716ae4d86e07740"></a>
### `\set vertical`

<a id="9e682f415c61f6f7"></a>
#### 구문
...
<a id="90bad908cd317516"></a>
### `\shutdown`

<a id="3bf53be1da529bf5"></a>
#### 구문
...
<a id="0e8da3f9da8d8834"></a>
#### 설명

GOLDILOCKS 서버를 종료한다.  
`\shutdown` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속하는 방법은 [서버 구동 및 종료](#1cd0f4e1950f2f7f)를 참조한다.
...
<a id="78d64fbff9f6ca0d"></a>
### `\spool`

<a id="e5e9b64441e5a8f5"></a>
#### 설명

gsql을 사용하여 출력되는 모든 결과를 terminal과 file에 기록한다.

<a id="6f4dce80bd076163"></a>
#### `\spool 'filename'`

<a id="56303cff5e8a420a"></a>
##### 구문
...
<a id="c38f2e769c2dac9d"></a>
#### `\spool OFF`

<a id="1217deb58ce61195"></a>
##### 구문
...
<a id="63b9db0a5a1806a2"></a>
#### `\spool`

<a id="284e588507a4cada"></a>
##### 구문
...
<a id="9eec5d5c59b4ab1a"></a>
### `\startup`

<a id="861f1929a1b8e07a"></a>
#### 구문
...
<a id="56f213b3b96b71fa"></a>
#### 설명

GOLDILOCKS 서버를 구동한다.  
`\startup` 명령을 수행하려면 SYSDBA 또는 ADMIN role로 접속해야 한다. SYSDBA로 접속 방법은 [서버 구동 및 종료](#1cd0f4e1950f2f7f)를 참조한다.
...
서버의 구동 단계는 NOMOUNT, MOUNT, OPEN 단계로 구분되며 자세한 내용은 [다단계 시작](../part-02-administration-manual/5-goldilocks-데이터베이스-관리-기본.md#1b66f042f1c4f4be)을 참조한다.   
`\startup` 명령을 수행한 후에 다음 단계로 전이하려면 [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13)를 참조한다.

<a id="7a6ec02fffd66318"></a>
#### 사용 예
...
<a id="f4b746e481fc748e"></a>
### `\var`

<a id="98e64622a169a244"></a>
#### 구문
...
<a id="ed84c7a476311f32"></a>
#### 설명

호스트 변수를 선언한다.  
variable_name과 동일한 호스트 변수가 선언되어 있을 경우, 기존의 호스트 변수를 삭제하고 새로 선언한다. variable_name의 최대 길이는 128 byte 이다.  
호스트 변수의 data_type은 GOLDILOCKS의 데이터 타입과 동일한데 자세한 내용은 [Data Type](../part-03-sql-manual/11-sql-elements.md#0e511565dcb79955)을 참조한다.  
호스트 변수는 [`\exec :var := value`](#90a8efde9c6fcbff) 명령을 이용하여 값을 대입하며, [`\print`](#4eefbe4e20e65218) 명령을 통해 값을 조회할 수 있다.
...
호스트 변수는 SQL 구문의 입력 또는 출력 인자로 사용할 수 있으며 SQL 구문이나 [`\exec :var := value`](#90a8efde9c6fcbff) 명령에서 사용할 때는 colon (:) 기호를 호스트 변수 앞에 붙여 호스트 변수임을 명시해야 한다.

<a id="e75b402c78000ec8"></a>
#### 사용 예
...
[UPDATE name RETURNING .. INTO](../part-03-sql-manual/20-sql-references-h-z.md#4fc5ee649d3baf4f) 구문을 수행하는 다음 예에서 SET 절과 WHERE 절의 호스트 변수 :v_id 는 입력 인자로 사용되었으며 INTO 절의 :v_id는 출력 인자로 사용되었다.

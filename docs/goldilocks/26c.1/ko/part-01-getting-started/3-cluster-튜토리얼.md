<a id="a71dc3a5ebc81373"></a>

# 3. Cluster 튜토리얼

> 원본: [GOLDILOCKS 26c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/26c_1/manual/ko/a71dc3a5ebc81373)  
> 태그: `26c.1_0_tag`

[← 2. 튜토리얼](2-튜토리얼.md) · [전체 목차](../README.md) · [4. What's New →](4-what-s-new.md)

<a id="d48d8db80b979d48"></a>
...
C/S 모델의 dedicated 모드에서 GOLDILOCKS cluster system을 시작하거나 종료하려고 할 경우, 즉 gsqlnet 을 이용하고자 할 경우에는 [listener](../part-06-utility-manual/43-glsnr.md#45fbbb5be8315566)가 실행되어 있어야 한다.
...
- [ALTER CLUSTER GROUP name ADD MEMBER](../part-03-sql-manual/18-sql-references-a-b.md#1b3565c76f35ea0a)
- [CREATE CLUSTER GROUP](../part-03-sql-manual/19-sql-references-c-g.md#cb13f01b6a67d24a)
...
> GOLDILOCKS cluster system에서 단일 노드만 종료하는 경우에 사용할 수 있는 종료 모드는 ABORT가 유일하다. ABORT 모드에서는 접속 중인 session들의 상태와 관계없이 gmaster를 즉시 종료시켜 instance를 내린다.
...
<a id="d42d6e84fbcdc0ac"></a>
| 파일 이름 | Server | Client | 설명 |
| --- | --- | --- | --- |
| DBMS_LOCK.sql | O | X | DBMS_LOCK package 생성 스크립트 |
| DBMS_OUTPUT.sql | O | X | DBMS_OUTPUT package 생성 스크립트 |
| DBMS_SQL.sql | O | X | DBMS_SQL package 생성 스크립트 |
| DBMS_STANDARD.sql | O | X | DBMS_STANDARD package 생성 스크립트 |

<a id="8eb15bcbf8edbf0f"></a>
...
다음은 build-in package 를 사용하기 위해 package 를 생성하는 방법이다.
...
테이블 생성에 대한 자세한 내용은 [CREATE TABLE](../part-03-sql-manual/19-sql-references-c-g.md#ce6ecbcf1ea593ea) 구문과 [Cluster Table과 Shard](../part-03-sql-manual/14-cluster-objects.md#daaa25a5e8a42038) 개념을 참조한다.

Sharding 정책 옵션을 생략할 경우 [DEFAULT_SHARDING](../part-02-administration-manual/10-server-property.md#1a409609d661f5ac) 프로퍼티에 의해 결정된다. DEFAULT_SHARDING 의 기본값은 0으로써 cloned table을 생성한다.
...
- AT CLUSTER WIDE 
    ...
    - Cluster group과 cluster member를 추가할 때 [ALTER TABLE name REBALANCE](../part-03-sql-manual/18-sql-references-a-b.md#4dcbc8cc43487ef2) 구문을 통해 clone을 재배치할 수 있다. 
- AT CLUSTER GROUP group_list 
    - 지정한 cluster group들의 모든 cluster member에 clone을 배치한다. 
    - 지정된 cluster group에 cluster member를 추가할 때 [ALTER TABLE name REBALANCE](../part-03-sql-manual/18-sql-references-a-b.md#4dcbc8cc43487ef2) 구문을 통해 clone을 재배치할 수 있다. 
...
테이블 row들은 id column의 해시값을 기준으로 24 개의 shard 중 하나에 분배되며, 24 개의 shard는 cluster system 전체에 골고루 배치된다. 자세한 내용은 [Cluster Table과 Shard](../part-03-sql-manual/14-cluster-objects.md#daaa25a5e8a42038)를 참조한다.

<a id="4c8936ce3f971789"></a>
#### Range Sharding Strategy
...
다음은 생성되는 list shard들을 CLUSTER WIDE하게 배치되도록 테이블을 생성하는 구문의 예이다. 테이블을 생성한 후에 추가되는 cluster group을 포함하도록 shard를 재배치하기 위해 REBALANCE 기능을 사용할 수 있다.
...
<a id="bf4d3d87222f121e"></a>
### 인덱스 관리

<a id="2c3669a673b54881"></a>
...
사용자는 테이블을 생성할 때 global secondary index의 생성 여부를 [DEFAULT_GLOBAL_SECONDARY_INDEX_CREATION](../part-02-administration-manual/10-server-property.md#c51bcdce7adcc3db)을 통해 선택할 수 있고 테이블을 생성한 후에 생성된 global secondary index를 삭제하거나 다시 생성할 수도 있다. global secondary index는 테이블당 한 개만 생성할 수 있다.
...
Standalone에서 사용되는 시퀀스와 마찬가지로 global 시퀀스 객체에서도 cache size를 생성 옵션으로 지정할 수 있는데, 이는 global 시퀀스 객체로부터 몇 개의 시퀀스 값들을 획득하여 local cache에 적재해 둘 것 인지를 의미한다. 다음은 cache size를 5로 지정하여 global 시퀀스 객체를 생성하였을 때 각 member 노드의 local cache 상황과 NEXTVAL 호출 시의 반환값을 정리한 것이다.

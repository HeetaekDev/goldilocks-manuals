<a id="055ebafc00db2920"></a>

# 7. GOLDILOCKS 데이터베이스의 백업과 복구

> 원본: [GOLDILOCKS 26c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/26c_1/manual/ko/055ebafc00db2920)  
> 태그: `26c.1_0_tag`
...
<a id="d7b9c507f7d995c7"></a>
## 백업과 복구

<a id="2625c9bead9d2325"></a>
### 백업

<a id="c6f8c04a31d606c1"></a>
#### 백업/ 복구 목적
...
<a id="c7b1a1feffbde7ec"></a>
![Incremental backup](../assets/images/bc8c198ffe5633be.png)

<a id="41400df9c612401d"></a>
##### 데이터베이스 증분 백업
...
체인지 트래킹이 enable 되면 체인지 트래킹 파일과 공유 메모리를 생성한다. 체인지 트래킹 파일과 공유 메모리의 구조는 동일하고 [CHANGE_TRACKING_EXTENT_SIZE](10-server-property.md#be1cff326f96d642) 프로퍼티에 설정된 개수만큼의 페이지 묶음에 대한 변경 플래그를 저장하는 블록들로 구성된다.
...
체인지 트래킹이 enable 되면 기본적으로 [CHANGE_TRACKING_FILE](10-server-property.md#4f436a868b271594) 프로퍼티에 설정된 위치에 파일이 생성된다. enable 할 때 체인지 트래킹을 위한 파일 이름이나 저장 위치까지 설정하면 설정된 위치에 파일이 생성되고, 저장 위치가 설정되지 않은 경우에는 SYSTEM_TABLESPACE_DIR 프로퍼티에 설정된 위치에 체인지 트래킹 파일이 생성된다.
...
<a id="16fc66535d28376c"></a>
##### 병렬 증분 백업

병렬로 여러 개의 백업 파일을 생성할 수 있는 기능이다.

백업 파일은 데이터 파일과 1:N 의 구조를 갖는다. 즉, 한 개의 백업 파일에는 여러 개의 데이터 파일이 저장되고 한 개의 데이터 파일은 여러 백업 파일에 나뉘어 저장되지 않는다.

병렬 백업은 반드시 분할 계수 (piece 개수)를 지정해야 하며, 병렬 계수 (parallel factor)가 분할 계수보다 큰 경우는 병렬 계수가 분할 계수로 조정된다.

다음은 네 개의 thread로 병렬 백업을 수행하는 예이다.  
'%p' 는 piece format specifier 이고 총 네 개의 backup piece 파일이 만들어 진다.
...
<a id="e6c25a4f47ec3cff"></a>
![Recovery를 위한 최소 checkpoint LSN을 구하는 과정](../assets/images/b522e772450f6f3e.png)

<a id="2efcb7bca703d86f"></a>
...
불완전 복구를 완료할 SCN을 확인한 후, 해당 로그의 SCN까지만 복구한다. Standalone 환경에서는 SCN의 lcn만 유효하기 때문에, 예를 들어 lcn 300까지 복구하는 경우 다음과 같이 수행한다. 클러스터 환경에서 특정 SCN까지의 복구 방법은 [클러스터 환경에서 불완전 복구](#b14eb524769c1b7b) 절을 참조한다.
...
> SCN이 순서대로 로그에 쓰여지지 않기 때문에 SCN '0.0.300'까지 수행했을 때 그 이상 복구될 수도 있다.  
> 다음은 SCN이 역전된 경우에 복구하는 예이다.  
>   
> 로그: --- LSN 90 (SCN '0.0.3') -- LSN 91 (SCN '0.0.5') -- LSN 92 (SCN '0.0.4')  
>   
> gSQL> ALTER DATABASE RECOVER UNTIL CHANGE SCN '0.0.3';  
> → LSN 90까지 복구된다.  
>   
> gSQL> ALTER DATABASE RECOVER UNTIL CHANGE SCN '0.0.4';  
> → LSN 92까지 복구된다. (SCN '0.0.4'가 있는 LSN 92까지 복구한다.)  
>   
> gSQL> ALTER DATABASE RECOVER UNTIL CHANGE SCN '0.0.5';  
> → LSN 92까지 복구된다. (SCN '0.0.5'까지 복구할 때 임의의 SCN '0.0.4'와 SCN '0.0.5'가 모두 복구된다.)
...
<a id="95b8b8dc78e5d033"></a>
#### 복구 예

<a id="182f0ff9e79bf2ac"></a>
##### 제어 파일 훼손
...
<a id="b14eb524769c1b7b"></a>
#### 클러스터 환경에서 불완전 복구

클러스터 환경에서도 [불완전 복구](#d82c357b74443413)를 수행할 수 있다.   
다만, 클러스터의 특성상 모든 멤버의 시점 (SCN) 을 동일하게 맞추어야 데이터베이스 정합성을 보장할 수 있다.   
이를 위해 다음과 같이 세 단계 절차로 불완전 복구를 수행한 후 데이터베이스를 재시작한다.  
이 과정을 통해 모든 멤버의 GCN을 동일하게 맞추고, 각 group 내 멤버들의 DCN 또한 일치시킨다.

1. 임의의 멤버에서 불완전 복구 수행  
   - LSN, SCN 또는 시간을 기준으로 특정 시점까지 복구를 수행한다.  
   - 복구 완료 후 해당 멤버의 최신 SCN을 확인한다.
2. 다른 그룹의 멤버에서 불완전 복구 수행  
   - 1 단계에서 복구를 수행한 멤버가 속한 group 을 제외한 각 group에서 임의의 멤버를 선택한다.  
   - 1 단계에서 확인한 최신 SCN 의 GCN 을 기준으로 복구를 수행한다.  
   - 복구 완료 후 해당 멤버의 최신 SCN을 확인한다.
3. 나머지 멤버에서 불완전 복구 수행  
   - 1, 2단계에서 확인한 SCN의 GCN 및 DCN을 기준으로 나머지 모든 멤버에 대해 복구를 수행한다.

예를 들어, 각 group 에 두 개의 member가 있는 두 개의 group (G1{g1n1, g1n2}, G2{g2n1, g2n2}) 으로 구성된 cluster 환경을 가정한다. 이 경우에도 cluster 구성과 상관없이, 먼저 임의의 member 에서 원하는 시점까지 불완전 복구를 수행한 후에 위 절차에 따라 나머지 멤버에 대한 복구를 진행한다.

1. G1 group의 g1n1에서 특정 시각 '2026-03-09 16:38:35.148078'까지 불완전 복구를 수행한 후, 복구된 최신 SCN을 확인한다.
...
2. G2 group의 g2n1에서 scn '11.*'까지 불완전 복구를 수행한 후, 복구된 최신 scn을 확인한다.
...
3. G1, G2 group의 나머지 멤버들 (g1n2, g2n2) 에 대해, 1, 2단계에서 확인한 group 별 최신 SCN을 기준으로 불완전 복구를 수행한다.
...
이를 위해 GOLDILOCKS는 commit된 global transaction 정보를 MEM_TRANS_TBS에 트랜잭션 레코드 형태로 기록하고 해당 레코드에 새로운 레코드가 기록될 때 필요에 따라 이전 레코드를 글로벌 트랜잭션 로그파일에 기록한다. 이후 비정상 종료된 member는 log를 이용하여 재시작 복구 또는 수동 복구를 수행하고 'PREPARE' 상태인 in doubt 트랜잭션이 남아 있으면 서비스 중인 member들로부터 해당 트랜잭션의 COMMIT/ ROLLBACK 정보를 얻어 복구한다.

GOLDILOCKS는 데이터베이스를 생성할 때 두 개의 로그 그룹으로 구성된 글로벌 트랜잭션 로그 파일을 미리 생성하여 in doubt 트랜잭션의 정보를 로그 파일에 기록한다. 이 때 로그 파일이 가득 차면 다음 로그 파일을 사용한다. 가득 찬 로그 파일은 archive 시스템 thread에 의해 archiving 된 후 재사용된다. 글로벌 트랜잭션 로그 파일은 ARCHIVELOG 모드와 관계없이 클러스터 환경에서 무조건 archiving 된다. 이는 이전 in doubt 트랜잭션의 정보를 참조할 수 있도록 하기 위함이다.

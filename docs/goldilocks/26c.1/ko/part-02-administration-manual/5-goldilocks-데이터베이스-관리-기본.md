<a id="5cd26cc480b4ba12"></a>

# 5. GOLDILOCKS 데이터베이스 관리 기본

> 원본: [GOLDILOCKS 26c.1 User Manual (ko)](https://manual.sunjesoft.co.kr/goldilocks/26c_1/manual/ko/5cd26cc480b4ba12)  
> 태그: `26c.1_0_tag`
...
<a id="5b69643145287e8d"></a>
## GOLDILOCKS 데이터베이스 생성과 구성

<a id="47dbdd90631e1d41"></a>
### 데이터베이스 생성
...
<a id="8dddce64958f94e1"></a>
<table><thead><tr><th align="center">고려 사항</th><th align="center">참고</th></tr></thead><tbody><tr><td valign="middle">데이터베이스에서 사용할 테이블, 인덱스가 사용할 공간의 크기를 고려해야 한다.</td><td valign="middle"><ul><li><a href="6-goldilocks-데이터베이스의-구조-및-저장-구조.md#4cee6e4e7a3cd2d1">GOLDILOCKS 데이터베이스의 구조 및 저장 구조</a></li></ul></td></tr><tr><td valign="middle">데이터베이스의 파일들이 생성될 위치를 고려해야 한다. 파일들을 적절히 분배하여 disk IO를 분산함으로써 데이터베이스 성능을 향상시킬 수 있기 때문이다. 예를 들어, redo log file을 별도의 디스크에 배치하거나 스트라이핑하고 데이터 파일을 여러 개의 디스크로 분산시켜 disk IO를 분산시키고 병렬 disk IO를 수행할 수 있다.</td><td valign="middle"><ul><li><a href="6-goldilocks-데이터베이스의-구조-및-저장-구조.md#8eb835a2cdd387c3">Redo Log File 관리</a></li></ul></td></tr><tr><td valign="middle">서버 프로퍼티 파일에 설정된 각 프로퍼티의 개념과 동작을 숙지하고 지속적으로 관리해야 한다.</td><td valign="middle"><ul><li><a href="#80aa9e47a271eabd">초기 Property 설정</a></li><li><a href="#318c0c1f59e62c60">GOLDILOCKS 설정파일을 이용한 초기 Property 관리</a></li><li><a href="10-server-property.md#e95fbf953ce3e17d">Server Property</a></li></ul></td></tr></tbody></table>

위의 고려 사항 외에도 [Getting Started](../part-01-getting-started/README.md#c2f4e0b29f22934b) 매뉴얼의 [Database 생성](../part-01-getting-started/2-튜토리얼.md#742a53122599ef9a) 부분을 참조하여 데이터베이스를 생성한다.
...
>   
> 자세한 내용은 [ALTER SYSTEM SET property_name](../part-03-sql-manual/18-sql-references-a-b.md#d0c54353bc77cbe4), [ALTER SYSTEM RESET property_name](../part-03-sql-manual/18-sql-references-a-b.md#922972ea95cf99e3)을 참조한다.
...
- Nomount에서 관리자가 할 수 있는 작업
    - Nomount 프로퍼티 조정 
    - 자세한 내용은 [ALTER SYSTEM MOUNT DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13), [ALTER DATABASE RESTORE CONTROLFILE FROM 'file_name'](../part-03-sql-manual/18-sql-references-a-b.md#60ea018060bd0c35) 을 참조한다.
...
- Mount에서 관리자가 할 수 있는 작업
    - Mount 프로퍼티 조정 
    - [ALTER SYSTEM {MOUNT | OPEN} DATABASE](../part-03-sql-manual/18-sql-references-a-b.md#cbf215370461aa13)
    - [ALTER DATABASE ADD LOGFILE](../part-03-sql-manual/18-sql-references-a-b.md#975c069a4f386a7c)
    - [ALTER DATABASE DROP LOGFILE](../part-03-sql-manual/18-sql-references-a-b.md#512d71b4f3b2f884)
    - [ALTER DATABASE RENAME GLOBAL TRANSACTION LOGFILE](../part-03-sql-manual/18-sql-references-a-b.md#99bcbcc9f4d63ec3)
    - [ALTER DATABASE RENAME LOGFILE](../part-03-sql-manual/18-sql-references-a-b.md#0e0424376b3a0ab9)
    - [ALTER DATABASE { ARCHIVELOG | NOARCHIVELOG }](../part-03-sql-manual/18-sql-references-a-b.md#052eab7887cba477)
    - [ALTER DATABASE DELETE BACKUP](../part-03-sql-manual/18-sql-references-a-b.md#d1c1804d37182cca)
    - [ALTER DATABASE REGISTER](../part-03-sql-manual/18-sql-references-a-b.md#a996ea8c8491c041)
    - [ALTER DATABASE RECOVER](../part-03-sql-manual/18-sql-references-a-b.md#233fbcf85decd22e)
    - [ALTER DATABASE RESTORE](../part-03-sql-manual/18-sql-references-a-b.md#60ea018060bd0c35)
    - [ALTER SESSION SET property_name](../part-03-sql-manual/18-sql-references-a-b.md#2a4ad434eedabea8)
    - [ALTER SYSTEM RESET property_name](../part-03-sql-manual/18-sql-references-a-b.md#922972ea95cf99e3)
    - [ALTER SYSTEM SWITCH LOGFILE](../part-03-sql-manual/18-sql-references-a-b.md#3e74ff8636ece361)
    - [ALTER SYSTEM { KILL | DISCONNECT } SESSION](../part-03-sql-manual/18-sql-references-a-b.md#864ac81bb69e4ec0)
    - [ALTER TABLESPACE name ADD [DATAFILE|MEMORY]](../part-03-sql-manual/18-sql-references-a-b.md#0ed6c4c98eaca683)
    - [ALTER TABLESPACE name RENAME DATAFILE](../part-03-sql-manual/18-sql-references-a-b.md#53130577648dcf50)
    - [ALTER TABLESPACE name [ONLINE|OFFLINE]](../part-03-sql-manual/18-sql-references-a-b.md#b9a7e09545483657)
...
<a id="50185575344810f9"></a>
#### Timer Thread

타이머 thread는 사용자 트랜잭션들의 시간 측정 비용을 줄이기 위해 비동기적으로 시스템에 시간을 설정하며 사용자 트랜잭션들은 시스템에 설정된 시간을 읽는다.  
시간의 정밀도는 [TIMER_INTERVAL](10-server-property.md#98833ef747fb4e1b)에 따라 설정되는데 기본값은 10 ms 이다.

타이머 thread에서 설정된 시간을 사용하는 경우는 다음과 같다. 이 때 TIMER_INTERVAL 값만큼 오차가 발생한다. 예를 들어 TIMER_INTERVAL이 10 ms 인 경우, 오차는 10 ms 이다.
...
디스크 테이블스페이스에 저장된 테이블, 인덱스 페이지들이 버퍼에 캐싱되어 변경되면 체크포인트 리스트에 연결되고, 버퍼 캐쉬에서 재사용을 위해 갱신된 페이지들은 버퍼 replace list에 연결된다. I/O 슬레이브 thread들은 체크포인트 시 또는 주기적으로 IO 체크포인트 리스트와 버퍼 replace list의 페이지를 디스크에 반영한다.

변경된 페이지를 저장할 때는 한꺼번에 가능한 많이 저장하는 것이 성능 향상에 도움이 된다. 한 번에 기록되는 페이지 개수는 [MAXIMUM_FLUSH_PAGE_COUNT](10-server-property.md#13765a88902e4073) 프로퍼티에 따른다.
...
<a id="6c51a2b3c5cddf7f"></a>
### 리스너 프로세스

리스너 프로세스는 Client/ Server 환경에서 네트워크를 통한 원격 접속을 가능하게 한다. 리스너 프로세스는 [LISTEN_PORT](../part-06-utility-manual/43-glsnr.md#2e26b65f2d2dd35a)로 클라이언트 접속을 기다리는데 클라이언트가 접속할 때 dedicated 모드일 경우, 새로운 gserver를 구동하여 클라이언트와 연결하고 shared 모드일 경우, load-balancer (gbalancer)를 통해 부하가 적은 dispatcher (gdispatcher)를 선택하여 클라이언트와 연결한다.
...
<a id="cb835e259fe4ea56"></a>
## 메모리 관리

<a id="23f47d588ba80569"></a>
### GOLDILOCKS 메모리 구조

GOLDILOCKS는 시스템의 모든 세션에서 공유하기 위한 메모리 (SSA)와 데이터베이스 페이지를 위한 공유 메모리, 각 세션마다 독립적으로 사용하는 heap 전용 메모리 (PSA)들을 사용한다.

<a id="0b9507292ab77538"></a>
![Shared memory](../assets/images/4252a5e3b985db94.png)

<a id="5e08cc8e8b51edf6"></a>
### SSA 관리
...
SSA의 물리적 시작 주소는 [SHARED_MEMORY_STATIC_KEY](10-server-property.md#35ce92e943afa27c)와 [SHARED_MEMORY_ADDRESS](10-server-property.md#1f174eb8af0095ae)에 의해 결정된다. 만약 다른 프로그램에서 같은 SHARED_MEMORY_STATIC_KEY에 지정된 공유 메모리 key와 SHARED_MEMORY_ADDRESS에 지정된 메모리를 이미 사용하고 있다면 다음과 같은 에러가 발생한다.
...
SSA 크기는 [SHARED_MEMORY_STATIC_SIZE](10-server-property.md#eadde14e86afa8c7)에 의해 결정된다. Session/ lock/ transaction pool 및 dictionary cache에서 사용되는 메모리들은 시스템에서 자동으로 관리하며 사용자가 임의로 사용량을 제어할 수 없는 반면에 log buffer와 plan cache는 사용자가 임의로 제어할 수 있다.
...
<a id="ffbc94281cf6d8d5"></a>
### PSA 관리

Private Static Area (PSA)는 각 세션마다 독립적으로 사용하는 heap 메모리 영역이다. 최대 크기는 [PRIVATE_STATIC_AREA_SIZE](10-server-property.md#ac29014e15c42d84)에 의해 결정된다.
...
<a id="d84f5939fcde0602"></a>
### TRACE 파일을 이용한 모니터링

GOLDILOCKS 데이터베이스는 instance의 시작부터 종료까지 시스템 전반적인 오류, 경고, 정보를 기록하는 시스템 로그와 XA 트랜잭션 로그, DDL 로그와 같은 trace 로그를 제공한다. 또한 SQL trace log를 제공하는데 자세한 내용은 [SQL Trace Log](../part-03-sql-manual/15-sql-tuning.md#c6ea0344490bf8ee)를 참조한다.

<a id="6351d690f53bc4db"></a>
#### TRACE 파일 관리
...
<a id="181c70b2820802bc"></a>
#### XA 로그

분산 트랜잭션을 처리하기 위한 XA 트랜잭션 인터페이스 start, close, end, rollback, prepare, commit, recover, forget 등에 대한 성공, 실패 로그를 기록한다. GOLDILOCKS 데이터베이스는 XA 추적 로그를 기본적으로 기록하지 않도록 한다. XA 추적 로그를 기록하기 위해서는 다음과 같이 TRACE_XA를 ON시켜야 한다. XA 트랜잭션 인터페이스의 자세한 스펙은 [XA API References](../part-05-developer-manual/34-odbc.md#8b6862223af1b090)를 참조한다.
...
'xa.trc'에 기록되는 XA 추적 로그는 다음과 같다. 먼저 수행된 XA 인터페이스가 기록되고 성공 (complete) 또는 실패 (failed) 여부가 기록된다. 수행된 세션의 세션 ID와 XA 트랜잭션 ID 등의 정보가 함께 기록되고 실패할 경우 [XA API References](../part-05-developer-manual/34-odbc.md#8b6862223af1b090)에 정의된 에러 코드가 기록된다.
...
<a id="af9eafde28f44c07"></a>
#### 이중화 Trace 로그

GOLDILOCKS의 이중화 툴인 CYCLONE 및 LOGMIRROR를 이용할 때 별도의 파일로 이중화 관련 추적 로그를 기록한다. 이중화 추적 로그는 CYCLONE의 [운영하기](../part-07-replication/55-cyclone.md#af77edebd57b24ca) 및 LOGMIRROR의 [운영하기](../part-07-replication/56-logmirror.md#4ecdf4bf57afad0e)을 참조한다.

<a id="452f7154fd7208e9"></a>
#### Listener 로그

Listener 프로세스가 시작된 후 종료될 때까지 발생하는 오류와 정보를 listener 로그에 기록한다.

<a id="bf2708ee48332b71"></a>
##### Listener 로그 포맷
...
V$LOCK_WAIT를 이용하여 데드락을 유발한 트랜잭션들을 찾을 수 있는데 관리자는 이를 모니터링하여 데드락을 해제해야 한다. V$LOCK_WAIT에 대한 자세한 내용은 [V$LOCK_WAIT](9-database-information.md#c270548a9fba5427)를 참조한다. 또한 데드락의 원인인 래치 아이템을 모니터링 하기 위해서는 [V$LATCH](9-database-information.md#ed7103f33d8ddfb1)를 참조한다.
